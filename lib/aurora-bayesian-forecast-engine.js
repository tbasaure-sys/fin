...
function getSemiconductorTwin(input = {}) {
  return input?.semiconductorTwin || input?.sectorTwin?.semiconductorTwin || null;
}
...
function updateDistribution(dist = {}, meanDelta = 0, sdMultiplier = 1) {
  const mean = clamp(numeric(dist.mean, numeric(dist.p50, 0)) + meanDelta, dist.min, dist.max);
  const sd = Math.max(1e-6, numeric(dist.sd, 0.01) * clamp(sdMultiplier, 0.55, 2.8));
  return {
    ...dist,
    mean,
    p10: clamp(mean - sd * 1.2816, dist.min, dist.max),
    p50: mean,
    p90: clamp(mean + sd * 1.2816, dist.min, dist.max),
    sd,
  };
}

function applySectorTwinAdjustments(posterior, semiconductorTwin = null) {
  if (!semiconductorTwin?.applicable) {
    return {
      posterior,
      adjustment: {
        applied: false,
        reason: semiconductorTwin ? "semiconductor_twin_not_applicable" : "semiconductor_twin_missing",
      },
    };
  }
  const adjustments = semiconductorTwin.adjustments || {};
  const scores = semiconductorTwin.scores || {};
  const bottleneckEvidenceDelta = clamp(numeric(adjustments.bottleneckEvidenceDelta, 0), -0.12, 0.14);
  const marginDurabilityDelta = clamp(numeric(adjustments.marginDurabilityDelta, 0), -0.12, 0.14);
  const reinvestmentConfidenceDelta = clamp(numeric(adjustments.reinvestmentConfidenceDelta, 0), -0.12, 0.12);
  const cycleRiskPenalty = clamp(numeric(adjustments.cycleRiskPenalty, 0), 0, 0.18);
  const uncertaintyMultiplier = clamp(numeric(adjustments.forecastUncertaintyMultiplier, 1), 0.92, 1.58);
  const deltas = {
    growth: clamp(bottleneckEvidenceDelta * 0.25 + reinvestmentConfidenceDelta * 0.12 - cycleRiskPenalty * 0.35, -0.05, 0.04),
    margin: clamp(marginDurabilityDelta - cycleRiskPenalty * 0.25, -0.08, 0.08),
    roic: clamp(bottleneckEvidenceDelta * 0.7 - cycleRiskPenalty * 0.35, -0.08, 0.09),
    reinvestment: clamp(reinvestmentConfidenceDelta * 0.5 + Math.max(0, bottleneckEvidenceDelta) * 0.12 + cycleRiskPenalty * 0.18, -0.04, 0.08),
    wacc: clamp(cycleRiskPenalty * 0.22, 0, 0.045),
  };
  const next = JSON.parse(JSON.stringify(posterior));
  next.growth = updateDistribution(next.growth, deltas.growth, uncertaintyMultiplier);
  next.margin = updateDistribution(next.margin, deltas.margin, uncertaintyMultiplier);
  next.roic = updateDistribution(next.roic, deltas.roic, uncertaintyMultiplier);
  next.reinvestment = updateDistribution(next.reinvestment, deltas.reinvestment, uncertaintyMultiplier);
  next.wacc = updateDistribution(next.wacc, deltas.wacc, uncertaintyMultiplier);
  next.terminalGrowth = updateDistribution(next.terminalGrowth, 0, clamp(1 + Math.max(0, scores.cycleRisk - 0.5) * 0.18, 1, 1.18));

  return {
    posterior: next,
    adjustment: {
      applied: true,
      version: "aurora_sector_twin_forecast_adjustment_v1",
      source: "aurora_semiconductor_twin_v1",
      decision: semiconductorTwin.decision,
      state: semiconductorTwin.state,
      deltas,
      uncertaintyMultiplier,
      scores: {
        bottleneckDurability: numeric(scores.bottleneckDurability, null),
        cycleRisk: numeric(scores.cycleRisk, null),
        capacityPressure: numeric(scores.capacityPressure, null),
        inventoryOverhang: numeric(scores.inventoryOverhang, null),
        aspPower: numeric(scores.aspPower, null),
      },
      notes: [
        marginDurabilityDelta > 0 ? "Bottleneck and ASP evidence support margin durability." : null,
        cycleRiskPenalty > 0 ? "Capacity, inventory, or ASP cycle risk widens forecast uncertainty." : null,
        bottleneckEvidenceDelta > 0 ? "Scarcity evidence supports ROIC persistence." : null,
      ].filter(Boolean),
    },
  };
}
...
export function buildAuroraBayesianForecastEngine(input = {}, options = {}) {
  ...
  const semiconductorTwin = getSemiconductorTwin(input);
  ...
  const dependencePosterior = applyDependenceAdjustments(rawPosterior, { equilibrium, driverGraph });
  const sectorTwinResult = applySectorTwinAdjustments(dependencePosterior, semiconductorTwin);
  const posterior = sectorTwinResult.posterior;
  ...
  return {
    ...
    priorStack,
    dependencePosterior,
    sectorTwinAdjustment: sectorTwinResult.adjustment,
    posterior,
    scenarios,
    expectedFairValue,
    expectedReturn,
