...
export const JEV_RUBRIC = 'bls-evidence-v3';
...
export function questionsFor(kind, index) {
  ...
  if(kind==='investment_thesis')return {
    relation:choice(at+'Does the quoted evidence support the factual premise in item.premise? Assess ONLY the premise, separately from the conditional argument in item.statement. Preserve attribution and distinguish hypothetical risk disclosures from events that occurred.',{supports:'Every material factual clause of the premise has direct textual support',contradicts:'The quoted evidence contradicts the premise',context:'Relevant context but at least one factual premise is unsupported',unknown:'Insufficient evidence or ambiguous premise'}),
    reasoning:choice(at+'Assess the argument in item.statement given its premise and item.kind. A conditional investment hypothesis or proposed invalidation test is not expected to be a reported fact. Is it explicitly conditional and bounded, or does it smuggle in unsupported facts, causality, valuation or return certainty? Do not certify predictions.',{bounded:'Clearly conditional inference or proposed test with assumptions exposed, or a faithful reported fact',overreach:'Unsupported factual addition, causal certainty or forecast presented as established',unknown:'Cannot determine whether the argument respects the evidence'}),
    timing:choice(at+'For a catalyst item, compare its cited event date and actual occurrence to item.asOf. A completed dividend announcement, past fiscal year result or expired plan is historical even if it might have continuing effects. A fiscal year is not a calendar deadline. Upcoming requires explicit evidence of a future event; a growth trend alone is not a dated catalyst. For non-catalysts choose not_applicable.',{upcoming:'Explicit future milestone after the cutoff, not already completed',historical:'Completed event or elapsed deadline',undated:'Timing, occurrence or continuing status cannot be established',not_applicable:'Not a catalyst item',unknown:'Insufficient evidence to classify'}),
  };
  return {
