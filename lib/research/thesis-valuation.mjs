...
export const VALUATION_VERSION='bls-thesis-fcff-v1';
const DAY=86400000;
...
export function valueThesis(financial,assumptions,quote,at){
 ...
 const result={version:VALUATION_VERSION,status:'blocked',license:'conditional_research',performance:null,predictiveClaim:false,
  ticker:financial.ticker,base:null,scenarios:[],sensitivity:[],comparison:null,blockers,warnings:[...(financial.warnings||[])]};
 const cutoff=financial.asOf;
 if(!fresh(cutoff,at,20000)||!validFact(f.revenue,'USD',cutoff)||!validFact(f.ebit,'USD',cutoff)
  ||f.revenue.start!==f.ebit.start||f.revenue.end!==f.ebit.end||!bounded((Date.parse(f.revenue.end)-Date.parse(f.revenue.start))/DAY,330,380)
 ...
 if(!['cash','debt','shares'].every(k=>validFact(f[k],k==='shares'?'shares':'USD',cutoff)&&f[k].value>=0&&fresh(f[k].end,at,550))
  ||!(f.shares?.value>0)||f.cash?.end!==f.debt?.end)blockers.push('CAPITAL_FACTS_UNRESOLVED');
 ...
 if(!quoteOk)blockers.push('QUOTE_UNAVAILABLE');else if(!sharesOk)blockers.push('CURRENT_SHARES_UNRECONCILED');
...
export function portfolioImpact(portfolio,valuation,quote,at){
 const out={status:portfolio?.status==='available'?'empty':'unavailable',quantity:null,recordedWeight:null,baseDeltaUsd:null,scenarioDeltas:[],coverage:{priced:0,total:0},warnings:[]};
