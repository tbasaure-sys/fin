...
export function compileDossier(packet) {
  ...
  if(packet.revenueBreakdown&&(!sourceIds.has(packet.revenueBreakdown.sourceId)||
    packet.revenueBreakdown.partitions?.some(p=>p.sourceId!==packet.revenueBreakdown.sourceId)))throw Error('ORPHAN_REVENUE_SOURCE');
  const sections = topics.map((topic) => {
    if (packet.chunks.some(c => c.kind)) {
      const selection = selectEvidence(packet.chunks, packet.sources, topic);
      return {id: topic.id, ...selection, status: selection.extracts.length ? 'extracts_available' : 'no_extracts'};
    }
  ...
  return {
    version: packet.chunks.some(c => c.kind) ? 'bls-filing-dossier-v2' : 'bls-filing-dossier-v1',
    ...
    sections,
    revenueBreakdown:packet.revenueBreakdown??null,
    coverage: {
