...
export function documentBlocks(html) {
  return parseDocument(html, false).blocks;
}
export function documentStructure(html) {
  return parseDocument(html, true);
}
function parseDocument(html, structured) {
  // Streaming tokenizer: a multi-MB iXBRL filing must not create a browser DOM.
  const parser = new SAXParser(),
    blocks = [],
    stack = [], tables = [];
  let activeTable = null, tableDepth = 0;
  let text = "",
    row = false;
  const flush = () => {
    const t = normalize(text).replace(/\s*\|\s*$/, "");
    if (t.length >= (structured ? 1 : 5)) blocks.push(t);
    text = "";
  };
  ...
  parser.on("startTag", (t) => {
    ...
    if (t.tagName === 'table') {
      if (!tableDepth) activeTable = {start: blocks.length, rows: 0, nested: false};
      else activeTable.nested = true;
      tableDepth++;
    }
    if (t.tagName === 'tr' && activeTable) activeTable.rows++;
  ...
  parser.on("endTag", (t) => {
    ...
    if (!hidden) {
      ...
      if (t.tagName === 'table' && tableDepth && --tableDepth === 0) {
        tables.push({...activeTable, end: blocks.length - 1, complete: !activeTable.nested});
        activeTable = null;
      }
    }
  ...
  parser.end(html);
  flush();
  if (activeTable) tables.push({...activeTable, end: blocks.length - 1, complete: false});
  return {version: 'document-structure-v1', blocks, tables};
}
export function makeChunks(blocks, docId) {
  if (!Array.isArray(blocks)) return structuredChunks(blocks, docId);
  const chunks = [];
  let text = "",
    start = 0;
...
function structuredChunks(document, docId) {
  const {blocks, tables} = document;
  const tableByStart = new Map(tables.filter(t => t.end >= t.start).map(t => [t.start, t]));
  const chunks = [];
  let start = 0, parts = [], chars = 0, lastTableEnd = -1, documentSection = null;
  const flush = end => {
    if (parts.length) chunks.push({id: `${docId}:${start}`, text: parts.join('\n'),
      kind: 'prose', blockStart: start, blockEnd: end, documentSection});
    parts = []; chars = 0;
  };
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (table) {
      flush(i - 1);
      // Title/units immediately before a physical table remain attached, with
      // their original block locations. Never borrow context across another table.
      let contextStart = i;
      for (let j = i - 1; j >= Math.max(lastTableEnd + 1, i - 3); j--) {
        if (blocks[j].length > 220) break;
        contextStart = j;
      }
      chunks.push({id: `${docId}:T${i}`, kind: 'table',
        text: blocks.slice(contextStart, table.end + 1).join('\n'),
        blockStart: i, blockEnd: table.end, contextBlockStart: contextStart,
        documentSection, table: {complete: table.complete, rows: table.rows, nested: table.nested}});
      lastTableEnd = table.end; i = table.end; start = i + 1;
      continue;
    }
    const item = blocks[i].match(/^item\s+(\d+[a-z]?)[.:]?\s+[^|]{2,160}$/i);
    if (item) { flush(i - 1); documentSection = item[1].toLowerCase(); start = i; }
    if (parts.length && chars + blocks[i].length + 1 > 2200) { flush(i - 1); start = i; }
    if (!parts.length) start = i;
    parts.push(blocks[i]); chars += blocks[i].length + 1;
  }
...
export function retrieve(chunks, topic) {
  ...
  const scored = chunks
    ...
    .filter((x) => x.score > 0 && (x.kind !== 'table' || x.table?.complete === true) &&
      (x.kind !== 'prose' || x.text.length >= 100))
