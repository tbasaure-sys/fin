...
export function selectFilings(recent, asOf = new Date().toISOString()) {
  ...
  const annual = rows.filter((x) => ["10-K", "20-F", "40-F"].includes(x.form)).at(-1);
  if (!annual) throw Error("NO_RECENT_ANNUAL_FILING");
  const sinceAnnual = rows.filter(x => Date.parse(x.acceptedAt) >= Date.parse(annual.acceptedAt));
  // Foreign issuer 6-Ks may be frequent and their primary document may only
  // reference exhibits. Keep a bounded selection; never describe it as complete.
  const chosen = annual.form === "10-K"
    ? sinceAnnual.filter(x => x.form !== "6-K")
    : [...sinceAnnual.filter(x => x.form !== "6-K"), ...sinceAnnual.filter(x => x.form === "6-K").slice(-4)]
