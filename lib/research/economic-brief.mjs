...
export function operatingSensitivity(operating,retainedMarginChange=0){
 if(operating?.status!=='available'||!finite(retainedMarginChange)||retainedMarginChange<0||retainedMarginChange>1)return null;
 const changeVsObserved=-(1-retainedMarginChange)*operating.marginEffect;
 const operatingIncome=operating.current+changeVsObserved;
 return {retainedMarginChange,operatingIncome,margin:operatingIncome/operating.currentRevenue,changeVsObserved,
  period:{start:operating.start,end:operating.end},forecast:false,assumption:'current_reported_revenue_unchanged'};
}
export function economicBrief({dossier,reading}){
 ...
 const ebit={prior:fact('ebit','prior'),current:fact('ebit','current')};
 let operating=absent('INCOMPATIBLE_REVENUE_AND_OPERATING_INCOME');
 if(validPair(revenue)&&validPair(ebit)&&revenue.prior.value>0&&revenue.current.value>0
  &&sameBasis(revenue.prior,ebit.prior)&&sameBasis(revenue.current,ebit.current)){
  const priorMargin=ebit.prior.value/revenue.prior.value,currentMargin=ebit.current.value/revenue.current.value;
  // Ordered identity: change revenue first at the prior margin, then apply the
  // margin change to current revenue. The cross-term belongs to margin here;
  // this is not a unique causal attribution, volume estimate or normalization.
  const revenueEffect=(revenue.current.value-revenue.prior.value)*priorMargin;
  const marginEffect=revenue.current.value*(currentMargin-priorMargin),change=ebit.current.value-ebit.prior.value;
  const reconciliationGap=change-revenueEffect-marginEffect;
  if([priorMargin,currentMargin,revenueEffect,marginEffect,change,reconciliationGap].every(finite)
   &&Math.abs(reconciliationGap)<=1e-10*Math.max(1,Math.abs(ebit.prior.value),Math.abs(ebit.current.value)))
   operating={status:'available',basis,start:ebit.current.start,end:ebit.current.end,priorStart:ebit.prior.start,priorEnd:ebit.prior.end,
    prior:ebit.prior.value,current:ebit.current.value,priorRevenue:revenue.prior.value,currentRevenue:revenue.current.value,
    priorMargin,currentMargin,revenueEffect,marginEffect,change,reconciliationGap,
    method:'revenue_first_at_prior_margin_then_margin_at_current_revenue',economicCauseVerified:false,normalized:false,evidence:{revenue,ebit}};
 }
 ...
 return {version:'economic-brief-v1',ticker:dossier?.ticker,asOf:dossier?.asOf,
  status:[growth,operating,cash,capital].some(x=>x.status==='available')?'available':'unresolved',growth,operating,cash,capital,growthCashComparable,
