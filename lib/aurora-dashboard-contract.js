...
function calibrationAdoptionGate(input = {}) {
  const gate = input.calibrationIntegration?.calibrationAdoptionGate || null;
  if (!gate) {
    return {
      available: false,
      status: "missing",
      decisionUse: "not_available",
      primaryBranch: "raw",
      canPromote: false,
      canStage: false,
      mustUseRawPrimary: true,
      mustAbstain: false,
      calibratedWeight: 0,
      rawWeight: 1,
      shadowWeight: 0,
      ...
      warnings: [],
      memo: {
        headline: "Calibration adoption gate is not connected.",
        nextStep: "Build a calibration integration packet.",
      },
    };
  }
  return {
    available: true,
    version: gate.version || "aurora_calibration_adoption_gate_v1",
    status: gate.status || "unknown",
    decisionUse: gate.decisionUse || "unknown",
    primaryBranch: gate.adoption?.primaryBranch || "raw",
    canPromote: Boolean(gate.canPromote),
    canStage: Boolean(gate.canStage),
    mustUseRawPrimary: gate.mustUseRawPrimary !== false,
    mustAbstain: Boolean(gate.mustAbstain),
    calibratedWeight: numeric(gate.adoption?.calibratedWeight, 0),
    rawWeight: numeric(gate.adoption?.rawWeight, 1),
    shadowWeight: numeric(gate.adoption?.shadowWeight, 0),
    maxPositionSizeMultiplier: numeric(gate.adoption?.maxPositionSizeMultiplier, 0),
    authorityScore: numeric(gate.evidence?.authorityScore, null),
    evidenceTier: gate.evidence?.evidenceTier || "unknown",
    activeSegment: gate.evidence?.activeSegment || null,
    contextualApplied: Boolean(gate.evidence?.contextualApplied),
    checklist: arrayOrEmpty(gate.checklist),
    blockers: arrayOrEmpty(gate.blockers),
    warnings: arrayOrEmpty(gate.warnings),
    memo: gate.memo || {},
  };
}

function sectorTwinPanel(input = {}) {
  const twin = input.semiconductorTwin || null;
  if (!twin) {
    return {
      available: false,
      type: "missing",
      state: "missing",
      decision: "sector_twin_missing",
      scores: {},
      headline: "Sector twin is not connected.",
    };
  }
  return {
    available: Boolean(twin.applicable),
    type: "semiconductor",
    state: twin.state || "unknown",
    decision: twin.decision || "unknown",
    scores: twin.scores || {},
    adjustments: twin.adjustments || {},
    falsifiers: twin.falsifiers || [],
    headline: twin.dashboard?.headline || twin.memo?.headline || null,
  };
}

function competitiveMoatPanel(input = {}) {
  const moat = input.competitiveMoat || null;
  if (!moat) {
    return {
      available: false,
      decision: "competitive_moat_missing",
      topCompetitor: null,
      moatHalfLife: null,
      headline: "Competitive moat engine is not connected.",
    };
  }
  return {
    available: moat.decision !== "competitive_graph_pending",
    decision: moat.decision || "unknown",
    competitorCount: numeric(moat.aggregate?.competitorCount, 0),
    maxThreat: numeric(moat.aggregate?.maxThreat, null),
    averageThreat: numeric(moat.aggregate?.averageThreat, null),
    topCompetitor: moat.dashboard?.topCompetitor || null,
    moatHalfLife: moat.dashboard?.moatHalfLife || null,
    falsifiers: moat.falsifiers || [],
    headline: moat.dashboard?.headline || moat.memo?.headline || null,
  };
}
...
function visualizationContract(input = {}) {
  ...
  const hasCalibrationAdoptionGate = Boolean(input.calibrationIntegration?.calibrationAdoptionGate);
  const semiconductorTwin = input.semiconductorTwin || null;
  const competitiveMoat = input.competitiveMoat || null;
  ...
  return [
    ...
    visualization(
      "competitor_graph",
      "Competitor graph",
      competitiveMoat?.graph?.nodes?.length ? "ready" : competitiveMoat ? "partial" : "missing",
      competitiveMoat?.graph?.nodes?.length
        ? "Competitor nodes, threat edges, and moat half-life adjustment are available."
        : competitiveMoat
          ? "Competitive moat engine is present but peer inputs are missing."
          : "Competitive moat engine is missing.",
      "competitiveMoat.graph",
    ),
    visualization(
    ...
    visualization(
      ...
      "Calibration adoption gate",
      hasCalibrationAdoptionGate ? "ready" : "missing",
      hasCalibrationAdoptionGate ? "Calibration branch, adoption weight, and promotion checklist are available." : "Calibration adoption gate is missing.",
      "calibrationIntegration.calibrationAdoptionGate",
    ),
    visualization(
      "sector_twin_semiconductor",
      "Semiconductor sector twin",
      semiconductorTwin?.applicable ? "ready" : semiconductorTwin ? "not_applicable" : "missing",
      semiconductorTwin?.applicable
        ? "Semiconductor capacity, inventory, ASP, capex cycle, and bottleneck durability are available."
        : semiconductorTwin
          ? "Company is outside the semiconductor kernel."
          : "Semiconductor sector twin is missing.",
      "semiconductorTwin.dashboard",
    ),
    visualization(
      "irr_distribution",
...
function primaryPanel(input = {}) {
  ...
  const adoptionGate = calibrationAdoptionGate(input);
  const sectorTwin = sectorTwinPanel(input);
  const competitiveMoat = competitiveMoatPanel(input);
  ...
  return {
    ...
    calibrationAdoptionGate: adoptionGate,
    sectorTwin,
    competitiveMoat,
  };
}
...
function warnings(input = {}, panel = {}) {
  return [
    ...
    input.calibrationIntegration?.calibrationAdoptionGate?.mustAbstain ? "Calibration adoption gate blocks decision use." : null,
    panel.calibrationAuthority?.hardBlocks?.length
      ...
      : null,
    panel.calibrationAdoptionGate?.blockers?.length
      ? `Calibration adoption gate blockers: ${panel.calibrationAdoptionGate.blockers.join(", ")}.`
      : null,
    panel.sectorTwin?.decision === "semiconductor_glut_risk" ? "Semiconductor twin warns about capacity, inventory, or ASP cycle risk." : null,
    panel.competitiveMoat?.decision === "moat_fade_risk" ? "Competitive graph warns that moat half-life may be too long." : null,
...
function investorQuestions(input = {}, panel = {}) {
  ...
  return [
    ...
    panel.calibrationAdoptionGate?.available
      ? "Should calibrated output be primary, guardrailed, shadow, or blocked?"
      : "What calibration adoption gate is needed before UI integration?",
    panel.sectorTwin?.available
      ? "Is this semiconductor thesis a durable bottleneck or a capacity cycle?"
      : "Which sector kernel should be connected for this business model?",
    panel.competitiveMoat?.available
      ? "Which competitor is most likely to shorten the moat half-life?"
      : "Which peer set is needed to build the competitor graph?",
