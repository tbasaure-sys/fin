...
function valueRange(input = {}) {
  ...
  return {
    source: calibrated ? "calibrated_valuation_ensemble" : ensemble ? "valuation_ensemble" : "forecast_scenarios",
    p10: firstFinite(ensemble?.p10, quantile(forecastValues, 0.1)),
    p50: firstFinite(
      ensemble?.p50,
    ...
    p90: firstFinite(ensemble?.p90, quantile(forecastValues, 0.9)),
  };
}
...
function visualizationContract(input = {}) {
  ...
  return [
    ...
    visualization(
      ...
      dominantDrivers(input).length ? "proxy" : "missing",
...
function primaryPanel(input = {}) {
  ...
  return {
    valueRange: range,
    marketPrice: price,
    probabilityValueBelowPrice: probabilityValueBelowPrice(input, range),
    expectedIrr5y: irr.p50,
    irrDistribution: irr,
    probabilityNegativeIrr: probabilityValueBelowPrice(input, range),
    moatHalfLife: moat,
...
export function buildAuroraDashboardContract(input = {}, options = {}) {
  ...
  return {
    ...
    visualizations,
    readiness: ready,
    sensitivityProxy: panel.dominantDrivers,
    warnings: warn,
