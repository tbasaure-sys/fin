...
export function parsePortfolioUpdatePayload(body) {
  ...
  if (payload.confirmHoldings === true) {
    return { confirmHoldings: true };
  }
  ...
  const avgCostUsd = readOptionalNumber(payload.avgCostUsd ?? payload.avg_cost_usd, "avgCostUsd", { min: 0.00000001, max: 1_000_000_000 });
  const purchaseDate = readString(payload.purchaseDate ?? payload.purchase_date, "purchaseDate", { max: 10 });
  if (purchaseDate && !/^\d{4}-\d{2}-\d{2}$/.test(purchaseDate)) {
    throw new RequestValidationError("purchaseDate must use YYYY-MM-DD.");
  }
