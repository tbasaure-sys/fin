...
function readMoneyAmount(value, label) {
  return readOptionalNumber(value, label, { min: 0, max: 1_000_000_000 }) ?? 0;
}
...
export function parsePortfolioUpdatePayload(body) {
  ...
  if (payload.replacePortfolio === true || payload.replace_portfolio === true) {
    if (!Array.isArray(payload.holdings) || payload.holdings.length < 1) {
      throw new RequestValidationError("A confirmed portfolio replacement requires at least one holding.");
    }
    if (payload.holdings.length > 100) {
      throw new RequestValidationError("A portfolio replacement supports at most 100 holdings.");
    }
    const holdings = payload.holdings.map((raw, index) => {
      const holding = requireObject(raw, `holdings[${index}]`);
      const ticker = readString(holding.ticker || holding.symbol, `holdings[${index}].ticker`, { required: true, max: 16 })
        .toUpperCase()
        .replace(/[^A-Z0-9.\-]/g, "");
      const quantity = readOptionalNumber(holding.quantity ?? holding.shares, `holdings[${index}].quantity`, { min: 0.00000001, max: 1_000_000_000 });
      if (!ticker || quantity === undefined) {
        throw new RequestValidationError(`holdings[${index}] requires ticker and a positive quantity.`);
      }
      const avgCostUsd = readOptionalNumber(holding.avgCostUsd ?? holding.avg_cost_usd, `holdings[${index}].avgCostUsd`, { min: 0, max: 1_000_000_000 });
      const currentPriceUsd = readOptionalNumber(holding.currentPriceUsd ?? holding.current_price_usd, `holdings[${index}].currentPriceUsd`, { min: 0, max: 1_000_000_000 });
      const marketValueUsd = readOptionalNumber(holding.marketValueUsd ?? holding.market_value_usd, `holdings[${index}].marketValueUsd`, { min: 0, max: 1_000_000_000_000 });
      const purchaseDate = readString(holding.purchaseDate ?? holding.purchase_date, `holdings[${index}].purchaseDate`, { max: 10 });
      if (purchaseDate && !/^\d{4}-\d{2}-\d{2}$/.test(purchaseDate)) {
        throw new RequestValidationError(`holdings[${index}].purchaseDate must use YYYY-MM-DD.`);
      }
      const sector = readString(holding.sector, `holdings[${index}].sector`, { max: 80 });
      const assetType = readString(holding.assetType ?? holding.asset_type, `holdings[${index}].assetType`, { max: 40 });
      return {
        ticker,
        quantity,
        ...(avgCostUsd === undefined ? {} : { avgCostUsd }),
        ...(currentPriceUsd === undefined ? {} : { currentPriceUsd }),
        ...(marketValueUsd === undefined ? {} : { marketValueUsd }),
        ...(purchaseDate ? { purchaseDate } : {}),
        ...(sector ? { sector } : {}),
        ...(assetType ? { assetType } : {}),
      };
    });
    return { replacePortfolio: true, holdings };
  }
  const rawCashEvent = payload.cashEvent || payload.cash_event;
  if (rawCashEvent !== undefined) {
    const cashEvent = requireObject(rawCashEvent, "cashEvent");
    const eventType = readString(cashEvent.eventType || cashEvent.event_type || cashEvent.type || cashEvent.action, "cashEvent.eventType", { required: true, max: 32 })
      .toLowerCase()
      .replace(/[^a-z_]/g, "");
    const amountUsd = readOptionalNumber(cashEvent.amountUsd ?? cashEvent.amount_usd ?? cashEvent.amount, "cashEvent.amountUsd", { min: 0.01, max: 1_000_000_000 });
    if (amountUsd === undefined) {
      throw new RequestValidationError("cashEvent.amountUsd is required.");
    }
    return {
      cashEvent: {
        eventType,
        amountUsd,
        note: readString(cashEvent.note || cashEvent.memo, "cashEvent.note", { max: 400 }),
      },
    };
  }

  const instruction = readString(payload.instruction || payload.text, "instruction", { max: 400 });
  const price = readOptionalNumber(payload.price ?? payload.currentPrice ?? payload.explicitPrice, "price", { min: 0 });
  const tradeDate = readString(payload.tradeDate ?? payload.trade_date, "tradeDate", { max: 10 });
  if (tradeDate && !/^\d{4}-\d{2}-\d{2}$/.test(tradeDate)) {
    throw new RequestValidationError("tradeDate must use YYYY-MM-DD.");
  }
  const preview = payload.preview === true;
  const ticker = readString(payload.ticker || payload.symbol, "ticker", { max: 16 })
    .toUpperCase()
    .replace(/[^A-Z0-9.\-]/g, "");
  const quantity = readOptionalNumber(payload.quantity ?? payload.target_quantity ?? payload.targetQuantity, "quantity", { min: 0 });
  const targetValueUsd = readOptionalNumber(
    payload.target_value_usd ??
      payload.targetValueUsd ??
      payload.market_value_usd ??
      payload.marketValueUsd ??
      payload.value_usd ??
      payload.valueUsd,
    "targetValueUsd",
    { min: 0 },
  ...
  if (ticker && (quantity !== undefined || targetValueUsd !== undefined)) {
    return {
      ticker,
      ...(quantity !== undefined ? { quantity } : {}),
      ...(targetValueUsd !== undefined ? { targetValueUsd } : {}),
  ...
  if (!instruction) {
    throw new RequestValidationError("Instruction is required unless ticker with quantity or target value is provided.");
  }
...
export function parseFinancePlanPayload(body) {
  const payload = requireObject(body, "Payload");
  const baseCurrency = readString(payload.baseCurrency || payload.base_currency || "USD", "baseCurrency", { max: 8 })
    .toUpperCase()
    .replace(/[^A-Z]/g, "")
    .slice(0, 3) || "USD";

  return {
    monthlyIncome: readMoneyAmount(payload.monthlyIncome ?? payload.monthly_income, "monthlyIncome"),
    fixedExpenses: readMoneyAmount(payload.fixedExpenses ?? payload.fixed_expenses, "fixedExpenses"),
    variableExpenses: readMoneyAmount(payload.variableExpenses ?? payload.variable_expenses, "variableExpenses"),
    safetyBuffer: readMoneyAmount(payload.safetyBuffer ?? payload.safety_buffer, "safetyBuffer"),
    targetMonthlyInvestment: readMoneyAmount(
      payload.targetMonthlyInvestment ?? payload.target_monthly_investment,
...
export function parsePhantomDiversificationPayload(body) {
  ...
  if (payload.holdings.length > 60) {
    throw new RequestValidationError("Portfolio intelligence analyzes up to 60 positive holdings per run.");
  }
...
export function parseDecisionPayload(body) {
  ...
  const asOfDate = readString(payload.asOfDate ?? payload.as_of_date, "asOfDate", { max: 10 });
  if (asOfDate && !/^\d{4}-\d{2}-\d{2}$/.test(asOfDate)) {
    throw new RequestValidationError("asOfDate must use YYYY-MM-DD.");
  }
  ...
  return {
    ...
    signalRunIds: readOptionalStringArray(payload.signalRunIds ?? payload.signal_run_ids, "signalRunIds", { maxItems: 12, maxLength: 64 }) || [],
    subjectType: readString(payload.subjectType ?? payload.subject_type, "subjectType", { max: 32 }) || undefined,
    subjectKey: readString(payload.subjectKey ?? payload.subject_key, "subjectKey", { max: 80 }) || undefined,
