...
import { resolveAppUrl } from "../request-origin.js";
...
function buildPlanSnapshot({
  ...
  return {
    ...
    access: {
      ...
      paywallHeadline: privateWorkspace ? null : "Private workspace access is not enabled for this account yet",
      paywallMessage: privateWorkspace
        ? null
        : "BLS Prime is still in early access. Ask the product owner to enable this workspace before relying on private holdings or live updates.",
    },
  };
}
...
export async function createCheckoutLink({ user, plan, request = null }) {
  ...
  const appUrl = resolveAppUrl(request, getPublicAppUrl());

  if (!polarToken || !polarProductId) {
    const url = new URL(fallbackBaseUrl, appUrl);
...
export async function createBillingPortalLink({ userId, request = null }) {
