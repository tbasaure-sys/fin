...
import {namedQuarterScope} from '../research/source-scope-check.mjs';

export const MODEL = 'openai/gpt-oss-120b';
export const VERSION = 'filing-analysis-groq-v11.3';
const REVIEW_MODEL = 'openai/gpt-oss-120b';
const REVIEW_MODELS = new Set([REVIEW_MODEL,'qwen/qwen3.8-27b']);
...
// Mask document/calendar locators before masking the form itself. This only
// classifies their syntax; review must still check the actual reporting period.
const reportDate=/\b(?:(?:informe (?:anual|trimestral)|(?:annual|quarterly) report|10[-\u2010-\u2013][KQ](?:\/A)?)\s+(?:Q[1-4]\s+)?(?:19|20)\d{2}|Q[1-4]\s+(?:19|20)\d{2}|(?:19|20)\d{2}\s+Q[1-4])\b(?!\s*(?:[%$€]|[.,]\d|mil\w*|bill\w*|trill\w*|bn\b|USD\b|EUR\b|d[oó]lares|dollars|euros|percent|por ciento))/giu;
const spacedDate=/\b(?:0?[1-9]|[12]\d|3[01])\s+(?:ene|jan|feb|mar|abr|apr|may|jun|jul|ago|aug|sep|oct|nov|dic|dec)\s+(?:19|20)\d{2}\b(?!\s*(?:[%$€]|[.,]\d|mil\w*|bill\w*|trill\w*|bn\b|USD\b|EUR\b|d[oó]lares|dollars|euros|percent|por ciento))/giu;
const monthYear=/\b(?:enero|january|febrero|february|marzo|march|abril|april|mayo|may|junio|june|julio|july|agosto|august|septiembre|september|octubre|october|noviembre|november|diciembre|december)\s+(?:de\s+)?(?:19|20)\d{2}\b(?!\s*(?:[%$€]|[.,]\d|mil\w*|bill\w*|trill\w*|bn\b|USD\b|EUR\b|d[oó]lares|dollars|euros|percent|por ciento))/giu;
const comparativeYear=/\b(?:frente a|compared (?:with|to)|versus|vs\.?)\s+(?:19|20)\d{2}\b(?!\s*(?:[%$€]|[.,]\d|mil\w*|bill\w*|trill\w*|bn\b|USD\b|EUR\b|d[oó]lares|dollars|euros|percent|por ciento))/giu;
const hasQuantity=s=>/[0-9]/.test(s.replace(reportDate,'').replace(spacedDate,'').replace(monthYear,'').replace(comparativeYear,'').replace(/\b(?:Microsoft|Office)\s*365\b/g,'').replace(/\b(?:10[-\u2010-\u2013][KQ]|8[-\u2010-\u2013]K)(?:\/A)?\b/gi,'').replace(/\b(?:note|nota)\s+\d{1,3}\b/gi,'').replace(/\bitem\s+(?:[1-9]|1[0-6])[A-C]?\b/gi,'').replace(calendarYear,'').replace(calendarDate,''))||writtenQuantity.test(s);
...
async function completion({apiKey,fetcher,messages,responseSchema,model,signal,maxTokens,diagnose}) {
  let response;
  const alternateReviewer=model==='qwen/qwen3.8-27b';
  try {
    response=await fetcher('https://api.groq.com/openai/v1/chat/completions',{
      ...
      body:JSON.stringify({model,messages,temperature:0.2,reasoning_effort:alternateReviewer?'high':'low',
        ...(alternateReviewer?{reasoning_format:'hidden'}:{}),max_completion_tokens:maxTokens,
        response_format:{type:'json_schema',json_schema:{name:'filing_analysis',strict:!alternateReviewer,schema:responseSchema}}}),
    });
...
export function applyReferenceAnalysisReview(analysis,raw,dossier,{model=REVIEW_MODEL}={}){
  if(!REVIEW_MODELS.has(model))throw Error('UNSUPPORTED_REVIEW_MODEL');
  let resolved;
  ...
  return finalizeReview(analysis,{...raw,agenda},dossier,reviews,{model,protocol:'reference-review-v1'});
}
function finalizeReview(analysis,raw,dossier,reviews,metadata){
  for(const section of analysis.sections)for(const [i,finding] of section.findings.entries()){
    const id=`${section.id}:${i}`,review=reviews.get(id);
    const scopeCheck=namedQuarterScope(finding.text,finding.evidence.map(e=>e.quote));
    reviews.set(id,{...review,scopeCheck,reviewVerdictBeforeScopeCheck:review.verdict,
      ...(scopeCheck.status==='conflict'?{verdict:'unsupported',
        reason:`El trimestre nombrado en la afirmación (${scopeCheck.claimedQuarters.join(', ')}) no coincide con los trimestres explícitos de sus citas (${scopeCheck.sourceQuarters.join(', ')}). Se necesita evidencia del período afirmado.`}:{})});
  }
...
export async function generateAnalysis(dossier, { apiKey=process.env.GROQ_API_KEY, language='es', reviewModel=REVIEW_MODEL, resume=null, onCheckpoint=null, fetcher=fetch, wait=waitForProvider, diagnose=event=>console.warn('[filing-analysis]',JSON.stringify(event)) }={}) {
  if(!apiKey?.trim()) throw Error('PROVIDER_UNAVAILABLE');
  if(!REVIEW_MODELS.has(reviewModel))throw Error('UNSUPPORTED_REVIEW_MODEL');
  const stages=['draft','review','reconstruction','reconstruction_review'];
  const progress=resume?structuredClone(resume):{version:'analysis-progress-v1',engineVersion:VERSION,dossierHash:hash(dossier),language,reviewModel,
    completed:{},work:{},recovery:[],attempts:[],providerCalls:0,capacityAdjustmentUsed:false,outputCeiling:null};
  if(progress.version!=='analysis-progress-v1'||progress.engineVersion!==VERSION||progress.dossierHash!==hash(dossier)||progress.language!==language||progress.reviewModel!==reviewModel
  ...
  // Keep source text and temporal scope intact. URLs, byte offsets and digests
  // belong to server-owned citation resolution, not the model's reading budget.
  const context={ticker:dossier.ticker,name:dossier.name,asOf:dossier.asOf,
    sources:dossier.sources.map(({id,form,acceptedAt,periodEnd})=>({id,form,acceptedAt,periodEnd})),
    sections:dossier.sections.map(s=>({id:s.id,question:{business:'¿Cómo opera la empresa y qué límites describe? Da actividades y riesgos concretos con atribución.',cash:'¿Qué explica los movimientos de caja y qué obligaciones describe? Distingue niveles de cambios; deja los cálculos al motor financiero.',thesis:'¿Qué riesgos condicionales documenta la empresa, y qué pregunta permitiría contrastarlos? No hay precio ni consenso en este paquete.'}[s.id],
      extracts:s.extracts.map(({id,text,kind,documentSection})=>({id,text,kind,documentSection}))}))};
...
AGENDA: relevant means economically pertinent, grounded premises, and an unanswered question or a feasible distinguishing task. sourceIds name extracts motivating the question, NOT containing its answer. Debt disclosure can motivate a maturity question even without a schedule. Reject a task that assumes a public KPI/table exists without evidence; asking whether it exists is allowed. Reject questions already answered, irrelevant product trivia and unsupported premises. Missing answer alone is NOT a reason to reject. Use uncertain if relevance cannot be assessed.`},
  ...
  const second=await runStage('review',{messages:reviewMessagesFor(draft),responseSchema:reviewSchema,model:reviewModel,maxTokens:3200},raw=>applyReferenceAnalysisReview(draft,raw,dossier,{model:reviewModel}));
  ...
  if(targets.length){
   ...
   if(onCheckpoint||deadline-Date.now()>12000){
    try{
     ...
     const checked=await runStage('reconstruction_review',{messages:reviewMessagesFor(redo.value),responseSchema:reviewSchema,model:reviewModel,maxTokens:3200},raw=>applyReferenceAnalysisReview(redo.value,raw,dossier,{model:reviewModel}));
     let added=0;
  ...
  return {...analysis,provider:'groq',model:MODEL,language:language==='en'?'en':'es',generatedAt:new Date().toISOString(),
    ...
    reviewPromptHash:hash({reviewMessages:second.messages,reviewSchema,model:reviewModel}),reviewResponseHash:second.responseHash,
