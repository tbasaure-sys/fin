...
import { assessPortfolioFreshness } from "../portfolio/intelligence.js";
import { recordedPerformanceSeries } from "../portfolio/recorded-performance.mjs";
...
function normalizePortfolioGenerationStartedAt(value) {
  const millis = parseUpdatedAt(value);
...
async function ensurePortfolioGenerationColumn(sql) {
  if (globalThis.__BLS_PORTFOLIO_GENERATION_READY__) return;
  await sql.query(`
    ALTER TABLE bls_portfolio_positions
    ADD COLUMN IF NOT EXISTS portfolio_generation_started_at TIMESTAMPTZ
  `);
  globalThis.__BLS_PORTFOLIO_GENERATION_READY__ = true;
}
...
export function attachHistoryExternalFlows(historyRows, flowRows) {
  const rows = [...(Array.isArray(historyRows) ? historyRows : [])].sort((left, right) => {
    const leftTime = dateMillis(left.captured_at || left.capture_bucket || left.date) ?? 0;
    const rightTime = dateMillis(right.captured_at || right.capture_bucket || right.date) ?? 0;
  ...
  return rows.map((row, index) => {
    const currentTime = dateMillis(row.captured_at || row.capture_bucket || row.date);
    const previousTime = index > 0 ? dateMillis(rows[index - 1].captured_at || rows[index - 1].capture_bucket || rows[index - 1].date) : null;
    ...
    return {
      ...
      external_flows: intervalFlows.map(flow => ({amount:flow.flowUsd,at:new Date(flow.time).toISOString()})),
...
export function buildHistorySeries(rows) {
  return recordedPerformanceSeries(rows);
}
...
export function buildHistoryPerformanceMetrics(rows) {
  ...
  const incomplete = series.some(row => !['baseline','calculated','awaiting_capital'].includes(row.performance_status));
  const totalTwr = !incomplete && portfolioPoints.length >= 2 && first > 0 && last !== null ? (last / first) - 1 : null;
  const flowCount = series.reduce((sum, row) => sum + (Math.abs(parseNumber(row.external_flow_usd) || 0) > 0 ? 1 : 0), 0);
  const moneyWeightedReturn = incomplete ? null : moneyWeightedReturnFromHistory(rows);
  return {
    totalTwr,
    ...
    performanceMethod: incomplete ? 'incomplete_recorded_history' : series.some(row => row.performance_method === 'modified_dietz_approximation') ? 'modified_dietz_approximation' : 'twr_observed_boundaries',
    performanceStatus: incomplete ? 'unresolved' : 'calculated',
...
function moneyWeightedReturnFromHistory(rows) {
  const sorted = [...(Array.isArray(rows) ? rows : [])]
    .map((row) => ({
      ...
      flows: row.external_flows,
      timing: row.external_flow_timing,
    }))
  ...
  const cashflows = [
    ...
    ...sorted.slice(1).flatMap((row,i) => Array.isArray(row.flows)
      ? row.flows.map(flow=>({date:flow.at,amount:-flow.amount}))
      : row.flow !== 0 ? [{date:row.timing==='start'?sorted[i].date:row.date,amount:-row.flow}] : []),
    { date: last.date, amount: last.value },
  ];
  return solveXirr(cashflows);
}

async function getNeonPortfolioHistory(workspaceId, limit = 320, portfolioGenerationStartedAt = null) {
  const sql = await ensureNeonWorkspace(workspaceId);
  await ensurePortfolioCashLedger(sql);
  const generationStartedAt = normalizePortfolioGenerationStartedAt(portfolioGenerationStartedAt);
  const [historyRows, tradeRows, cashLedgerRows] = await Promise.all([
    sql.query(
      `SELECT capture_bucket, captured_at, total_value_usd, benchmark_symbol, benchmark_price_usd, metadata
       ...
       LIMIT $2`,
      [workspaceId, limit, generationStartedAt],
    ),
    sql.query(
      `SELECT side, quantity, price_usd, created_at
       ...
       LIMIT 2000`,
      [workspaceId, generationStartedAt],
    ),
    sql.query(
  ...
  // Trades are internal exchanges, never evidence of deposits. Legacy trade-only
  // histories do not identify how the portfolio was funded.
  return attachHistoryExternalFlows(mappedHistory, cashFlowRows).map(row => ({
    ...row, flow_coverage: !cashLedgerRows.length && tradeRows.length ? 'unknown' : 'recorded',
  }));
}

async function appendNeonPortfolioHistorySnapshot(
  workspaceId,
  snapshot,
  holdings,
  sourceLabel,
  portfolioGenerationStartedAt = null,
) {
  ...
  if (!(totalValueUsd > 0)) {
    return getNeonPortfolioHistory(workspaceId, 320, generationStartedAt);
  }

  const benchmarkSymbol = "SPY";
  const benchmarkPriceUsd = getCurrentPriceForTicker(snapshot, benchmarkSymbol);
  const generatedAtMillis = parseUpdatedAt(snapshot?.generated_at || snapshot?.portfolio?.quotes_as_of) ?? Date.now();
  const generatedAt = new Date(generatedAtMillis).toISOString();
  const generationStartedAtMillis = parseUpdatedAt(generationStartedAt);
  if (generationStartedAtMillis !== null && generatedAtMillis < generationStartedAtMillis) {
    return getNeonPortfolioHistory(workspaceId, 320, generationStartedAt);
  }
  ...
  await sql.query(
    ...
    [
      ...
      JSON.stringify({
        ...
        portfolio_generation_started_at: generationStartedAt,
      }),
    ],
  );
...
async function loadNeonHoldingsState(workspaceId) {
  ...
  await ensurePortfolioGenerationColumn(sql);
  const positions = await sql.query(
    `SELECT
      ...
      sector,
      industry,
      portfolio_generation_started_at,
      updated_at
  ...
  const portfolioGenerationStartedAt = positions
    .map((row) => normalizePortfolioGenerationStartedAt(row.portfolio_generation_started_at))
    .filter(Boolean)
    .sort()
    .at(-1) || null;
  const latestTrade = await sql.query(
    `SELECT ticker, side, quantity, price_usd, source, created_at
     ...
     LIMIT 1`,
    [workspaceId, portfolioGenerationStartedAt],
  );
  ...
  return {
    ...
    portfolio_generation_started_at: portfolioGenerationStartedAt,
...
async function saveHoldingsState(workspaceId, payload) {
  if (usingNeonStorage()) {
    ...
    const portfolioGenerationStartedAt = normalizePortfolioGenerationStartedAt(
      payload?.portfolio_generation_started_at,
    );
    const statements = [
...
function normalizeHoldingsForOutput(holdings) {
  return [...holdings]
    .map((row) => ({
      ...
      value_source: row.value_source || row.valueSource || null,
      sector: row.sector || "Unknown",
      industry: row.industry || "Unknown",
    }))
    .filter((row) => row.ticker)
...
function parseTradeInstruction(text) {
  ...
  const tickerCandidates = [...raw.matchAll(/\b([A-Za-z]{1,6}(?:\.[A-Za-z]{1,2})?)\b/g)]
    ...
    .filter((token) => !["I", "A", "AN", "AND", "THE", "OF", "IN", "FOR", "TO", "DE", "EN", "Y", "USD", "BUY", "BOUGHT", "BUYING", "SELL", "SOLD", "SELLING", "TRIM", "REDUCE", "CLOSE", "COMPRE", "COMPRAR", "COMPRÉ", "VENDI", "VENDER", "VENDÍ", "ACCION", "ACCIONES", "SHARES", "SHARE", "STOCK"].includes(token));
  const usefulTickerCandidates = tickerCandidates.filter((token) => !["COMPR", "VEND"].includes(token));
  const explicitTicker = explicitTickerMatch?.[1]?.toUpperCase() || null;
  const ticker = ((explicitTicker && explicitTicker !== "USD" ? explicitTicker : null) || usefulTickerCandidates.at(-1) || null)?.toUpperCase() || null;
  ...
  if (/\bcompr(?:e|ar|\u00e9)(?=\s|$)/i.test(lowered)) side = "buy";
  if (/\bvend(?:i|er|\u00ed)(?=\s|$)/i.test(lowered)) side = "sell";
  ...
  return {
    text: raw,
    ticker,
    tickerCandidates: [...new Set(usefulTickerCandidates)],
    side,
    shares,
    amountUsd,
    explicitPrice,
...
function assertUsableTradeInstruction(instruction) {
  if (!instruction) {
    throw new Error("No pudimos entender la operación. Prueba con ‘compré USD 200 de NVDA’ o ‘vendí 2 acciones de AAPL’.");
  }
  if (instruction.tickerCandidates?.length > 1) {
    throw new Error("Registra una compra o venta por vez. Por ejemplo: ‘vendí 2 acciones de ZVRA’.");
  }
  if (instruction.shares === null && instruction.amountUsd === null) {
    throw new Error(`Indica cuántas acciones o cuántos USD operaste en ${instruction.ticker}.`);
  }
  return instruction;
}

async function getHistoricalTradePrice(ticker, tradeDate) {
  const date = parsePurchaseDate(tradeDate);
  if (!date) return null;
  const from = new Date(`${date}T00:00:00Z`);
  from.setUTCDate(from.getUTCDate() - 10);
  const fromDate = from.toISOString().slice(0, 10);
  const series = await fetchDailyCloseHistory([ticker], { fromDate, toDate: date });
  const candidates = Object.entries(series?.[ticker] || {})
    .filter(([session, close]) => session <= date && parseNumber(close) !== null)
    .sort(([left], [right]) => right.localeCompare(left));
  if (!candidates.length) return null;
  const [priceDate, price] = candidates[0];
  return { price: parseNumber(price), priceDate };
}

export async function previewHoldingsInstruction(snapshot, input) {
  const instruction = assertUsableTradeInstruction(parseTradeInstruction(input?.instruction || input?.text || input));
  const tradeDate = parsePurchaseDate(input?.tradeDate ?? input?.trade_date);
  const explicitPrice = parseNumber(input?.price) ?? parseNumber(input?.currentPrice) ?? parseNumber(input?.explicitPrice) ?? instruction.explicitPrice;

  if (!tradeDate && explicitPrice === null) {
    return {
      status: "needs_date",
      ticker: instruction.ticker,
      side: instruction.side,
      instruction: instruction.text,
      message: "¿Cuándo hiciste esta operación? Usaremos el cierre disponible de esa fecha antes de cambiar tu cartera.",
    };
  }

  const historical = explicitPrice === null ? await getHistoricalTradePrice(instruction.ticker, tradeDate) : null;
  const price = explicitPrice ?? historical?.price;
  if (price === null || price === undefined) {
    throw new Error(`No encontramos un cierre para ${instruction.ticker} en torno al ${tradeDate}. Prueba otra fecha o agrega el precio manualmente.`);
  }

  const quantity = instruction.shares !== null ? instruction.shares : (instruction.amountUsd / price);
  return {
    status: "ready",
    ticker: instruction.ticker,
    side: instruction.side,
    ...
    price,
    priceDate: historical?.priceDate || null,
    priceSource: explicitPrice !== null ? "manual" : "historical_close",
    quantity,
    amountUsd: Math.abs(quantity * price),
  };
}
...
function buildOverlayPortfolioAnalytics(snapshot, holdings, sourceLabel) {
  const rows = Array.isArray(holdings) ? holdings : [];
  const existingAnalytics = (snapshot?.portfolio || {}).analytics || {};
  const totalValueUsd = rows.reduce((sum, row) => sum + (parseNumber(row.market_value_usd) || 0), 0);
  const coveredRows = rows.filter((row) => parseNumber(row.quantity) > 0 && parseNumber(row.avg_cost_usd) > 0 && parseNumber(row.market_value_usd) !== null);
  const totalCostUsd = coveredRows.reduce((sum, row) => {
    const quantity = parseNumber(row.quantity);
    const avgCost = parseNumber(row.avg_cost_usd);
  ...
  const coveredValueUsd = coveredRows.reduce((sum, row) => sum + parseNumber(row.market_value_usd), 0);
  const unrealizedReturn = totalCostUsd > 0 ? (coveredValueUsd / totalCostUsd) - 1 : null;
  const unrealizedPnlUsd = totalCostUsd > 0 ? coveredValueUsd - totalCostUsd : null;
  const existingTotalPnlUsd = parseNumber(
    existingAnalytics["Total P&L incl. realized/dividends"]
      ?? existingAnalytics.total_pnl_incl_realized_dividends_usd,
  );
  const existingTotalReturn = parseNumber(
    existingAnalytics["Total return incl. dividends"]
      ?? existingAnalytics.total_return_incl_dividends,
  );
  const realizedPnlUsd = parseNumber(existingAnalytics["Realized P&L"] ?? existingAnalytics.realized_pnl_usd);
  const dividendsUsd = parseNumber(existingAnalytics.Dividends ?? existingAnalytics.dividends_usd);
  const hasActualAccountingComponents = realizedPnlUsd !== null || dividendsUsd !== null;
  const actualTotalPnlUsd = existingTotalPnlUsd !== null
    ? existingTotalPnlUsd
    : unrealizedPnlUsd !== null && hasActualAccountingComponents
      ? unrealizedPnlUsd + (realizedPnlUsd || 0) + (dividendsUsd || 0)
      : null;
  const actualTotalReturn = existingTotalReturn !== null
    ? existingTotalReturn
    : actualTotalPnlUsd !== null && totalCostUsd > 0
      ? actualTotalPnlUsd / totalCostUsd
      : null;
  ...
  return {
    analytics: {
      ...
      "Active cost basis": totalCostUsd > 0 ? totalCostUsd : null,
      "Unrealized P&L": unrealizedPnlUsd,
      "Unrealized Return": unrealizedReturn,
      ...(actualTotalPnlUsd !== null ? { "Total P&L incl. realized/dividends": actualTotalPnlUsd } : {}),
      ...(actualTotalReturn !== null ? { "Total return incl. dividends": actualTotalReturn } : {}),
...
const TRUSTWORTHY_RECORDED_PERFORMANCE_METHODS = new Set([
  "broker_personal_history",
  "twr_external_flow_adjusted",
  "twr_no_recorded_flows",
  "time_weighted_external_flow_adjusted",
  "time_weighted_no_recorded_flows",
]);

const STALE_DERIVED_PERFORMANCE_KEYS = [
  "Unrealized P&L",
  "unrealized_pnl_usd",
  "Unrealized Return",
  "unrealized_return",
  "Total P&L incl. realized/dividends",
  "total_pnl_incl_realized_dividends_usd",
  "Total return incl. dividends",
  "total_return_incl_dividends",
  "Annual Return",
  "annual_return",
  "Annual Volatility",
  "annual_volatility",
...
function suppressStaleDerivedPerformance(overlayPortfolio, status, preserveRecordedHistory) {
  const analytics = { ...(overlayPortfolio?.analytics || {}) };
  for (const key of STALE_DERIVED_PERFORMANCE_KEYS) {
    delete analytics[key];
  }
  if (!preserveRecordedHistory) {
    delete analytics["Time Weighted Return"];
    delete analytics["Money Weighted Return"];
    delete analytics["Performance Method"];
    delete analytics["External Flow Count"];
  }
  analytics["Performance Status"] = status;
  analytics["Analytics Source"] = "stale_holdings_review_only";
...
function buildFreshnessBlockedPerformanceReport(freshness, preserveRecordedHistory) {
  const status = freshness.status === "stale" ? "stale_confirmation_required" : "confirmation_required";
  return {
    version: "holdings_performance_v1",
    status,
    availability: "insufficient_freshness",
    freshnessStatus: freshness.status,
    inputs: {
      status: "insufficient_freshness",
      actions: ["Confirma tus posiciones actuales para calcular el rendimiento de la cartera."],
    },
    current: null,
    personalHeadline: {
      available: false,
    ...
    reconstructedQuality: null,
    backcast: null,
    backcastStatus: status,
    backcastQuality: null,
    twr: null,
    twrAvailable: false,
    preservedRecordedHistory: Boolean(preserveRecordedHistory),
    explanation: ["Confirma tus posiciones actuales para actualizar precios y calcular el rendimiento."],
  };
}

function applyInstructionToHoldings(holdings, instruction, price, meta, tradeDate = null) {
  ...
  if (!hasExplicitSize) {
    throw new Error(`Indica cuántas acciones o cuántos USD operaste en ${ticker}.`);
  }
  const signed = instruction.side === "sell" ? -1 : 1;
  if (instruction.side === "sell" && !existing) {
    throw new Error(`No puedes vender ${ticker} porque no aparece en tus posiciones actuales.`);
  }
  ...
  if (existing && nextQuantity <= 0) {
    ...
    if (nextQuantity <= 0) {
      throw new Error(`La operación de ${ticker} debe dejar una posición positiva.`);
    }
    normalized.unshift({
      ticker,
      ...
      currency: "USD",
      avg_cost_usd: price,
      purchase_date: tradeDate || new Date().toISOString().slice(0, 10),
      current_price_usd: price,
...
function applyExplicitEditToHoldings(holdings, input, price, meta) {
  ...
  if (targetQuantity === null) {
    throw new Error("Indica una cantidad objetivo o un valor objetivo para la posición.");
  }
  ...
  if (existingIndex >= 0) {
    normalized[existingIndex] = {
      ...
      avg_cost_usd: editAvgCost ?? parseNumber(existing.avg_cost_usd),
...
function buildEmptyWorkspacePortfolioSnapshot(snapshot) {
  ...
  return {
    ...
    screener: {
      ...screener,
      rows: Array.isArray(screener.rows)
        ? screener.rows.map((row) => ({ ...row, is_current_holding: false }))
        : [],
    },
    portfolio: {
      ...portfolio,
      ...
      holdings_sync_status: "confirmation_required",
      holdings_sync_label: "Confirma una cartera para comenzar",
...
export async function applyLocalPortfolioOverlay(snapshot, workspaceId = "default") {
  ...
  const freshness = assessPortfolioFreshness(state?.updated_at);
  const portfolioGenerationStartedAt = normalizePortfolioGenerationStartedAt(
    state?.portfolio_generation_started_at,
  );
  const portfolioBaseSnapshot = portfolioGenerationStartedAt
    ? buildEmptyWorkspacePortfolioSnapshot(snapshot)
    : snapshot;
  ...
  if (!holdings || !holdings.length) return buildEmptyWorkspacePortfolioSnapshot(snapshot);

  // A current quote must never make an unconfirmed, old share count look like a
  // current portfolio. Stale positions remain visible for review, but are not
  // repriced or appended to performance history until the owner confirms them.
  const liveQuotes = freshness.canAnalyze
    ? await fetchFmpQuotes(holdings.map((h) => h.ticker).filter(Boolean))
    : {};
  const pricedHoldings = normalizeHoldingsForOutput(rebuildWeights(
    holdings.map((holding) => {
      ...
      return {
        ...
        value_source: "fmp_quote",
      };
    }),
  ));

  let historyRows = [];
  if (usingNeonStorage() && freshness.canAnalyze) {
    try {
      historyRows = await appendNeonPortfolioHistorySnapshot(
        workspaceId,
        snapshot,
        pricedHoldings,
        sourceLabel,
  ...
  let overlayPortfolio = buildOverlayPortfolioAnalytics(portfolioBaseSnapshot, pricedHoldings, sourceLabel);
  const existingGrowthComparison = Array.isArray(portfolioBaseSnapshot?.portfolio?.current_mix_vs_spy)
    ? portfolioBaseSnapshot.portfolio.current_mix_vs_spy
    : [];
  const existingGrowthMethods = existingGrowthComparison
    .map((row) => String(row?.performance_method || row?.performanceMethod || "").trim())
    .filter(Boolean);
  const hasExplicitNonBackcastRow = existingGrowthMethods.some((method) => method !== "current_weight_backcast");
  const existingPerformanceMethod = String(
    portfolioBaseSnapshot?.portfolio?.analytics?.["Performance Method"] || "",
  ).trim();
  const existingSeriesIsBackcast = !hasExplicitNonBackcastRow && (
    existingPerformanceMethod === "current_weight_backcast"
      || (existingGrowthMethods.length > 0 && existingGrowthMethods.every((method) => method === "current_weight_backcast"))
  );
  const allRowsDeclareTrustworthyHistory = existingGrowthComparison.length > 0
    && existingGrowthComparison.every((row) => TRUSTWORTHY_RECORDED_PERFORMANCE_METHODS.has(
      String(row?.performance_method || row?.performanceMethod || "").trim(),
    ));
  const analyticsDeclareTrustworthyHistory = existingGrowthComparison.length > 0
    && existingGrowthMethods.length === 0
    && TRUSTWORTHY_RECORDED_PERFORMANCE_METHODS.has(existingPerformanceMethod);
  const hasTrustworthyRecordedHistory = allRowsDeclareTrustworthyHistory || analyticsDeclareTrustworthyHistory;
  const existingPersonalGrowth = existingGrowthComparison.length && !existingSeriesIsBackcast
    && (freshness.canAnalyze || hasTrustworthyRecordedHistory)
      ? existingGrowthComparison
      : [];
  const hasExistingPersonalGrowth = historyRows.length > 0 || existingPersonalGrowth.length > 0;
  let growthComparison = historyRows.length
    ? buildHistorySeries(historyRows)
    : existingPersonalGrowth;
  ...
  const freshnessBlockedStatus = freshness.status === "stale"
    ? "stale_confirmation_required"
    : "confirmation_required";
  if (!freshness.canAnalyze) {
    overlayPortfolio = suppressStaleDerivedPerformance(
      overlayPortfolio,
      freshnessBlockedStatus,
      hasTrustworthyRecordedHistory,
    );
  }

  // Snapshot-free performance: always backcast the current book with real
  // closes. When purchase metadata exists, also retain the personal trajectory.
  let performanceReport = freshness.canAnalyze
    ? null
    : buildFreshnessBlockedPerformanceReport(freshness, hasTrustworthyRecordedHistory);
  if (freshness.canAnalyze) try {
    const inputAssessment = assessPerformanceInputs(pricedHoldings);
    const benchmarkSymbol = "SPY";
    const currentTickers = [...new Set(pricedHoldings.map((row) => String(row.ticker || "").toUpperCase()).filter(Boolean))];
    ...
    const earliestPurchase = datedHoldings
      ...
      .sort()[0];
    const historyStart = earliestPurchase && earliestPurchase < threeYearsAgo.toISOString().slice(0, 10)
      ? earliestPurchase
      : threeYearsAgo.toISOString().slice(0, 10);
    const priceHistory = await fetchDailyCloseHistory([...currentTickers, benchmarkSymbol], { fromDate: historyStart });
    ...
    performanceReport = buildPerformanceReport({
      holdings: pricedHoldings,
      actualPerformance: {
        totalPnlUsd: overlayPortfolio.analytics["Total P&L incl. realized/dividends"],
        totalReturn: overlayPortfolio.analytics["Total return incl. dividends"],
        costBasisUsd: overlayPortfolio.analytics["Active cost basis"],
      },
      reconstruction,
      backcast,
    ...
    // A personal reconstruction can fill a short snapshot history. The
    // current-weight backcast remains inside performance_report only: it must
    // never replace the owner's tracked or reconstructed growth series.
  ...
  return {
    ...portfolioBaseSnapshot,
    screener: {
      ...(portfolioBaseSnapshot?.screener || {}),
      rows: Array.isArray(portfolioBaseSnapshot?.screener?.rows)
        ? portfolioBaseSnapshot.screener.rows.map((row) => ({
          ...row,
          is_current_holding: currentTickers.has(String(row?.ticker || row?.symbol || "").toUpperCase()),
        }))
        : [],
    },
    portfolio: {
      ...(portfolioBaseSnapshot?.portfolio || {}),
      ...overlayPortfolio,
      current_mix_vs_spy: growthComparison,
      performance_report: performanceReport,
      holdings_source: source,
      holdings_source_label: sourceLabel,
      holdings_source_available: true,
      holdings_sync_status: !freshness.canAnalyze
        ? freshnessBlockedStatus
        : state?.sync_status || (source === "remote_overlay" ? "remote_synced" : "local_only"),
      holdings_sync_label: !freshness.canAnalyze
        ? freshness.status === "stale"
          ? "Confirma las posiciones para actualizar precios"
          : "Confirma las posiciones para calcular el rendimiento"
        : state?.sync_label || (source === "remote_overlay" ? "Remote + local saved" : "Saved locally"),
      holdings_updated_at: state?.updated_at || null,
      holdings: pricedHoldings,
...
async function replaceConfirmedHoldings(snapshot, workspaceId, input) {
  const requested = Array.isArray(input?.holdings) ? input.holdings : [];
  if (!requested.length) throw new Error("A confirmed portfolio requires at least one holding.");

  const tickers = [...new Set(requested.map((row) => String(row?.ticker || "").trim().toUpperCase()).filter(Boolean))];
  const liveQuotes = await fetchFmpQuotes(tickers);
  const aggregated = new Map();
  for (const raw of requested) {
    const ticker = String(raw?.ticker || "").trim().toUpperCase();
    const quantity = parseNumber(raw?.quantity);
    if (!ticker || quantity === null || quantity <= 0) continue;
    const suppliedPrice = parseNumber(raw?.currentPriceUsd ?? raw?.current_price_usd);
    const suppliedValue = parseNumber(raw?.marketValueUsd ?? raw?.market_value_usd);
    const livePrice = parseNumber(liveQuotes[ticker]);
    const price = livePrice ?? suppliedPrice ?? (suppliedValue !== null ? suppliedValue / quantity : null);
    if (price === null || !(price > 0)) {
      throw new Error(`No current price found for ${ticker}. Add a current price before confirming the portfolio.`);
    }
    const meta = getTickerMeta(snapshot, ticker);
    const avgCost = parseNumber(raw?.avgCostUsd ?? raw?.avg_cost_usd);
    const existing = aggregated.get(ticker);
    if (existing) {
      const combinedQuantity = existing.quantity + quantity;
      const combinedCost = existing.avg_cost_usd !== null && avgCost !== null
        ? ((existing.avg_cost_usd * existing.quantity) + (avgCost * quantity)) / combinedQuantity
        : existing.avg_cost_usd ?? avgCost;
      aggregated.set(ticker, {
        ...existing,
        quantity: combinedQuantity,
        avg_cost_usd: combinedCost,
        current_price_usd: price,
        market_value_usd: combinedQuantity * price,
      });
      continue;
    }
    aggregated.set(ticker, {
      ticker,
      asset_type: raw?.assetType || raw?.asset_type || meta.asset_type || "equity",
      quantity,
      currency: "USD",
      avg_cost_usd: avgCost,
      purchase_date: parsePurchaseDate(raw?.purchaseDate ?? raw?.purchase_date),
      current_price_usd: price,
      market_value_usd: quantity * price,
      weight: null,
      source_sheet: "Portfolio confirmation",
      sector: raw?.sector || meta.sector || "Unknown",
      industry: raw?.industry || meta.industry || "Unknown",
    });
  }

  const holdings = normalizeHoldingsForOutput(rebuildWeights([...aggregated.values()]));
  if (!holdings.length) throw new Error("A confirmed portfolio requires at least one positive holding.");
  const portfolioGenerationStartedAt = new Date().toISOString();
  const nextState = {
    available: true,
    source: usingNeonStorage() ? "neon_portfolio" : "ui_editable_overlay",
    source_label: "Cartera confirmada",
    updated_at: portfolioGenerationStartedAt,
    portfolio_generation_started_at: portfolioGenerationStartedAt,
    workspace_id: workspaceId,
    instruction: `Confirmed ${holdings.length} portfolio positions`,
    holdings,
  };
  ...
  return {
    ...
    sync_label: syncResult.remotePutUrl === "neon"
      ? "Confirmed in Neon"
      : syncResult.remotePutUrl ? (syncResult.remoteSynced ? "Confirmed remotely" : "Saved locally, remote sync failed") : "Confirmed locally",
  };
}

export async function updateHoldingsFromInstruction(snapshot, workspaceId, input) {
  if (input?.replacePortfolio === true) {
    return replaceConfirmedHoldings(snapshot, workspaceId, input);
  }
  if (input?.confirmHoldings === true) {
    const state = await loadHoldingsState(workspaceId);
    const holdings = normalizeHoldingsForOutput(state?.holdings || []);
    if (!holdings.length) throw new Error("No hay posiciones guardadas para confirmar.");
    const nextState = {
      available: true,
      source: state?.source || (usingNeonStorage() ? "neon_portfolio" : "ui_editable_overlay"),
      source_label: state?.source_label || "Cartera confirmada",
      updated_at: new Date().toISOString(),
      portfolio_generation_started_at: normalizePortfolioGenerationStartedAt(state?.portfolio_generation_started_at),
      workspace_id: workspaceId,
      instruction: `Confirmed ${holdings.length} existing positions`,
      holdings,
    };
    const syncResult = await saveHoldingsState(workspaceId, nextState);
    return {
      ...nextState,
      sync_status: syncResult.remotePutUrl === "neon" ? "neon_synced" : syncResult.remotePutUrl ? (syncResult.remoteSynced ? "remote_synced" : "remote_sync_failed") : "local_only",
      sync_label: syncResult.remotePutUrl === "neon" ? "Posiciones confirmadas en Neon" : "Posiciones confirmadas",
    };
  }
  ...
  if (cashEvent) {
    ...
    const nextState = {
      ...
      updated_at: new Date().toISOString(),
      portfolio_generation_started_at: normalizePortfolioGenerationStartedAt(
        state?.portfolio_generation_started_at,
      ),
      workspace_id: workspaceId,
  ...
  if (isExplicitHoldingsEdit(input)) {
    const ticker = String(input.ticker || input.symbol || "").toUpperCase();
    const state = await loadHoldingsState(workspaceId);
    const savedHolding = state?.holdings?.find((row) => String(row.ticker || "").toUpperCase() === ticker);
    const currentPrice = parseNumber(input?.price) ?? parseNumber(input?.currentPrice) ?? parseNumber(savedHolding?.current_price_usd) ?? getCurrentPriceForTicker(snapshot, ticker);
    if (currentPrice === null) {
      throw new Error(`No encontramos un precio para ${ticker}. Agrega un precio manual o prueba otra fecha.`);
    }
    ...
    const nextState = {
      ...
      source_label: remoteHoldingsStateEnabled() ? "Cartera sincronizada" : "Editada en la aplicación",
      updated_at: new Date().toISOString(),
      portfolio_generation_started_at: normalizePortfolioGenerationStartedAt(
        state?.portfolio_generation_started_at,
      ),
    ...
    return {
      ...
      sync_label: syncResult.remotePutUrl === "neon"
        ? "Guardada en Neon"
        : syncResult.remotePutUrl ? (syncResult.remoteSynced ? "Guardada y sincronizada" : "Guardada localmente; falló la sincronización") : "Guardada localmente",
    };
  }

  const instruction = assertUsableTradeInstruction(parseTradeInstruction(input?.instruction || input?.text || input));
  const tradeDate = parsePurchaseDate(input?.tradeDate ?? input?.trade_date);
  const suppliedPrice = parseNumber(input?.price) ?? parseNumber(input?.currentPrice) ?? parseNumber(input?.explicitPrice) ?? instruction.explicitPrice;
  const historicalPrice = suppliedPrice === null && tradeDate ? await getHistoricalTradePrice(instruction.ticker, tradeDate) : null;
  const currentPrice = suppliedPrice ?? historicalPrice?.price ?? getCurrentPriceForTicker(snapshot, instruction.ticker);
  if (currentPrice === null) {
    throw new Error(`No encontramos un precio para ${instruction.ticker}. Indica la fecha de la operación o agrega el precio manualmente.`);
  }
  ...
  const nextHoldings = applyInstructionToHoldings(baselineHoldings, instruction, currentPrice, meta, tradeDate);
  const totalValue = nextHoldings.reduce((sum, row) => sum + (parseNumber(row.market_value_usd) || 0), 0);
  ...
  const nextState = {
    ...
    trade: {
      ...
      price_source: suppliedPrice !== null ? "manual" : historicalPrice ? "historical_close" : "current_quote",
      parsed: instruction,
      meta,
    },
    holdings: nextHoldings.map((row) => ({
