...
function attachHistoryExternalFlows(historyRows, tradeRows) {
  ...
  const trades = (Array.isArray(tradeRows) ? tradeRows : [])
    .map((trade) => ({
      ...trade,
      flowUsd: signedExternalFlowUsd(trade),
      time: dateMillis(trade.created_at || trade.trade_date || trade.date),
    }))
  ...
  return rows.map((row, index) => {
    ...
    const externalFlowUsd = intervalTrades.reduce((sum, trade) => sum + trade.flowUsd, 0);
    return {
      ...row,
...
export function buildHistorySeries(rows) {
  ...
  return seriesRows.map((row, index) => {
    ...
    return {
      ...
      portfolio_growth: portfolioValue !== null ? twrIndex : null,
      value_growth:
...
async function getNeonPortfolioHistory(workspaceId, limit = 320) {
  ...
  const [historyRows, tradeRows] = await Promise.all([
    sql.query(
  ...
  return attachHistoryExternalFlows(mappedHistory, tradeRows);
}
