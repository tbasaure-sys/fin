...
function buildOverlayPortfolioAnalytics(snapshot, holdings, sourceLabel) {
  ...
  const unrealizedPnlUsd = totalCostUsd > 0 ? totalValueUsd - totalCostUsd : null;
...
function buildFreshnessBlockedPerformanceReport(freshness, preserveRecordedHistory) {
  ...
  return {
    ...
    inputs: {
      ...
      actions: ["Confirm the current holdings before running portfolio performance analysis."],
    },
    current: null,
    personalHeadline: {
      available: false,
    ...
    explanation: ["Confirm the current holdings before running portfolio performance analysis."],
  };
}
    ...
    portfolio: {
      ...
      holdings_sync_label: !freshness.canAnalyze
        ? freshness.status === "stale"
          ? "Confirmation required before repricing"
          : "Confirm holdings before analysis"
...
async function replaceConfirmedHoldings(snapshot, workspaceId, input) {
  ...
  const nextState = {
    ...
    source_label: "Confirmed portfolio",
