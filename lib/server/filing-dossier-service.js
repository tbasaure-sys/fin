...
import {revenueBreakdown} from '../research/revenue-breakdown.mjs';
import {
  selectFilings,
  documentStructure,
  makeChunks,
...
export function createFilingService({
  ...
  async function build(raw) {
    ...
    try {
      ...
      const filings = selectFilings(submissions.filings.recent, asOf),
        sources = [],
        chunks = [];
      const newest=[...filings].sort((a,b)=>Date.parse(b.periodEnd)-Date.parse(a.periodEnd)||Date.parse(b.acceptedAt)-Date.parse(a.acceptedAt))[0];
      let revenue=null;
      for (const [i, filing] of filings.entries()) {
        ...
        const raw = await get(url),
          id = `D${i + 1}`,
          blocks = documentStructure(raw.toString());
        sources.push({
          ...filing,
          id,
          url,
        ...
        if(filing.accession===newest.accession)revenue=revenueBreakdown(raw.toString(),{...sources.at(-1),cik});
  ...
  return async function load(raw) {
    const ticker = cleanTicker(raw);
    if (!ticker) throw Error("INVALID_TICKER");
    // Identical readers join the same bounded job, including when the first
    // browser request disconnects. Different issuers retain the existing limit.
    if (pending.has(ticker)) return pending.get(ticker);
    const job = build(ticker);
    pending.set(ticker, job);
    try {
      return await job;
    } finally {
