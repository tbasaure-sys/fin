    ...
    try {
      ...
      for (const [i, filing] of filings.entries()) {
        ...
        try { raw = await get(url); }
          ...
          if (filing.form !== '6-K') throw error;
          unavailable.push({form: filing.form, url, filedAt: filing.filedAt});
          continue;
        }
        const id = `D${i + 1}`,
      ...
      const value = {
        ...
        delivery: "live_fetch",
        unavailable,
        scope: sources.some(s => /^(20-F|40-F)/.test(s.form)) ? "annual_and_latest_four_6k_primary_documents_excludes_exhibits" : "annual_and_subsequent_quarterlies",
      };
