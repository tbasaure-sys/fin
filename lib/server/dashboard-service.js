...
function buildFallbackWorkspaceDashboard(workspaceId, error) {
...
export async function getWorkspaceDashboard(workspaceId) {
  try {
    const [watchlist, alerts, savedViews, commandHistory, escrowDecisions, decisionEvents, backendSnapshotResult] = await Promise.all([
      ...
      fetchBackendSnapshot().catch(async (error) => {
        const persisted = await loadPersistedDashboardSnapshot();
        return persisted || buildUnavailableSnapshot(error);
      }),
    ]);
    ...
    return buildFallbackWorkspaceDashboard(workspaceId, error);
  }
}
