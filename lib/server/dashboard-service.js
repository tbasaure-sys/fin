...
function isSnapshotUsable(snapshot) {
  ...
  if (ageHours > operational.snapshot.maxAgeHours) return false;
  ...
  if (Number.isFinite(staleDays) && staleDays > operational.snapshot.maxQuoteStaleDaysWithoutPortfolio && !hasPortfolioContext) return false;
  if (quotesAsOf === null && !hasPortfolioContext && ageHours > operational.snapshot.maxAgeHoursWithoutQuotes) return false;
