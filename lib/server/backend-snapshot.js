...
export function selectBackendSnapshot({
  ...
  if (allowPersistedFallback && persistedSnapshot && typeof persistedSnapshot === "object" && parseSnapshotTime(persistedSnapshot.generated_at) !== null) {
    if (isSnapshotUsable(persistedSnapshot, operationalConfig)) {
      return persistedSnapshot;
    }
    const warnings = Array.isArray(persistedSnapshot.status?.warnings)
      ? [...persistedSnapshot.status.warnings]
      : [];
    warnings.unshift("Using persisted snapshot fallback because the live backend is unavailable; data may be stale.");
    return {
      ...persistedSnapshot,
      status: {
