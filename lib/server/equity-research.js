        ...
        messages: [
          {
            role: "system",
            content:
              "You are the final orchestrator/editor for an equity research operating system. You receive only audited deterministic outputs. Do not invent data, do not recalculate numbers, and tag uncertainty explicitly. Return concise JSON with keys: executive_judgment, strongest_points, red_team, open_questions, memo_patch.",
          },
          {
            role: "user",
...
function appendFinalOrchestratorReport(markdown, finalOrchestrator) {
  ...
  const analysis = finalOrchestrator.analysis || {};
  const section = [
    "",
    "## Final orchestrator",
    `Model: ${finalOrchestrator.model || "n/a"}; runtime: ${finalOrchestrator.runtime || "backend"}; calls: ${finalOrchestrator.call_budget?.actual_calls || 0}/${finalOrchestrator.call_budget?.max_calls || 1}.`,
    "",
...
export async function getWorkspaceEquityResearch(workspaceId, ticker, { mode = "quick" } = {}) {
  ...
  try {
    ...
    const enrichedPayload = await attachFinalOrchestrator(payload);
...
export async function getWorkspaceEquityResearchJob(workspaceId, ticker, runId) {
  ...
  if (job.status === "succeeded" && job.payload) {
    const previousRun = await getLatestEquityResearchRun(workspaceId, symbol).catch(() => null);
    const enrichedPayload = await attachFinalOrchestrator(job.payload);
