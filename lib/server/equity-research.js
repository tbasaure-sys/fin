...
function buildMarketOnlyResearchPayload(ticker, snapshot) {
  const symbol = cleanTicker(ticker);
  const price = Number(snapshot?.price);
  const marketCap = Number(snapshot?.marketCap);
  const currency = String(snapshot?.currency || "USD").trim().toUpperCase();
  const marketDate = String(snapshot?.marketDate || "").trim();
  const marketProvider = String(snapshot?.source || "Proveedor de mercado").trim();
  const sourceId = /financial modeling prep/i.test(marketProvider) ? "fmp:quote" : "yfinance:prices";
  const fundamentals = snapshot?.fundamentals || {};
  const annualRow = {
    fiscal_year: finiteOrNull(fundamentals.fiscalYear),
    revenue: finiteOrNull(fundamentals.revenue),
    net_income: finiteOrNull(fundamentals.netIncome),
    total_equity: finiteOrNull(fundamentals.totalEquity),
    free_cash_flow: finiteOrNull(fundamentals.freeCashFlow),
    cash: finiteOrNull(fundamentals.cash),
    total_debt: finiteOrNull(fundamentals.debt),
    diluted_shares: finiteOrNull(fundamentals.dilutedShares),
  };
  const hasFundamentals = Object.entries(annualRow).some(([key, value]) => key !== "fiscal_year" && value !== null);
  if (!symbol || !Number.isFinite(price) || price <= 0 || !/^\d{4}-\d{2}-\d{2}$/.test(marketDate)) {
    throw new Error("No pudimos recuperar una cotización fechada para esta acción.");
  }
  return {
    ok: true,
    ticker: symbol,
    ...
    company_profile: {
      name: String(snapshot?.name || symbol).trim() || symbol,
      ticker: symbol,
      exchange: String(snapshot?.exchange || "").trim() || null,
      currency: /^[A-Z]{3}$/.test(currency) ? currency : "USD",
      market_cap: Number.isFinite(marketCap) && marketCap > 0 ? marketCap : null,
      sector: String(snapshot?.sector || "").trim() || null,
      industry: String(snapshot?.industry || "").trim() || null,
    },
    financials: {
      annual: hasFundamentals ? [annualRow] : [],
      ratios: {
        latest_revenue: annualRow.revenue,
        latest_net_income: annualRow.net_income,
        latest_total_equity: annualRow.total_equity,
        latest_fcf: annualRow.free_cash_flow,
        latest_cash: annualRow.cash,
        latest_debt: annualRow.total_debt,
        latest_diluted_shares: annualRow.diluted_shares,
      },
      quality_flags: [],
    },
    valuation: {
      ...
      financial_data_as_of: null,
      reason: "La cotización permite construir un rango exploratorio mientras se amplía la base financiera.",
      range: null,
      scenarios: [],
      price_validation: {
      ...
      reliability: {
        ...
        status: "blocked",
        score: 0.2,
        reasons: ["La estimación parte de cotización y priors sectoriales amplios."],
        limitations: ["El intervalo se ampliará o estrechará al incorporar estados financieros."],
      },
    },
    sources: {
      coverage: {
      ...
      records: [{
        ...
        status: "ok",
        label: "Cotización fechada",
        as_of: marketDate,
        retrieved_at: new Date().toISOString(),
      }],
      data_points: [{ metric: "current_price", value: price, normalized_value: price, source_id: sourceId }],
    },
...
export async function resolvePublicEquityResearchPayload(ticker, { backendLoader, marketLoader } = {}) {
  const symbol = cleanTicker(ticker);
  if (!symbol || typeof backendLoader !== "function" || typeof marketLoader !== "function") {
    throw new Error("No pudimos iniciar la lectura de esta acción.");
  }
  try {
    const payload = await backendLoader(symbol, "quick");
    if (payload && payload.ok !== false) return { source: "canonical_backend", payload };
  } catch {
    // The quote fallback below keeps the valuation useful when the canonical service is temporarily unavailable.
  }
  const snapshot = await marketLoader(symbol);
  const instrumentType = String(snapshot?.instrumentType || "EQUITY").trim().toUpperCase();
  if (!new Set(["EQUITY", "ADR"]).has(instrumentType)) {
    throw new Error("Este flujo admite sólo una acción o ADR.");
  }
  if (cleanTicker(snapshot?.ticker) !== symbol) {
    throw new Error("La cotización recuperada no corresponde a la acción solicitada.");
  }
  return { source: "market_fallback", payload: buildMarketOnlyResearchPayload(symbol, snapshot) };
}
...
const PUBLIC_AUDIT_MESSAGES = Object.freeze({
  ...
  evidence_coverage_gap: "La cobertura de datos o fuentes todavía es parcial.",
