...
export function validateInvestmentThesis(raw,dossier){
 ...
 const sections=raw.sections.map((s,i)=>{
  ...
  const findings=s.findings.map(f=>{
   ...
   if(numbers(f.text).some(n=>!citedNumbers.has(n)))return null;
   return {kind:'hypothesis',text:f.text,evidence};
  });
  return {id:s.id,findings:findings.filter(Boolean),withheld:findings.filter(f=>f===null).length,unknowns:s.unknowns,checks:s.checks};
 });
