 ...
 *
 * It produces five clearly-separated readings:
 ...
 *  - backcast:       hypothetical behavior of today's positions held constant
 *
...
/**
 * Select the portfolio owner's headline performance without consulting any
 * hypothetical history. `actual` is the canonical portfolio accounting
 * reading when available; `current` is the cost-basis reading derived from the
 * currently loaded holdings. A current-weight backcast is deliberately not an
 * input to this selector.
 */
export function selectPersonalPerformanceHeadline({ actual = null, current = null } = {}) {
  const actualPnl = toNumber(actual?.totalPnlUsd);
  const actualReturn = toNumber(actual?.totalReturn);
  const actualCostBasis = toNumber(actual?.costBasisUsd);
  if (actualPnl !== null || actualReturn !== null) {
    return {
      ...
      costBasisUsd: actualCostBasis,
    };
  }

  const currentPnl = toNumber(current?.totalPnlUsd);
  const currentReturn = toNumber(current?.totalReturn);
  const currentCostBasis = toNumber(current?.totalCostUsd);
  if (currentPnl !== null || currentReturn !== null) {
    return {
      available: true,
      method: "current_cost_basis",
      pnlUsd: currentPnl,
      returnValue: currentReturn,
      costBasisUsd: currentCostBasis,
    };
  }
...
/**
 * Backcast the positions held today across real historical closes. This is a
 * hypothetical history of the current book, not the investor's realized
 * return, so it deliberately needs neither purchase dates nor saved snapshots.
 */
export function buildCurrentWeightBackcast({ holdings, priceHistory, benchmarkHistory, benchmarkSymbol = "SPY" }) {
  const allHoldings = (Array.isArray(holdings) ? holdings : [])
    .filter((row) => row?.ticker && (toNumber(row.quantity) || 0) > 0);
  const rows = allHoldings
    .map((row) => {
      const ticker = String(row.ticker).toUpperCase();
      const quantity = toNumber(row.quantity);
      const series = priceHistory?.[ticker];
      if (quantity === null || quantity <= 0 || !series || !Object.keys(series).length) return null;
      return { ticker, quantity, series };
    })
    .filter(Boolean);
  ...
  if (!rows.length) {
    return { status: "insufficient_inputs", method: "current_weight_backcast", series: [], includedTickers, excludedTickers };
  }

  const dates = sortedDatesFromPanel(Object.fromEntries(rows.map((row) => [row.ticker, row.series])));
  const rawSeries = [];
  for (const date of dates) {
    let portfolioValue = 0;
    let priced = 0;
    for (const row of rows) {
      const close = closeOnOrBefore(row.series, date);
      if (close !== null && close > 0) {
        portfolioValue += row.quantity * close;
        priced += 1;
      }
    }
    if (priced !== rows.length || portfolioValue <= 0) continue;
    rawSeries.push({
      date,
      portfolio_value_usd: portfolioValue,
      benchmark_close: closeOnOrBefore(benchmarkHistory, date),
    });
  }

  if (rawSeries.length < 2) {
    return { status: "insufficient_history", method: "current_weight_backcast", series: [], includedTickers, excludedTickers };
  }

  const firstPortfolioValue = rawSeries[0].portfolio_value_usd;
  const firstBenchmarkClose = rawSeries.find((row) => row.benchmark_close !== null)?.benchmark_close || null;
  const series = rawSeries.map((row) => ({
    date: row.date,
    portfolio_value_usd: row.portfolio_value_usd,
    portfolio_growth: row.portfolio_value_usd / firstPortfolioValue,
    spy_growth: firstBenchmarkClose && row.benchmark_close ? row.benchmark_close / firstBenchmarkClose : null,
    external_flow_usd: 0,
    performance_method: "current_weight_backcast",
  }));
  const last = rawSeries[rawSeries.length - 1];

  return {
    status: "ok",
    method: "current_weight_backcast",
    methodLabel: "Backcast hipotético de las posiciones actuales",
    interpretation: "hypothetical_current_book_diagnostic",
    benchmarkSymbol,
    includedTickers,
    excludedTickers,
    series,
    valueChangeUsd: last.portfolio_value_usd - firstPortfolioValue,
  };
}
...
function metricsFromValueSeries(series, valueKey, benchmarkKey, riskFreeRate = 0.04) {
  ...
  const sharpeRatio = annualizedReturn !== null && annualVolatility !== null && annualVolatility > 0
    ? (annualizedReturn - riskFreeRate) / annualVolatility
    : null;
  ...
  return {
    ...
    sharpeRatio,
    sharpeRatioLabel: sharpeRatio !== null ? sharpeRatio.toFixed(2) : null,
    maxDrawdown,
 ...
 */
export function buildPerformanceReport({ holdings, actualPerformance = null, reconstruction = null, backcast = null, snapshotHistoryRows = [], twrMetrics = null, benchmarkSymbol = "SPY" }) {
  const inputs = assessPerformanceInputs(holdings);
  const current = buildCostBasisPerformance(holdings);
  const personalHeadline = selectPersonalPerformanceHeadline({ actual: actualPerformance, current });
  ...
  const reconstructed = reconstruction?.status === "ok" && reconQuality?.usable
    ...
    : null;
  const backcastQuality = backcast?.series?.length ? assessSeriesQuality(backcast.series) : null;
  const backcastReading = backcast?.status === "ok" && backcastQuality?.usable
    ? {
      ...metricsFromValueSeries(backcast.series, "portfolio_growth", "spy_growth"),
      method: "current_weight_backcast",
      methodLabel: "Backcast hipotético de las posiciones actuales",
      interpretation: "hypothetical_current_book_diagnostic",
      includedTickers: backcast.includedTickers,
      excludedTickers: backcast.excludedTickers,
      valueChangeUsd: backcast.valueChangeUsd,
    }
    : null;

  // Recorded observations have their own flow/coverage validation. A genuine
  // terminal zero must not be rejected by a generic synthetic-chart quality gate.
  const twr = twrMetrics && snapshotHistoryRows.length >= 2
    ? {
      method: "twr_snapshots",
      methodLabel: twrMetrics.performanceMethod === 'modified_dietz_approximation' ? 'Rendimiento aproximado · Modified Dietz' : twrMetrics.performanceStatus === 'unresolved' ? 'Rendimiento pendiente · historial incompleto' : 'TWR · flujos y límites observados',
  ...
  } else if (backcastReading && backcastReading.totalReturn !== null) {
    explanation.push(`Como diagnóstico hipotético, mantener constantes las posiciones actuales habría producido ${backcastReading.totalReturnLabel} en ${backcastReading.spanDays} días frente a ${benchmarkSymbol}. No es tu P&L ni tu retorno personal.`);
  ...
  if (twr) {
    explanation.push(`${twr.methodLabel}: ${twr.totalTwrLabel || "no calculable"}. Ajusta los aportes y retiros registrados; no mide por sí solo habilidad ni alpha.`);
  }
  ...
  return {
    ...
    personalHeadline,
    reconstructed,
    backcast: backcastReading,
    backcastStatus: backcast?.status || "not_run",
    backcastQuality,
