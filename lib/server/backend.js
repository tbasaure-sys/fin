...
function requestTimeoutMs(path, options = {}) {
  ...
  if (method === "POST" && path === "/api/equity-research/jobs") {
    return envNumberFloor("BLS_PRIME_BACKEND_RESEARCH_JOB_TIMEOUT_MS", 15000, 10000);
  }
...
export async function startBackendEquityResearchJob(ticker, mode = "quick") {
  const symbol = String(ticker || "").trim().toUpperCase();
  const reportMode = mode === "full" ? "full" : "quick";
  return fetchBackendJson("/api/equity-research/jobs", {
    method: "POST",
    body: JSON.stringify({ ticker: symbol, mode: reportMode }),
  });
}
