...
function requestTimeoutMs(path, options = {}) {
  ...
  if (method === "GET" && path.startsWith("/api/equity-research/jobs/")) {
    return envNumberFloor("BLS_PRIME_BACKEND_RESEARCH_POLL_TIMEOUT_MS", 15000, 8000);
  }
  if (method === "GET" && path.startsWith("/api/equity-research")) {
    return envNumberFloor("BLS_PRIME_BACKEND_RESEARCH_DIRECT_TIMEOUT_MS", 12000, 8000);
  }
