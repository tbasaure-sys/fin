...
export function createResearchMarketService({
  ...
  async function portfolio(holdings) {
    if (scans >= 2) throw Error("BUSY");
    scans++;
    try {
      const universe = portfolioUniverse(holdings),
        results = new Array(universe.holdings.length);
      let index = 0;
      await Promise.all(
        Array.from({ length: Math.min(4, results.length) }, async () => {
  ...
  return { fundamentals, news, portfolio };
}
