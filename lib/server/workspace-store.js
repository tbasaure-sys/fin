...
async function getNeonPositionStories(workspaceId) {
  ...
  return rows.map((row, index) => normalizePositionStoryEntry({
    ticker: row.ticker,
    ...(row.payload || {}),
  }, index));
}
...
export async function upsertWorkspacePositionStory(workspaceId, story) {
  ...
  if (!usingNeonStorage()) {
    ...
    next.unshift(entry);
    store.positionStories.set(workspaceId, next.slice(0, 16));
...
export async function upsertWorkspaceCounterfactualOutcome(workspaceId, outcome) {
  ...
  if (!usingNeonStorage()) {
    ...
    const next = current.filter((item) => item.id !== entry.id);
    next.unshift(entry);
    store.counterfactualOutcomes.set(workspaceId, next.slice(0, 24));
