...
const store = globalThis.__BLS_PRIME_STORE__ || {
  ...
  mandates: new Map(),
  financePlans: new Map(),
};

globalThis.__BLS_PRIME_STORE__ = store;
if (!store.financePlans) store.financePlans = new Map();
...
function normalizeDecisionEventEntry(item, index = 0) {
  ...
  return {
    ...
    recordVersion: String(item?.recordVersion || item?.record_version || "legacy.v1").trim() || "legacy.v1",
    subjectType: String(item?.subjectType || item?.subject_type || "").trim() || null,
    subjectKey: String(item?.subjectKey || item?.subject_key || "").trim() || null,
    asOfDate: item?.asOfDate || item?.as_of_date || null,
...
function normalizeCurrencyCode(value) {
  const normalized = String(value || "USD").trim().toUpperCase().replace(/[^A-Z]/g, "").slice(0, 3);
  return normalized || "USD";
}

function normalizeFinancePlanEntry(item = {}) {
  return {
    monthlyIncome: normalizeNonNegativeNumber(item?.monthlyIncome ?? item?.monthly_income, 0),
    fixedExpenses: normalizeNonNegativeNumber(item?.fixedExpenses ?? item?.fixed_expenses, 0),
    variableExpenses: normalizeNonNegativeNumber(item?.variableExpenses ?? item?.variable_expenses, 0),
    safetyBuffer: normalizeNonNegativeNumber(item?.safetyBuffer ?? item?.safety_buffer, 0),
    targetMonthlyInvestment: normalizeNonNegativeNumber(
      item?.targetMonthlyInvestment ?? item?.target_monthly_investment,
      0,
    ),
    baseCurrency: normalizeCurrencyCode(item?.baseCurrency ?? item?.base_currency),
    updatedAt: item?.updatedAt || item?.updated_at || null,
  };
}
...
async function getNeonFinancePlan(workspaceId) {
  const sql = await ensureNeonWorkspace(workspaceId);
  const rows = await sql.query(
    `SELECT monthly_income, fixed_expenses, variable_expenses, safety_buffer,
            target_monthly_investment, base_currency, updated_at
     FROM bls_workspace_finance_plans
     WHERE workspace_id = $1
     LIMIT 1`,
    [workspaceId],
  );

  return normalizeFinancePlanEntry(rows[0] || {});
}
...
export async function getWorkspaceFinancePlan(workspaceId) {
  if (!usingNeonStorage()) {
    return ensureWorkspaceEntry(store.financePlans, workspaceId, normalizeFinancePlanEntry({}));
  }

  try {
    return await getNeonFinancePlan(workspaceId);
  } catch {
    return ensureWorkspaceEntry(store.financePlans, workspaceId, normalizeFinancePlanEntry({}));
  }
}

export async function upsertWorkspaceFinancePlan(workspaceId, financePlan) {
  const entry = normalizeFinancePlanEntry({
    ...financePlan,
    updatedAt: new Date().toISOString(),
  });

  if (!usingNeonStorage()) {
    store.financePlans.set(workspaceId, entry);
    return store.financePlans.get(workspaceId);
  }

  try {
    const sql = await ensureNeonWorkspace(workspaceId);
    await sql.query(
      `INSERT INTO bls_workspace_finance_plans (
         workspace_id,
         monthly_income,
         fixed_expenses,
         variable_expenses,
         safety_buffer,
         target_monthly_investment,
         base_currency
       )
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       ON CONFLICT (workspace_id)
       DO UPDATE SET
         ...
         target_monthly_investment = EXCLUDED.target_monthly_investment,
      ...
      [
        ...
        entry.monthlyIncome,
        entry.fixedExpenses,
        entry.variableExpenses,
        entry.safetyBuffer,
        entry.targetMonthlyInvestment,
        entry.baseCurrency,
      ],
    );
    return await getNeonFinancePlan(workspaceId);
