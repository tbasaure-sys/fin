...
function isE2EAuthBypassEnabled() {
  ...
  const value = String(process.env.BLS_PRIME_E2E_AUTH_BYPASS || "").trim().toLowerCase();
  return value === "1" || value === "true" || value === "yes";
}

async function buildE2EAuthSession() {
  const userId = "e2e-user";
  const workspaceId = String(process.env.BLS_PRIME_E2E_WORKSPACE_ID || "workspace-e2e").trim() || "workspace-e2e";
  const plan = await buildPlanContext("founder", userId);
...
function getSessionCookieDomain() {
  // Each preview is its own host. Production cookie settings are invalid here;
  // deriving `.vercel.app` would also target a shared public suffix.
  if (String(process.env.VERCEL_ENV || "").trim() === "preview") return undefined;
...
export async function requireApiPaidWorkspaceSession(request, workspaceId) {
  ...
  if (!sessionHasPrivateWorkspaceAccess(session)) {
    return new Response(
      JSON.stringify({
        error: "This workspace is not enabled for the current account yet.",
