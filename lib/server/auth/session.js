...
async function getSessionRecordByToken(token) {
  ...
  const rows = await sql.query(
    `SELECT
       ...
       u.display_name,
       u.plan,
       COALESCE(mw.id, ow.id) AS workspace_id,
       COALESCE(mw.name, ow.name) AS workspace_name,
       COALESCE(mw.visibility, ow.visibility) AS workspace_visibility
     FROM bls_auth_sessions s
     JOIN bls_user_profiles u ON u.id = s.user_profile_id
     LEFT JOIN bls_workspaces ow ON ow.owner_user_id = u.id
     LEFT JOIN bls_workspace_members wm ON wm.user_id = u.id
     LEFT JOIN bls_workspaces mw ON mw.id = wm.workspace_id
     WHERE s.session_token_hash = $1
       AND s.expires_at > NOW()
     ORDER BY CASE WHEN wm.role = 'owner' OR ow.owner_user_id IS NOT NULL THEN 0 ELSE 1 END,
              COALESCE(mw.created_at, ow.created_at) ASC NULLS LAST
     LIMIT 1`,
  ...
  const ensuredWorkspace = row.workspace_id
    ? null
  ...
  return {
    ...
    workspace: {
      id: ensuredWorkspace?.id || row.workspace_id || createWorkspaceIdForUser(row.user_id),
      name: ensuredWorkspace?.name || row.workspace_name || createWorkspaceNameForUser(row.display_name, row.email),
