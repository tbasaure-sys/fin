...
import {interimFinancials} from '../research/interim-financials.mjs';
...
export function normalizeThesisFinancials(raw,submissions,dossier,sourceHash,retrievedAt){
 ...
 function pick(concepts,{unit='USD',annual=false,namespace=gaap,end}={}){
  const all=concepts.flatMap((concept,priority)=>(namespace[concept]?.units?.[unit]||[]).filter(r=>{
   ...
   return sources.has(r.accn)&&typeof r.val==='number'&&Number.isFinite(r.val)&&Date.parse(r.end)<=Date.parse(sources.get(r.accn).acceptedAt)
    &&(!end||r.end===end)&&(!annual||(/^10-K(?:\/A)?$/.test(r.form)&&duration>=330&&duration<=380));
 ...
 const annualConcepts={
  ...
  ebit:['OperatingIncomeLoss'],netIncome:['NetIncomeLoss'],cfo:['NetCashProvidedByUsedInOperatingActivities'],
  capex:['PaymentsToAcquirePropertyPlantAndEquipment'],sbc:['ShareBasedCompensation'],
  buybacks:['PaymentsForRepurchaseOfCommonStock'],dividends:['PaymentsOfDividendsCommonStock','PaymentsOfDividends'],
  ...
  payablesChange:['IncreaseDecreaseInAccountsPayable'],
 };
 // Comparative years contained in the approved filings, not a fabricated PIT history.
 const ends=[...new Set(annualConcepts.revenue.flatMap(c=>(gaap[c]?.units?.USD||[]).filter(r=>{
  const days=(Date.parse(r.end)-Date.parse(r.start))/86400000;
  return sources.has(r.accn)&&/^10-K(?:\/A)?$/.test(r.form)&&days>=330&&days<=380&&Date.parse(r.end)<=Date.parse(sources.get(r.accn).acceptedAt);
 }).map(r=>r.end)))].sort().slice(-3);
 const history=ends.map(end=>({end,facts:Object.fromEntries(Object.entries(annualConcepts).map(([key,concepts])=>
  [key,pick(concepts,{annual:true,end,unit:key==='shares'?'shares':'USD'})]))}));
 ...
 return {ticker:dossier.ticker,cik:dossier.cik,asOf:dossier.asOf,retrievedAt,currency:'USD',
  ...
  facts:{revenue,ebit,cash,debt,shares,currentShares},history,interim:interimFinancials({raw,dossier,history,sourceHash}),warnings:['DEBT_COMPONENTS_REQUIRE_LEASE_AND_CLAIMS_REVIEW','LIVE_COMPANY_METADATA_NOT_HISTORICAL'],sourceHash};
}
...
export function createFinancialLoader({fetcher=fetch,clock=()=>new Date(),includeQuote=true}={}){
 ...
 async function load(dossier){
  ...
  if(!includeQuote)return {financial,quote};
