...
function combinedEvidenceFreshness(evidence, now) {
  if (!evidence.length || evidence.some((row) => !row.sourceId || !row.asOf)) {
    return { status: "unknown", ageDays: null, usable: false };
  }
  const rows = evidence.map((row) => evidenceFreshness(row.asOf, now));
  const ageDays = Math.max(...rows.map((row) => row.ageDays));
  if (rows.some((row) => row.status === "future")) {
    return { status: "future", ageDays: Math.min(...rows.map((row) => row.ageDays)), usable: false };
  }
  if (rows.some((row) => row.status === "stale")) return { status: "stale", ageDays, usable: false };
  if (rows.some((row) => row.status === "lagged")) return { status: "lagged", ageDays, usable: true };
  return { status: "current", ageDays, usable: true };
}

function oldestEvidenceDate(evidence) {
  return evidence.map((row) => row.asOf).filter(Boolean).sort().at(0) || null;
}
...
function normalizeLiquidityLegacy(raw) {
...
function normalizeLiquidity(raw, payload, now) {
  const componentEntries = Object.entries(raw?.components || {});
  const components = componentEntries
    .filter(([key]) => !["us_net_liquidity", "us_net_liquidity_ex_rrp"].includes(key))
    .map(([key, value]) => ({
      label: String(value?.label || key),
      stance: finiteNumber(value?.impulse, 0) < 0 ? "drena levemente" : "aporta levemente",
    }));
  const main = raw?.components?.us_net_liquidity || raw?.components?.us_net_liquidity_ex_rrp || {};
  const explicitSourceIds = safeList(raw?.sourceIds || raw?.source_ids).map(String);
  const sourceIds = [...new Set((explicitSourceIds.length
    ? explicitSourceIds
    : componentEntries
        .filter(([key]) => !["us_net_liquidity", "us_net_liquidity_ex_rrp"].includes(key))
        .map(([key]) => key))
    .filter(Boolean))];
  const datedSeries = new Map(safeList(payload?.impulse_changes).map((row) => [
    String(row?.series_id || ""),
    normalizedDate(row?.date || row?.as_of || row?.asOf),
  ]));
  const overallAsOf = normalizedDate(raw?.asOf || raw?.as_of || raw?.latest_date || raw?.date);
  const evidence = sourceIds.map((sourceId) => {
    const component = raw?.components?.[sourceId] || {};
    return {
      sourceId,
      asOf: normalizedDate(component?.asOf || component?.as_of || component?.latest_date || component?.date)
        || overallAsOf
        || datedSeries.get(sourceId)
        || null,
    };
  });
  const measuredFreshness = combinedEvidenceFreshness(evidence, now);
  const usable = raw?.status === "available" && sourceIds.length > 0 && measuredFreshness.usable;
  return {
    status: usable ? "Parcial" : measuredFreshness.status === "stale" ? "Desactualizada" : "Pendiente",
    summary: raw?.summary || "La liquidez exige fuentes identificadas y fechadas antes de afectar una valuacion.",
    direction: main?.impulse_direction || "neutral",
    impulse: finiteNumber(main?.impulse, 0),
    components,
    asOf: oldestEvidenceDate(evidence),
    sourceIds,
    evidence,
    freshness: { ...measuredFreshness, usable },
    usable,
  };
}
...
export function normalizeMacroBrainPayload(raw, source = {}, options = {}) {
  ...
  const generatedOn = String(raw.generated_on || "");
  const now = options.now || new Date().toISOString();
  const macroEvidence = safeList(raw.impulse_changes).map((row) => ({
    sourceId: String(row?.series_id || "").trim() || null,
    asOf: normalizedDate(row?.date || row?.as_of || row?.asOf),
  }));
  const macroFreshness = combinedEvidenceFreshness(macroEvidence, now);
  const evidenceAsOf = oldestEvidenceDate(macroEvidence);
  ...
  return {
    ...
    freshnessLabel: evidenceAsOf ? `Datos verificados al ${evidenceAsOf}` : "Sin fecha verificable en las series",
    dataStatus: source.status || "Leído ahora desde la última corrida disponible.",
    live: macroFreshness.usable,
    dataState: macroFreshness.status,
    ...
    liquidity: normalizeLiquidity(raw.liquidity || {}, raw, now),
    theses,
    nextChecks,
    stability,
    ledger: {
