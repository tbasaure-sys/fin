...
export function createJevService({fetcher=fetch,env=process.env,clock=()=>Date.now()}={}) {
  ...
  async function evaluate(kind,items) {
    ...
    if(!['news','documents','thesis'].includes(kind)||items.length>60)throw Error('INVALID_REQUEST');
    const groups=[];for(let i=0;i<items.length;i+=10)groups.push(items.slice(i,i+10));
