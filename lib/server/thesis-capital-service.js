...
export function createCapitalService({store,thesisStore,loadFinancial,readPortfolio,clock=()=>new Date()}){
 ...
 return {
  ...
  async run(owner,workspace,body){
   ...
   const thesis=await owned(owner,body.thesisHash);let content;
   if(body.action==='load'){
    ...
    content={kind:'inputs',financial,quote,portfolio,assumptions:defaultAssumptions(financial)};
   }else{
    ...
    const at=clock().toISOString(),valuation=valueThesis(inputs.financial,body.assumptions,inputs.quote,at);
   ...
   const record={version:'bls-thesis-capital-record-v1',thesisHash:body.thesisHash,savedAt:clock().toISOString(),...content};record.hash=hash(record);
