...
function covarianceToCorrelation(covariance) {
  const volatility = covariance.map((row, index) => Math.sqrt(Math.max(row[index], 1e-16)));
  return covariance.map((row, i) => row.map((value, j) => {
    if (i === j) return 1;
    const denominator = volatility[i] * volatility[j];
    if (!(denominator > 0)) return 0;
    return Math.max(-1, Math.min(1, value / denominator));
  }));
}

function averageWithinCluster(indices, correlation) {
  if (indices.length < 2) return 0;
  let sum = 0;
  let count = 0;
  for (let left = 0; left < indices.length; left += 1) {
    for (let right = left + 1; right < indices.length; right += 1) {
      sum += correlation[indices[left]][indices[right]];
      count += 1;
    }
  }
...
function buildCorrelationClusters(holdings, correlation, threshold = 0.65) {
  ...
  while (remaining.size) {
    ...
    while (queue.length) {
      ...
      for (const candidate of [...remaining]) {
        if (correlation[current][candidate] >= threshold) {
  ...
  return components
    .map((indices) => {
      const sectors = indices
        .map((index) => holdings[index].sector)
        .filter((sector) => sector && sector !== "Unknown");
      const sectorCounts = sectors.reduce((counts, sector) => {
        counts.set(sector, (counts.get(sector) || 0) + 1);
        return counts;
      }, new Map());
      const dominantSector = [...sectorCounts.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0]?.[0] || null;
      const tickers = indices.map((index) => holdings[index].ticker);
      return {
        tickers,
        indices,
        weight: indices.reduce((sum, index) => sum + holdings[index].weight, 0),
        average_correlation: averageWithinCluster(indices, correlation),
        dominant_sector: dominantSector,
      };
    })
    .sort((left, right) => right.weight - left.weight || left.tickers[0].localeCompare(right.tickers[0]))
    .map((cluster, index) => ({
      id: `cluster-${index + 1}`,
      label: cluster.dominant_sector || (cluster.tickers.length > 1 ? cluster.tickers.join(" + ") : cluster.tickers[0]),
      tickers: cluster.tickers,
      holdings_count: cluster.tickers.length,
      weight: Number(cluster.weight.toFixed(4)),
      average_correlation: Number(cluster.average_correlation.toFixed(3)),
...
function seriesMetrics(returnRows, weights) {
  const records = [];
  const availableWindows = returnRows.length - (WINDOW_DAYS - 1);
  const targetPoints = weights.length > 24 ? 64 : SERIES_POINTS;
  const stride = Math.max(1, Math.ceil(availableWindows / targetPoints));
  for (let end = WINDOW_DAYS - 1; end < returnRows.length; end += 1) {
    const isLast = end === returnRows.length - 1;
    if (!isLast && (end - (WINDOW_DAYS - 1)) % stride !== 0) continue;
...
function contributorRows(returnRows, holdings, current) {
  ...
  for (let index = 0; index < holdings.length; index += 1) {
    ...
    const reducedCurrent = windowMetrics(
      reducedReturns.slice(-WINDOW_DAYS).map((row) => row.returns),
      reducedWeights,
    );
...
export function analyzePhantomBreadth(rows, priceHistory, { workspaceId = null } = {}) {
  ...
  const latestReturns = returnRows.slice(-WINDOW_DAYS).map((row) => row.returns);
  const currentCovariance = ledoitWolfShrinkage(latestReturns);
  const correlation = covarianceToCorrelation(currentCovariance);
  const clusters = buildCorrelationClusters(holdings, correlation);
  ...
  return {
    ...
    correlation_matrix: {
      ...
      values: correlation.map((row) => row.map((value) => Number(value.toFixed(3)))),
