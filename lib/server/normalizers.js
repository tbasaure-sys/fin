...
function buildEdgeBoard(snapshot, sharedAlpha) {
  ...
  const sectors = sectorRows.length
    ? sectorRows
    : (sharedAlpha.edgeBoard?.sectors || []).map((row) => ({ ...row, scoreLabel: formatEdgeScore(row.score) }));
  const countries = countryRows.length
    ? countryRows
    : (sharedAlpha.edgeBoard?.countries || []).map((row) => ({ ...row, scoreLabel: formatEdgeScore(row.score) }));
  const stocks = stockRows.length
    ? stockRows
    : (sharedAlpha.edgeBoard?.stocks || []).map((row) => ({ ...row, scoreLabel: formatEdgeScore(row.score) }));
...
function buildPortfolioModule(snapshot, watchlistCount, sharedAlpha) {
  ...
  return {
    ...
    charts: {
      growthComparison: buildPortfolioTrend(portfolio.current_mix_vs_spy),
      sectorExposure: buildSectorExposure(portfolio.sector_weights),
      valuationDistribution: buildValuationDistribution(portfolio.valuation_histogram),
    },
  };
}
...
function filterScannerIdeas(rows, holdingTickers) {
  return rows.filter((row) => {
    ...
    const discovery = numberOr(row.discovery_score, numberOr(row.composite_score, null));

    if (source === "discovery" || source === "watchlist") return true;
...
function buildFundamentalConfirmation(rows) {
  return rows.slice(0, 4).map((row) => ({
    ticker: row.ticker,
    signals: [
      { id: "quality", label: "Quality", value: clamp01(numberOr(row.quality_score, 0)) },
      { id: "value", label: "Value", value: clamp01(numberOr(row.value_score, 0)) },
      { id: "growth", label: "Growth", value: clamp01(numberOr(row.growth_score, 0)) },
      { id: "discipline", label: "Risk fit", value: clamp01(numberOr(row.risk_score, 0)) },
    ],
  }));
}
...
function buildRiskModule(snapshot, sharedAlpha) {
  ...
  return {
    ...
    signalBars: buildRiskSignalBars(snapshot),
