...
const MODULE_META = [
  ["actions", "Decision Brief", "Decision"],
  ["command", "Decision Rules", "Rules"],
...
function getCanonicalContract(snapshot) {
  const contract = snapshot?.bls_state_v2 || snapshot?.bls_state_v1;
  if (!contract || typeof contract !== "object") return null;
  if (!contract.measured_state || !contract.probabilistic_state || !contract.policy_state) return null;
  return contract;
}
...
function getContractStatus(snapshot) {
  ...
  return getCanonicalContract(snapshot) ? "canonical" : "fallback_legacy";
}
...
function buildStressMode(dashboardSnapshot, modules) {
  ...
  const failure = contract?.failure_modes || {};
  const nextAction = Array.isArray(contract?.repair_candidates) ? contract.repair_candidates[0] : null;
  ...
  const authority = numberOr(probabilistic.authority_score || contract?.uncertainty?.authority_policy_gate, null);
  ...
  return {
    ...
    roomToAct: humanizeEngineLabel(budget.budget_state || "-"),
    healing: humanizeEngineLabel(healing.state || "-"),
    reboundDriver: humanizeEngineLabel(sponsorship.type || "mixed"),
    riskAdds: humanizeEngineLabel(legitimacy.risk_add_state || "closed"),
    defensiveMoves: humanizeEngineLabel(legitimacy.defensive_state || "closed"),
    mainRisk: humanizeEngineLabel(failure.dominant_failure_mode || "none_material"),
    authority: authority === null ? "-" : fmtPct(authority),
    cadence: policy.review_cadence || "legacy",
    confirmation: humanizeEngineLabel(String(policy.required_confirmation || "legacy")),
    topMove:
      nextAction && Array.isArray(nextAction.trade_set) && nextAction.trade_set.length
        ? nextAction.trade_set.join(" | ")
        : "No valid repair fits the current rules",
    invalidation: Array.isArray(policy.invalidation_rules) ? policy.invalidation_rules[0] : null,
  };
}
  ...
  return {
    add:
      cluster.dominant === "G-dominated"
        ? `Adds should be smaller and slower because fragility is structural, not just emotional. ${cautionText}`
        : reboundQuality.state === "Restorative"
          ? `A rebound with structural repair justifies staged adds into the best names. ${cautionText}`
          : `Adds are acceptable only in the cleanest names while the rebound remains unproven. ${cautionText}`,
    trim:
      cluster.dominant === "G-dominated"
        ? "Trims matter more because a structural cluster can stay broken longer than price memory expects."
    ...
    summary: `${cluster.dominant} cluster, rebound confidence ${reboundConfidence.state.toLowerCase()}, rebound quality ${reboundQuality.state.toLowerCase()}.`,
  };
}
...
function buildLiveActionItems(snapshot, sharedAlpha) {
  ...
  if (holdCandidate) {
    ...
    liveActions.push({
      ...
      funding: "No change",
      conviction: `${holdCandidate.ticker} is still part of the portfolio's protection layer while the rest of the book stays selective. ${overlay.summary}`,
      whyNow: `The current setup still rewards keeping some dry powder and stability in the book instead of forcing every dollar into aggressive ideas. ${riskBudget} ${overlay.hold}`,
...
function buildContractActionItems(snapshot, sharedAlpha) {
  ...
  return repairs.slice(0, 3).map((repair, index) => {
    ...
    return {
      ...
      funding: repair.funding_source || "Current book",
      conviction: `${repair.classification.replace(/_/g, " ")} with recoverability delta ${fmtPct(repair.delta_recoverability)}. ${framework.cluster.dominant} with rebound confidence ${framework.reboundConfidence.state.toLowerCase()}.`,
      ...
      invalidation: Array.isArray(repair.invalidation) ? repair.invalidation.join("; ") : String(repair.invalidation || "Policy state changes."),
...
function buildActionsModule(snapshot, sharedAlpha) {
  ...
  const failure = contract?.failure_modes || {};
  const policy = contract?.policy_state || {};
  const fallbackActions = buildLiveActionItems(snapshot, sharedAlpha);
  const emptyContractAction = contract && contractActions.length === 0
    ? [{
        id: "no-valid-repair",
        type: "hold",
        ticker: "No trade now",
        company: "Current rules do not allow a safe repair",
        size: fmtPct(numberOr(policy.max_gross_add, 0)),
        funding: "None",
        role: `${humanizeEngineLabel(policy.mode || "observe")} stance`,
        conviction: `Risk adds are ${humanizeEngineLabel(legitimacy.risk_add_state || "closed")} and defensive moves are ${humanizeEngineLabel(legitimacy.defensive_state || "limited")}.`,
        whyNow: `Main risk: ${humanizeEngineLabel(failure.dominant_failure_mode || "state still fragile")}.`,
        watchFor: `What needs to improve: ${humanizeEngineLabel(policy.required_confirmation || "state must improve")}.`,
        invalidation: Array.isArray(policy.invalidation_rules) ? policy.invalidation_rules.join("; ") : "State improves enough to reopen decisions.",
        sourceLabel: "Canonical contract",
      }]
    : [];
  const actions = (emptyContractAction.length ? emptyContractAction : contractActions.length ? contractActions : fallbackActions).map((action, index) => ({
    ...action,
    priority: index + 1,
    plainLabel:
  ...
  return {
    ...
    subtitle: contract
      ? `Current stance: ${humanizeEngineLabel(policy.mode || liveAction || "observe")}. Adding risk is ${humanizeEngineLabel(legitimacy.risk_add_state || "state dependent")}.`
      : liveAction
        ? `Current engine stance: ${humanizeEngineLabel(liveAction)}. ${framework.cluster.dominant} with rebound confidence ${framework.reboundConfidence.state.toLowerCase()} and rebound quality ${framework.reboundQuality.state.toLowerCase()}.`
...
function buildContractProtocolModule(snapshot, sharedAlpha) {
  ...
  const frontierDistance = numberOr(budget.remaining_budget, autonomyScore) - numberOr(probabilistic.p_phantom_rebound, 0);
  ...
  return {
    ...
    protocol: `${mode}_mode`,
    protocolLabel: `${trustState} now`,
    trustScore: fmtPct(trustScore),
    trustState,
    decisionRights:
      legitimacy.risk_add_state === "open" ? "Adding risk is allowed"
        : legitimacy.risk_add_state === "conditional" ? "Only staged adds are allowed"
          : "Do not add risk",
    autonomyScore: fmtPct(autonomyScore),
    frontierDistance: formatSignedPct(frontierDistance),
    recoverabilityBudget: humanizeEngineLabel(budget.budget_state || "narrowing"),
    supportDependency: [
      {
        id: "room_to_act",
        label: "Room to act",
        value: fmtPct(numberOr(budget.remaining_budget, 0)),
        numeric: clamp01(numberOr(budget.remaining_budget, 0)),
      },
      {
        id: "healing",
        label: "Healing trend",
        value: humanizeEngineLabel(healing.state || "flat"),
        numeric: clamp01(numberOr(healing.confidence, 0)),
      },
    ],
    protectiveValue: [
      {
        id: "rebound_driver",
        label: "What is driving the rebound",
        value: humanizeEngineLabel(sponsorship.type || "mixed"),
        numeric: clamp01(numberOr(sponsorship.confidence, 0)),
      },
      {
        id: "gross_add",
        label: "Risk add limit",
      ...
      },
      {
        ...
        value: humanizeEngineLabel(legitimacy.defensive_state || "closed"),
        numeric: clamp01(legitimacy.defensive_state === "open" ? 1.0 : legitimacy.defensive_state === "conditional" ? 0.55 : 0.1),
      },
    ],
    ...
    playbook,
    notes: [
      `Current stance: ${trustState.toLowerCase()}.`,
      `Main risk: ${humanizeEngineLabel(failure.dominant_failure_mode || "none_material")}.`,
      `What needs to improve: ${humanizeEngineLabel(String(policy.required_confirmation || "n/a"))}.`,
    ],
  };
}
...
function buildVolatilityClusterDecomposition(snapshot, sharedAlpha) {
  ...
  if (contract) {
    ...
    const stance =
      dominant === "G-dominated"
        ? "Structural fragility dominates. Repair recoverability first and do not assume a fast re-opening."
        : dominant === "R-dominated"
          ? "Shock pressure dominates. Waiting for a rebound is more defensible while the structure still holds."
          : dominant === "compound"
            ? "Structural fragility and shock pressure are both elevated. Respect both repair and protection."
            : "The state is mixed. Wait for cleaner separation between structural repair and shock relief.";
    return {
      dominant,
      gScore,
      rScore,
      ...
      stance,
      drivers: [
        `Structural dominance probability is ${fmtPct(gScore)}.`,
        `Regime-shock dominance probability is ${fmtPct(rScore)}.`,
        `Authority is ${fmtPct(probabilistic.authority_score)} for this classification horizon.`,
      ],
    };
  }
  ...
  const stance =
    dominant === "G-dominated"
      ? "Do not assume a fast mean reversion. Reduce fragility first."
      : dominant === "R-dominated"
  ...
  if (contract) {
    ...
    const note =
      state === "High"
        ...
        : state === "Conditional"
          ? "Visible relief is possible, but the book should wait for structural confirmation before widening action rights."
...
function buildChileSectorMap(rows) {
  return (Array.isArray(rows) ? rows : [])
    .slice(0, 6)
    .map((row) => ({
      id: row.sector,
      label: row.sector || "Sector",
      ratio: clamp01(numberOr(row.avg_score, 0)),
      valueLabel: fmtPct(numberOr(row.avg_return_3m, null)),
    }));
}

function buildChileOpportunityMap(rows) {
  return (Array.isArray(rows) ? rows : [])
    .slice(0, 8)
    .map((row) => ({
      ticker: row.ticker,
      x: numberOr(row.value_score, null),
      y: numberOr(row.momentum_score, null),
      size: clamp01(numberOr(row.opportunity_score, 0.2)),
    }))
    .filter((row) => row.x !== null && row.y !== null);
}

function buildChileModule(snapshot) {
  const chile = snapshot?.chile_market || {};
  const rows = chile.rows || chile.preferred || [];
  const preferred = (chile.preferred?.length ? chile.preferred : rows).slice(0, 8);
  const benchmark = chile.benchmark || {};
  const fx = chile.fx || {};
  ...
  return {
    ...
    kicker: "Chile",
    title: "Chile Desk",
    headline: chile.headline || "Chile market view is loading.",
    benchmarkLabel: benchmark.ticker || "^IPSA",
    benchmarkPrice: formatNumberLike(numberOr(benchmark.price, null)) || "-",
    benchmarkReturn1m: fmtPct(numberOr(benchmark.return_1m, null)),
    benchmarkReturn3m: fmtPct(numberOr(benchmark.return_3m, null)),
    fxLabel: fx.ticker || "USDCLP",
    fxPrice: formatNumberLike(numberOr(fx.price, null)) || "-",
    fxReturn1m: fmtPct(numberOr(fx.return_1m, null)),
    coverageCount: String(chile?.overview?.coverage_count || preferred.length || 0),
    xbrlCoverage: String(chile?.overview?.xbrl_coverage_count || 0),
    cmfCoverage: String(chile?.overview?.cmf_coverage_count || 0),
    cmfPeriod: chile?.overview?.cmf_period || "No CMF pull yet",
    rows: preferred.map((row) => ({
      ticker: row.ticker,
      sector: row.sector || "Unknown",
      score: formatNumberLike(numberOr(row.opportunity_score, null)) || "-",
      return3m: fmtPct(numberOr(row.return_3m, null)),
      quality: fmtPct(numberOr(row.quality_score, null)),
      independence: fmtPct(numberOr(row.independence_score, null)),
      cmf: row.xbrl_revenue ? "XBRL" : row.cmf_revenue ? "CMF TXT" : "Price-only",
      theme: row.theme || "Chile equity",
    })),
    charts: {
      sectorMap: buildChileSectorMap(chile.sector_map),
      opportunityMap: buildChileOpportunityMap(rows),
    },
    leaders: (chile.leaders || []).slice(0, 3).map((row) => ({
      ticker: row.ticker,
      detail: `${row.sector || "Sector"} · ${fmtPct(numberOr(row.return_1m, null))}`,
    })),
    laggards: (chile.laggards || []).slice(0, 3).map((row) => ({
      ticker: row.ticker,
      detail: `${row.sector || "Sector"} · ${fmtPct(numberOr(row.return_1m, null))}`,
    })),
    note:
      rows.length
        ? "Chile Desk is using free market data with a structural score built from quality, value, momentum, and independence versus IPSA."
        : "Chile Desk is mounted and waiting for its first successful market pull.",
  };
}
...
export function normalizeWorkspaceDashboard({
  ...
  const modules = {
    ...
    chile: buildChileModule(snapshot),
  ...
  return {
    ...
    stress_mode: buildStressMode(snapshot, modules),
    workspace_summary: {
      id: workspaceId,
    ...
    data_control: {
      ...
      notes: [
        ...
        "Chile Desk runs off a separate lightweight market pull, so it can stay useful even when the main US research stack is sparse.",
        String(contractStatus).includes("canonical")
          ? "Backend state is driving the decision and repair framework."
          : "Parts of the terminal are still using fallback logic.",
      ],
    },
    ...
    alpha_briefing: {
      ...
      pulse: alpha.pulse,
      frameworkSignal: {
        cluster: modules.risk.clusterDecomposition.dominant,
