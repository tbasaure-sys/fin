...
function mergeBenchmarkFallback(primaryRows, fallbackRows) {
  const primary = Array.isArray(primaryRows) ? primaryRows : [];
  const fallback = Array.isArray(fallbackRows) ? fallbackRows : [];

  if (!primary.length) return fallback;
  if (!fallback.length || hasBenchmarkSeriesData(primary)) return primary;

  return primary.map((row, index) => {
    const fallbackIndex = primary.length === 1
      ? 0
      : Math.round((index / Math.max(primary.length - 1, 1)) * Math.max(fallback.length - 1, 0));
    const fallbackBenchmark = numberOr(fallback[fallbackIndex]?.benchmark, null);
    return {
      ...row,
      benchmark: numberOr(row?.benchmark, fallbackBenchmark),
    };
  });
}
...
function buildAlerts(snapshot, sharedAlpha) {
  ...
  if (!warnings.length && snapshotAgeDays !== null && snapshotAgeDays > 1.5) {
    ...
    alerts.push({
      ...
      body: generatedAt
        ? `The decision engine is reachable, but the latest stored snapshot is from ${generatedAt}.`
        : "The decision engine is reachable, but the latest stored snapshot does not have a reliable timestamp.",
...
function buildPortfolioModule(snapshot, watchlistCount, sharedAlpha) {
  ...
  const fallbackGrowthComparison = buildFallbackPortfolioTrend(sharedAlpha);
  const hydratedGrowthComparison = mergeBenchmarkFallback(liveGrowthComparison, fallbackGrowthComparison);
  ...
  const computedAnalytics = buildPerformanceAnalyticsFromSeries(hydratedGrowthComparison);
  ...
  return {
    ...
    charts: {
      growthComparison: hydratedGrowthComparison,
      sectorExposure,
      valuationDistribution,
    },
    chartSource:
      hydratedGrowthComparison.length >= 5 || sectorExposure.length || valuationDistribution.length
        ? "Live portfolio history and holdings snapshot"
        : holdingsSource.connected
          ? "Live holdings. History still limited."
...
function deriveEscrowStatus(item, actionLookup, blockedActionId) {
...
function buildWorkspaceEscrow(escrowDecisions, actions, blockedAction) {
  ...
  const items = (Array.isArray(escrowDecisions) ? escrowDecisions : [])
    .map((item) => {
      const derived = deriveEscrowStatus(item, actionLookup, blockedActionId);
...
function buildDecisionWorkspace({
  ...
  const escrow = buildWorkspaceEscrow(escrowDecisions, [primaryAction, ...secondaryActions], blockedAction);
...
function describeMandateOption(id) {
  if (id === "compound_without_fake_rebounds") {
    return {
      ...
      guardrails: [
        "Prefer ballast plus quality over beta spikes.",
        "Only widen risk when sponsorship and recoverability both improve.",
        "Treat phantom rebounds as traps, not invitations.",
      ],
      rankingBias: "Penalize fragile cyclicals and reward durable compounders plus ballast.",
  ...
  if (id === "defend_drawdown") {
    return {
      ...
      guardrails: [
        "Keep ballast visible in the top of the book.",
        "Trim positions that carry too much fragility for their reward.",
        "Make new adds prove themselves before they earn size.",
      ],
      rankingBias: "Favor stability sleeves, ballast, and lower-fragility repairs.",
...
function buildRecoverabilityMap(snapshot, xray, frontier) {
  ...
  return {
    headline: "Recoverability map",
    subhead: "Read holdings, watch ideas, and blocked temptations by recoverability versus phantom rebound risk.",
    filters: [
...
export function normalizeWorkspaceDashboard({
  ...
  const snapshotAgeDays = getSnapshotAgeDays(snapshot);
  const backendStatus = snapshot?.status?.warnings?.length
    ? "briefing"
    ...
    data_control: {
      analysisSource:
        backendStatus === "briefing"
          ? "Live backend, partial analysis"
          : backendStatus === "stale"
            ? "Live backend reachable, but the stored snapshot is stale"
            : "Live backend",
      contractStatus,
