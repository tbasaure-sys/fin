...
function buildStressTopMove(contract) {
  ...
  if (repair) {
    return {
      ...
      firstInvalidation: Array.isArray(repair.invalidation) ? repair.invalidation[0] : null,
    };
  }
  if (contract) {
...
function buildContractActionItems(snapshot, sharedAlpha) {
  ...
  if (!repairs.length) {
    ...
    const invalidation = Array.isArray(policy.invalidation_rules) && policy.invalidation_rules.length
      ? policy.invalidation_rules.join("; ")
    ...
    return [{
      ...
      whyNow: "Recoverability is too thin, phantom risk is still binding, or current sleeve permissions do not allow a repair that improves the book enough.",
...
function buildActionsModule(snapshot, sharedAlpha) {
  ...
  return {
    ...
    subtitle: blocked
      ? `Current engine stance: no valid repair under the canonical frontier. ${framework.cluster.dominant} with rebound confidence ${framework.reboundConfidence.state.toLowerCase()} and rebound quality ${framework.reboundQuality.state.toLowerCase()}.`
      : liveAction
...
export function normalizeWorkspaceDashboard({
  ...
  return {
    ...
    workspace_summary: {
      ...
      market_data_label: formatUpdatedAt(snapshot?.portfolio?.quotes_as_of || snapshot?.as_of_date),
      ...
      marketData: {
        ...
        freshnessLabel:
          snapshot?.portfolio?.quotes_stale_days === null
            ? "Price freshness unavailable"
            : snapshot.portfolio.quotes_stale_days === 0
              ? "Prices look current"
              : `Prices are ${snapshot.portfolio.quotes_stale_days} day(s) behind`,
      },
      notes: [
        ...
        "Stock ideas turn fully live only after Railway promotes a fresh discovery screener artifact.",
        contractStatus === "canonical_valid"
          ? "Canonical backend state is driving the decision engine."
          : "Terminal is using a fallback path for part of the decision engine.",
      ],
    },
