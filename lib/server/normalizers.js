...
function buildEdgeBoard(snapshot, sharedAlpha) {
  ...
  const stockRows = buildEdgeRows(stockSource, {
    ...
    noteBuilder: (row) => {
      ...
      if (gap !== null) parts.push(`quantitative filter ${fmtPct(gap)}`);
      return parts.length
        ? `Highest-ranked single-name research candidate. Ranked by ${parts.join(" and ")}; this screening signal prioritizes investigation and is not a traced valuation.`
        : "Highest-ranked single-name research candidate; investigate before acting.";
    },
    detailBuilder: (row, score) => ({
      ...
      support: [
        ...
        numberOr(row.valuation_gap, null) !== null
          ? `Quantitative filter signal: ${fmtPct(row.valuation_gap)} (not a traced valuation)`
          : "Quantitative filter signal still loading",
...
export function buildPortfolioTrend(rows) {
...
export function buildPerformanceAnalyticsFromSeries(series) {
  ...
  const incomplete = rows.some(row => row.performanceStatus && !['baseline','calculated','awaiting_capital'].includes(row.performanceStatus));
  if (portfolioPoints.length < 2 || incomplete) {
    return {
      annualReturn: null,
  ...
  if (addCandidate) {
    ...
    liveActions.push({
      ...
      watchFor: numberOr(addCandidate.valuation_gap, null) !== null
        ? `El indicador cuantitativo del filtro marca ${fmtPct(addCandidate.valuation_gap)}; úsalo para priorizar investigación, no como estimación fundamental validada.`
  ...
  if (trimCandidate) {
    ...
    liveActions.push({
      ...
      whyNow: numberOr(trimCandidate.valuation_gap, null) !== null
        ? `${trimCandidate.ticker} muestra una señal cuantitativa de brecha de ${fmtPct(trimCandidate.valuation_gap)}; no equivale a una valuación trazable. ${overlay.trim}`
...
function buildPortfolioModule(snapshot, watchlistCount, sharedAlpha) {
  ...
  const normalizeHolding = (row) => {
    ...
    return {
      ...
      industry: readText(row, ["industry", "subsector"]),
      ...
      valueSource: readText(row, ["value_source", "valueSource", "source"], currentPriceUsd !== null ? "stored_price" : brokerValueUsd !== null ? "broker_snapshot" : null),
  ...
  return {
    ...
    analytics: {
      ...
      timeWeightedReturn: numberOr(analytics["Time Weighted Return"], null),
      timeWeightedReturnLabel: numberOr(analytics["Time Weighted Return"], null) !== null
        ? formatSignedPct(numberOr(analytics["Time Weighted Return"], null))
        : null,
    ...
    chartSource:
      liveGrowthComparison.length >= 5
        ? (String(analytics["Performance Method"] || liveGrowthComparison[0]?.performanceMethod || "") === "current_weight_backcast"
            ? "Backcast con precios reales"
