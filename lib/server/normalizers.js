...
function describeHoldingsSource(snapshot) {
  ...
  if (source === "local_overlay") {
    return {
      source,
      label: label || "Private holdings file",
      connected: true,
      detail: "The UI is reading your private holdings file from the local overlay.",
    };
  }
  ...
  if (source) {
    return {
      ...
      detail: source === "backend_portfolio_manager"
        ...
        : "The UI is using the shared snapshot path.",
    };
  }

  return {
    source: hasHoldings ? "shared_snapshot" : "shared_snapshot",
    label: hasHoldings ? "Shared snapshot holdings" : "Shared snapshot",
    connected: false,
    detail: hasHoldings
      ? "The UI is showing holdings from the shared snapshot because no local overlay is attached."
      : "No private holdings file is attached, so the UI is using the shared snapshot.",
  };
}
...
function buildInternationalModule(snapshot, sharedAlpha) {
  ...
  return {
    ...
    note: rows.length
      ...
      : "International sleeves are seeded from the shared alpha portfolio until live rankings are promoted.",
  };
}
    ...
    data_control: {
      ...
      notes: [
        ...
        modules.portfolio.holdingsSource?.connected
          ...
          : "Private holdings overlay is not connected; the UI is using the shared snapshot path.",
