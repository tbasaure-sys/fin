...
function humanizeEngineLabel(value) {
  ...
  if (betaMatch) {
    ...
    if (beta <= 0.45) return "Take moderate risk";
    if (beta <= 0.75) return "Add risk selectively";
    return "Keep risk elevated";
  }
  ...
  if (value === "stage") return "Add risk selectively";
  if (value === "act") return "Risk can be added";
...
function buildAlerts(snapshot, sharedAlpha) {
  ...
  if (!warnings.length && snapshotAgeDays !== null && snapshotAgeDays > 1.5) {
    const generatedAt = snapshot?.generated_at || snapshot?.portfolio?.quotes_as_of || snapshot?.as_of_date;
    alerts.push({
      id: "stale-snapshot",
      severity: "medium",
      title: "Desk note: snapshot is stale",
      body: generatedAt
        ? `Live analysis is reachable, but the latest stored snapshot is from ${generatedAt}.`
        : "Live analysis is reachable, but the latest stored snapshot does not have a reliable timestamp.",
      action: "Run refresh and verify Railway outputs",
      source: "backend",
    });
  }
...
function buildActionsModule(snapshot, sharedAlpha) {
  ...
  return {
    ...
    subtitle: blocked
      ...
      : liveAction
      ? `Suggested position: ${humanizeEngineLabel(liveAction)}.`
...
function makeAdviceTitle(action, snapshot, stressMode) {
  ...
  if ((action.ticker && ["SGOV", "SHY", "BIL", "TLT"].includes(action.ticker)) || action.ticker === defensiveTicker) {
    return `Keep ${action.ticker} as defense`;
  }
...
function buildBackupAdvice(snapshot) {
  ...
  return {
    ...
    type: "hold",
    ticker: defensiveTicker,
    adviceTitle: `Keep ${defensiveTicker} as defense`,
    size: "Keep current size",
    sizeValue: 0,
    funding: "No change",
...
export function normalizeWorkspaceDashboard({
  ...
  return {
    ...
    data_control: {
      ...
      notes: [
        "Refresh rebuilds the analysis behind this workspace.",
        "Prices show the latest market date currently loaded into the workspace.",
        "New stock ideas only appear after a fresh discovery run is available.",
        ...
        contractStatus === "canonical_valid"
          ...
          : "Some live inputs are unavailable, so the app is showing limited output instead of placeholders.",
      ],
    },
