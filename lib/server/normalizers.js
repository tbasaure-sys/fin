...
function buildStressMode(dashboardSnapshot, modules) {
  ...
  const nextAction = modules?.actions?.actions?.[0] || null;
  ...
  const authority = numberOr(probabilistic.authority_score, null);
  ...
  return {
    ...
    topMove: nextAction ? `${nextAction.plainLabel || nextAction.type} ${nextAction.ticker}` : "No repair candidate yet",
...
function buildActionsModule(snapshot, sharedAlpha) {
  ...
  const actions = (contractActions.length ? contractActions : buildLiveActionItems(snapshot, sharedAlpha)).map((action, index) => ({
    ...action,
    priority: index + 1,
    plainLabel:
