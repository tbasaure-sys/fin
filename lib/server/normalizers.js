...
function buildPortfolioModule(snapshot, watchlistCount, sharedAlpha) {
  ...
  const hasPerformanceHistory = computedAnalytics.sessionCount >= 5 && computedAnalytics.totalReturn !== null;
  ...
  const annualReturnValue = analyticsSource === "holdings_proxy"
    ? computedAnalytics.annualReturn
  ...
  const sharpeRatioValue = analyticsSource === "holdings_proxy"
    ? numberOr(computedAnalytics.sharpeRatio, analytics["Sharpe Ratio"])
  ...
  const annualReturnLabel = annualReturnValue !== null
    ...
    : unrealizedReturnValue !== null
      ? `${formatSignedPct(unrealizedReturnValue)} current gain`
      : "Not enough history";
  ...
  const sharpeRatioLabel = sharpeRatioValue !== null
    ...
    : holdingsSource.connected
      ? "History needed"
      : "Not enough history";
  const totalReturnValue = hasPerformanceHistory ? computedAnalytics.totalReturn : unrealizedReturnValue;
  const totalReturnLabel = totalReturnValue !== null
    ? `${formatSignedPct(totalReturnValue)}${hasPerformanceHistory ? "" : " current gain"}`
    : "Not enough history";
  ...
  const maxDrawdownLabel = computedAnalytics.maxDrawdown !== null
    ? fmtPct(computedAnalytics.maxDrawdown)
    : holdingsSource.connected
      ? "History needed"
      : "Not enough history";
  ...
  return {
    ...
    chartSource:
      liveGrowthComparison.length || sectorExposure.length || valuationDistribution.length
        ? "Live portfolio history and holdings snapshot"
        : holdingsSource.connected
          ? "Private holdings are live. Personal performance history is not stored yet."
