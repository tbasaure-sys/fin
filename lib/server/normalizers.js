...
function humanizeContractStatus(value) {
  if (!value) return "Backup mode";
  if (value === "canonical_valid") return "Live analysis";
  if (value === "canonical_invalid_warn") return "Limited analysis";
  if (value === "fallback_cached_valid") return "Cached analysis";
  if (value === "fallback_legacy") return "Backup mode";
...
function buildPortfolioTrend(rows) {
  return (Array.isArray(rows) ? rows : [])
    .map((row, index) => ({
      ...
      portfolio: numberOr(row?.portfolio_growth, null),
      benchmark: numberOr(row?.spy_growth, numberOr(row?.benchmark_growth, null)),
    }))
...
function sumHoldingsMarketValue(rows) {
  const total = (Array.isArray(rows) ? rows : []).reduce(
    (sum, row) => sum + Math.max(0, numberOr(row?.market_value_usd, 0)),
    0,
  );
...
function formatMetricLabel(value, formatter, fallback = "History limited") {
...
function buildHoldingReturnBreakdown(rows) {
  const tracked = (Array.isArray(rows) ? rows : [])
    .map((row) => {
      ...
      const marketValue = numberOr(row?.market_value_usd, null);
...
function describeHoldingsSource(snapshot) {
  ...
  if (source === "neon_portfolio" || source === "neon_workspace") {
    return {
      source,
      label: label || "Private workspace",
      connected: true,
      detail: "Your holdings are saved in your private workspace and loaded from Neon.",
    };
  }
  ...
  if (source === "local_overlay" || source === "remote_overlay" || source === "ui_editable_overlay") {
    ...
    return {
      ...
      connected: true,
      detail: isRemote
        ? "Private holdings are loading from Neon."
        : "Private holdings are loading from the local overlay.",
    };
  }
  ...
  if (addCandidate) {
    ...
    liveActions.push({
      ...
      watchFor: numberOr(addCandidate.valuation_gap, null) !== null
        ? `Wait for a cleaner entry if the price remains ${fmtPct(addCandidate.valuation_gap)} away from fair value.`
...
function buildContractActionItems(snapshot, sharedAlpha) {
  ...
  if (!repairs.length) {
    ...
    return [{
      ...
      type: "hold",
      ticker: "Wait",
      company: "Do not add more risk yet",
      size: "No new buys",
      sizeValue: 0,
      funding: "Keep cash ready",
      conviction: "The market still does not support a new risk add.",
      whyNow: "Recovery is still weak, recent strength may not hold, or the current rules still block a better move.",
      watchFor: Array.isArray(policy.forbidden_sleeves) && policy.forbidden_sleeves.length
        ? `Still blocked for: ${policy.forbidden_sleeves.join(", ")}.`
        : `Wait for ${confirmation}.`,
      role: policy.mode ? `${policy.mode} mode` : "Protect",
      invalidation,
      sourceLabel: "Live rules",
      isBlockedState: true,
    }];
  }
  ...
  return repairs.slice(0, 3).map((repair, index) => {
    ...
    return {
      ...
      conviction: `This move improves resilience by ${fmtPct(repair.delta_recoverability)}. Market tone: ${(framework.cluster.dominantLabel || humanizeClusterLabel(framework.cluster.dominant)).toLowerCase()}.`,
      whyNow: tradeSet.length ? tradeSet.join(" | ") : "No trade set provided.",
      watchFor: (repair.binding_constraints || []).length
        ? `What could block it: ${(repair.binding_constraints || []).join(", ")}.`
        : "Wait for the risk picture to keep improving.",
...
function buildAlerts(snapshot, sharedAlpha) {
  ...
  const userWarnings = warnings.filter((warning) => !/runtime bootstrap|market:|alpha_volume_panel|fred request failed|internal server error|pipeline|traceback|exception|stack trace|\/api\/|railway|backend snapshot/i.test(String(warning || "")));
  ...
  if (freshnessIssue) {
    ...
    alerts.push({
      ...
      title: snapshotAgeDays !== null && snapshotAgeDays > 1.5 ? "Market session needs refresh" : "Live refresh is still catching up",
      body: generatedAt
        ? `The latest completed market session loaded here is from ${generatedAt}.`
        : "The workspace is using the latest completed session while live refresh catches up.",
  ...
  if (crashProb !== null && crashProb >= 0.65) {
    alerts.unshift({
      id: "tail-risk",
      severity: "high",
      title: "Structural risk is elevated",
      body: `The current market read still looks fragile (${fmtPct(crashProb)} stress risk). Keep sizing selective until the structure improves.`,
      action: "Keep sizing selective",
      source: "policy",
    });
  }
...
function buildAdviceHeadline(primary, secondary, stressMode) {
  ...
  if (primary?.type === "trim") {
    return secondary?.type === "hold"
      ? "Trim weaker risk, keep defense in place, and wait for cleaner adds."
...
function buildAdviceThresholds(stressMode) {
  ...
  return [
    ...
    },
    {
      ...
      meaning: "Protect first. Trims and hedges matter more than new risk.",
...
function buildAdviceFiberLine(action, stressMode) {
  ...
  return ambiguity === "High"
    ? "High ambiguity argues for keeping protection in place until the state becomes clearer."
...
function buildCautionAdvice(snapshot, modules, stressMode) {
  ...
  return {
    ...
    whyNow: `Do not cut protection too early while the book is still carrying enough special-situation and growth risk to need a defensive sleeve.`,
    watchFor: "Only reduce protection when recovery chance improves and the bounce remains real for long enough.",
    invalidation: "If protection stops helping during weak tape, reassess the defensive sleeve first.",
    sourceLabel: "Live analysis",
  };
}
...
function buildBackupAdvice(snapshot) {
  ...
  return {
    ...
    whyNow: "This cash should stay boring: it exists to support better portfolio decisions, not to become the headline.",
    watchFor: "Only cut this defensive sleeve when recovery chance improves and the bounce remains real long enough to trust.",
    invalidation: "If protection stops helping during weak tape, reassess the defensive sleeve.",
    sourceLabel: "Live analysis",
  };
}
...
function normalizeDecisionPacket(packet, snapshot, stressMode, sharedAlpha) {
  ...
  return {
    ...
    moves: moves.map((move, index) => ({
      ...
      sourceLabel: move.sourceLabel || "Live analysis",
...
function buildJustAdviceModule(snapshot, modules, stressMode, sharedAlpha) {
  ...
  return {
    ...
    currentRead: [
      ...
      { label: "Weak rebound risk", value: stressMode?.phantom || "-", detail: "Higher means green days are easier to fake." },
      { label: "Signal strength", value: stressMode?.authorityLabel || "-", detail: stressMode?.fiberAtlas?.headline || "Comparable-state history unavailable." },
    ],
    moves,
    thresholds,
...
function buildPortfolioModule(snapshot, watchlistCount, sharedAlpha) {
  ...
  const maxDrawdownLabel = computedAnalytics.maxDrawdown !== null
    ? (hasPerformanceHistory ? fmtPct(computedAnalytics.maxDrawdown) : "History limited")
    : holdingsSource.connected
      ? "History limited"
      : "History limited";
  ...
  const normalizeHolding = (row) => {
    const marketValueUsd = readNumber(row, ["market_value_usd", "marketValueUsd", "broker_value_usd", "brokerValueUsd", "analysis_value", "analysisValue"]);
    ...
    const unrealizedPnlUsd = readNumber(row, ["unrealized_pnl_usd", "unrealizedPnlUsd", "broker_total_gain_usd", "brokerTotalGainUsd"]);
    ...
    return {
      ...
      avgCostUsd,
      costBasisUsd,
      dividendsReceivedUsd: readNumber(row, ["dividends_received_usd", "dividendsReceivedUsd", "dividends_received", "dividends"]),
      marketValueUsd,
      brokerValueUsd: readNumber(row, ["broker_value_usd", "brokerValueUsd"], marketValueUsd),
      currentPriceUsd: readNumber(row, ["current_price_usd", "currentPriceUsd", "live_price", "livePrice"]),
      unrealizedPnlUsd,
      unrealizedReturn,
      ...
      valueSource: readText(row, ["value_source", "valueSource", "source"]),
  ...
  return {
    ...
    holdings: rows.map(normalizeHolding),
    ...
    analytics: {
      ...
      hasPerformanceHistory,
      hasBenchmarkHistory,
      activeCostBasisUsd: numberOr(analytics["Active cost basis"], numberOr(analytics.active_cost_basis_usd, null)),
      realizedPnlUsd: numberOr(analytics["Realized P&L"], numberOr(analytics.realized_pnl_usd, null)),
      dividendsUsd: numberOr(analytics.Dividends, numberOr(analytics.dividends_usd, null)),
      totalPnlInclRealizedDividendsUsd: numberOr(analytics["Total P&L incl. realized/dividends"], numberOr(analytics.total_pnl_incl_realized_dividends_usd, null)),
      totalReturnInclDividends: numberOr(analytics["Total return incl. dividends"], numberOr(analytics.total_return_incl_dividends, null)),
    },
    charts: {
    ...
    chartSource:
      liveGrowthComparison.length >= 5
        ...
        : holdingsSource.connected
          ? "Holdings connected. History is still building."
          : "No live portfolio history",
    holdingsSource,
    holdingsSync,
  };
}
...
function buildRiskModule(snapshot, sharedAlpha) {
  ...
  const liveMetrics = [
    ...
    { label: "Instability", value: fmtPct(snapshot?.overview?.legitimacy_risk) },
...
function normalizeWorkspaceAction(action, status = "allowed") {
  ...
  return {
    id: action.id || null,
    slot: action.slot || (status === "blocked" ? "Not yet" : "Advice"),
    status,
    title: action.title || "Advice",
    ...
    funding: action.funding || "No change",
    sizeLabel: action.size || "Staged",
    sizeValue: actionSizeValue(action),
    sourceLabel: action.sourceLabel || "Live analysis",
...
function responseLabel(value) {
  ...
  if (normalized === "executed_auto") return "Executed automatically";
...
function buildWorkspaceStateSummary(snapshot, modules, justAdvice, stressMode) {
  ...
  return {
    ...
    stance: humanizeEngineLabel(snapshot?.overview?.recommended_action) || "Wait for a cleaner signal",
  ...
  const items = (Array.isArray(escrowDecisions) ? escrowDecisions : [])
    .map((item) => {
      ...
      return {
        ...
        title: item.title || sourcePayload.title || "Staged decision",
        summary: item.summary || sourcePayload.summary || "",
        ticker: item.ticker || sourcePayload.ticker || null,
        slot: item.slot || sourcePayload.slot || "Also valid",
        tone: item.tone || sourcePayload.tone || "neutral",
        funding: item.funding || sourcePayload.funding || "No change",
        sizeLabel: item.sizeLabel || sourcePayload.sizeLabel || sourcePayload.size || "Staged",
...
function buildWorkspaceMemory(decisionEvents, packetMemory, stressMode) {
  ...
  let behaviorNote = "The system is still learning your response pattern.";
  if ((counts.deferred || 0) >= 2 && (counts.deferred || 0) > (counts.executed || 0)) {
    behaviorNote = "You are deferring more often than executing. Keep that discipline only when ambiguity remains high.";
  } else if ((counts.executed || 0) >= 2) {
    behaviorNote = "You are acting on surfaced decisions. Keep sizing small when the state weakens.";
  } else if ((counts.cancelled || 0) >= 2) {
    behaviorNote = "You are staging then cancelling often. Tighten your maturity conditions.";
  }

  const weeklyBrief = [
    ...(Array.isArray(packetMemory?.narrative) ? packetMemory.narrative.slice(0, 2) : []),
    behaviorNote,
    packetMemory?.penaltyReason ? `Calibration note: ${packetMemory.penaltyReason}` : null,
    stressMode?.whatNeedsToImprove ? `To reopen risk, watch for: ${stressMode.whatNeedsToImprove}.` : null,
...
function buildDecisionWorkspace({
  ...
  const blockedSource = moves.find((move) => String(move?.slot || "").toLowerCase().includes("not yet"))
    || (stressMode?.repairState === "frontier_blocked"
      ? {
        ...
        slot: "Not yet",
        tone: "hold",
        title: "Do not widen risk yet",
        summary: stressMode?.decisionSummary || "The market still does not support adding more risk.",
        why: stressMode?.decisionSummary || "The market still does not support adding more risk.",
        watchFor: stressMode?.topMove?.firstConstraint || null,
        trigger: stressMode?.whatNeedsToImprove || stressMode?.changeTrigger || null,
        funding: stressMode?.topMove?.funding || "Keep cash ready",
        size: "Wait",
        sourceLabel: "Live analysis",
        effects: [],
      }
  ...
  return {
    ...
    evidenceDrawer: {
      headline: justAdvice?.headline || "Keep the portfolio recoverable before widening risk",
      ...
      fiberTakeaway: justAdvice?.fiberTakeaway || stressMode?.fiberAtlas?.takeaway || "Comparable-state read unavailable.",
      decisionSummary: stressMode?.decisionSummary || "",
      topAnalog: stressMode?.topAnalog || "No analogs yet",
...
function describeMandateOption(id) {
  if (id === "compound_without_fake_rebounds") {
    return {
      ...
      rankingBias: "Penalize fragile cyclicals and reward durable compounders plus defense.",
      stagingBias: "Stage only when the legitimacy surface is improving.",
    };
  }
  ...
  if (id === "defend_drawdown") {
    return {
      ...
      statement: "Protect the book first, then add risk only after the downside is clearly contained.",
      guardrails: [
        "Keep protection visible in the top of the book.",
        "Trim positions that carry too much fragility for their reward.",
        "Make new adds prove themselves before they earn size.",
      ],
      rankingBias: "Favor stability sleeves, defense, and lower-fragility repairs.",
      stagingBias: "Use escrow to delay risk until downside pressure clearly eases.",
    };
  }
  ...
  if (id === "stage_only_on_recoverability") {
    return {
      ...
      statement: "Treat staged decisions as the default until recoverability is high enough to justify execution.",
      guardrails: [
        "Do not widen risk just because price bounced.",
        "Reward cleaner repair paths over exciting rebounds.",
        "Keep blocked actions visible so temptation stays explicit.",
      ],
      rankingBias: "Promote staged repairs and demote actions that still fail legitimacy.",
      stagingBias: "Escrow is the default until the frontier reopens.",
    };
  }
...
function inferHoldingRole(holding) {
  ...
  if ([
    ...
    return {
      id: "ballast",
      label: "Defense",
      description: "Keeps room to act while the frontier stays tight.",
      fragility: 0.12,
      recovery: 0.2,
      sponsorship: 0.3,
    };
  ...
  if (sector.includes("health") || sector.includes("consumer staples") || sector.includes("utilities")) {
    return {
      id: "stability",
      label: "Stability",
      description: "Carries the book through pressure without heroic market conditions.",
      fragility: 0.28,
      recovery: 0.38,
      sponsorship: 0.46,
    };
  ...
  if (sector.includes("technology") || sector.includes("communication")) {
    return {
      id: "compounder",
      label: "Compounder",
      description: "Earns upside if the structure heals rather than merely bounces.",
      fragility: 0.46,
      recovery: 0.74,
      sponsorship: 0.68,
    };
  ...
  if (sector.includes("industrial") || sector.includes("financial") || sector.includes("energy") || sector.includes("materials")) {
    return {
      id: "cyclical",
      label: "Cyclical",
      description: "Adds torque only if sponsorship and recoverability both improve.",
      fragility: 0.64,
      recovery: 0.67,
      sponsorship: 0.58,
    };
  ...
  if (sector.includes("real estate") || sector.includes("biotech") || sector.includes("consumer discretionary") || sector.includes("unknown")) {
    return {
      id: "fragile",
      label: "Fragile",
      description: "Works only if the rebound is real; otherwise it reloads fragility.",
      fragility: 0.78,
      recovery: 0.52,
      sponsorship: 0.42,
    };
  ...
  if (weightValue >= 0.08) {
    return {
      id: "core",
      label: "Core",
      description: "One of the positions materially carrying the current book.",
      fragility: 0.42,
      recovery: 0.58,
      sponsorship: 0.55,
    };
  ...
  return {
    id: "satellite",
    label: "Satellite",
    description: "A smaller sleeve that should earn its place through diversification or asymmetry.",
    fragility: 0.52,
    recovery: 0.5,
    sponsorship: 0.48,
  };
...
function buildPortfolioXRay(portfolioModule, stressMode, mandate) {
  ...
  const concentrationWarnings = [
    ...
    ballastShare <= 0.08 && String(stressMode?.mode || "").toLowerCase().includes("protect")
      ...
      : null,
    fragileShare >= 0.32 ? `Fragile sleeves account for ${fmtPct(fragileShare)} of the book. That is high for a cautious mandate.` : null,
  ].filter(Boolean);

  return {
    headline: "What is actually carrying the book right now.",
    subhead: mandate?.statement || "Read the portfolio by role, concentration, fragility, and recovery contribution.",
    ...
    concentration: {
      ...
      verdict:
        topFiveShare >= 0.58 ? "Concentrated"
          : topFiveShare >= 0.42 ? "Focused"
            : "Balanced",
    },
    roleBands,
    ...
    carryingNarrative:
      weightedHoldings.length
        ? `${weightedHoldings[0].ticker} is the largest visible carrier, while ${roleBands[0]?.label || "the top role"} is the biggest sleeve in the book.`
        : "No private holdings are connected yet.",
...
function decorateFrontierAction(action, lane, stressMode, mandate) {
  if (!action) return null;
  const laneLabel = lane === "unlocked" ? "Unlocked" : lane === "staged" ? "Staged" : "Illegitimate";
  const disproof = lane === "illegitimate"
    ? stressMode?.changeTrigger || stressMode?.whatNeedsToImprove || "Wait for a cleaner legitimacy signal."
    : stressMode?.invalidation || stressMode?.mainRisk || "Invalidate if the market structure weakens again.";
  const whyLane = lane === "unlocked"
    ? "This sits inside the current decision rights and can be acted on carefully."
    : lane === "staged"
      ? "The option is still open, but it needs live state confirmation."
      : "This is tempting, but the current legitimacy surface still blocks it.";
  ...
  return {
    ...
    mandateFit: mandate?.label || "Decision mandate",
...
function buildActionFrontier(decisionWorkspace, stressMode, mandate) {
  ...
  const staged = (Array.isArray(decisionWorkspace?.escrow?.items) ? decisionWorkspace.escrow.items : [])
    ...
    .map((item) => ({
      ...
      summary: item.summary || "A staged decision waiting for confirmation.",
      sizeLabel: item.sizeLabel || "Staged",
      sizeValue: item.sizeValue ?? null,
      funding: item.funding || "No change",
      evidenceBand: stressMode?.authorityLabel || "Usable",
      evidenceTier: stressMode?.evidenceTier || "Live",
      whyLane: "This action is staged, but the system still wants more confirmation before it is clearly legitimate.",
      disproofCondition:
        (Array.isArray(item.invalidationConditions) ? item.invalidationConditions[0] : null)
        || stressMode?.changeTrigger
        || "Cancel it if the market structure weakens.",
  ...
  return {
    ...
    laneSummary: [
      { id: "unlocked", label: "Unlocked", count: unlocked.length, description: "Currently legitimate" },
      { id: "staged", label: "Staged", count: staged.length, description: "Held in advisory escrow" },
      { id: "illegitimate", label: "Illegitimate", count: illegitimate.length, description: "Blocked by current state" },
    ],
    nextUnlockCondition: stressMode?.whatNeedsToImprove || decisionWorkspace?.reopenTrigger || "Wait for stronger sponsorship and recoverability.",
    closeCondition: decisionWorkspace?.closeTrigger || stressMode?.mainRisk || "If the state weakens again, cut risk back first.",
    mandateBias: mandate?.rankingBias || null,
    lanes: [
      { id: "unlocked", label: "Unlocked", items: unlocked },
      { id: "staged", label: "Staged", items: staged },
      { id: "illegitimate", label: "Illegitimate", items: illegitimate },
    ],
...
function buildPositionStories(xray, frontier, stressMode, snapshot, persistedStories) {
  ...
  const items = (Array.isArray(xray?.weightedHoldings) ? xray.weightedHoldings : [])
    ...
    .map((holding) => {
      ...
      const roleLine = `${holding.roleLabel} sleeve`;
      const whyExists = persisted?.whyExists || persisted?.why_exists || [
        `${ticker} currently acts as a ${roleLine.toLowerCase()} in the book.`,
        holding.roleId === "ballast"
          ? "It earns its place by keeping optionality alive while the frontier is still narrow."
          : holding.roleId === "compounder"
            ? "It is one of the cleaner ways for the portfolio to participate if recoverability improves."
            : "It belongs only if it earns more recovery contribution than fragility.",
      ];
      const whatBreaks = persisted?.whatBreaks || persisted?.what_breaks || [
        stressMode?.mainRisk ? `If ${String(stressMode.mainRisk).replace(/_/g, " ")} keeps rising, this sleeve loses its edge.` : "If the risk structure weakens again, this sleeve loses its edge.",
        holding.fragilityScore >= 0.65
          ? "It becomes harder to justify if sponsorship fades before recoverability improves."
          : "It stops helping if it contributes fragility without adding enough recovery.",
      ];
      const whatCouldReplace = persisted?.whatCouldReplace || persisted?.what_could_replace || replacementTickers
        ...
        .slice(0, 3)
        .map((candidate) => `${candidate} if you need a cleaner expression of the same risk budget.`);
      const improvesConfidence = persisted?.improvesConfidence || persisted?.improves_confidence || [
        stressMode?.whatNeedsToImprove || "A stronger recoverability read.",
        frontierItem?.watchFor || "Clearer sponsorship from the live state.",
      ].filter(Boolean);
      ...
      return {
        ...
        whatCouldReplace,
        improvesConfidence,
        disproofCondition: frontierItem?.disproofCondition || stressMode?.changeTrigger || "If the legitimacy surface weakens again.",
        evidenceBand: frontierItem?.evidenceBand || stressMode?.authorityLabel || "Usable",
        laneHint: frontierItem?.laneLabel || "Held",
      };
    });

  return {
    headline: "Position stories",
    subhead: "Every major holding should explain itself, tell you what would break it, and suggest what could replace it.",
...
function buildCounterfactualLedger(decisionEvents, counterfactualOutcomes) {
  ...
  const items = events.map((event) => {
    ...
    const verdict =
      excessDelta === null
        ? "Still settling"
        : excessDelta > 0.02
          ? "Decision added value"
          : excessDelta < -0.02
            ? "Decision lagged"
            : "Decision tracked the market";
    const lesson =
      responseKey === "executed"
        ? "Use this to learn whether acting early actually improved the book."
        : responseKey === "deferred"
          ? "Use this to learn whether waiting protected capital or cost upside."
          : responseKey === "rejected"
            ? "Use this to learn whether the pass protected you from a false move."
            : "This outcome is still being logged into memory.";
    ...
    return {
      ...
      occurredAt: event.occurredAt,
      verdict,
      portfolioDeltaLabel: portfolioDelta === null ? "Accumulating" : formatSignedPct(portfolioDelta),
      benchmarkDeltaLabel: benchmarkDelta === null ? "Accumulating" : formatSignedPct(benchmarkDelta),
      excessDeltaLabel: excessDelta === null ? "Not ready" : formatSignedPct(excessDelta),
      note: event.note || "",
      lesson,
    };
  });
  ...
  return {
    ...
    subhead: "Track what happened after you acted, waited, or passed so the system can earn trust.",
    items,
...
function buildMemoryGuidance(memory, stressMode, mandate) {
  ...
  let profileLabel = "Still learning";
  let profileSummary = "The system is still gathering enough decisions to adapt its guidance to your behavior.";

  if (deferred >= 2 && deferred > executed) {
    profileLabel = "Deliberate under ambiguity";
    profileSummary = "You wait when the state is murky. The system should keep the tone sober and surface clearer disproof conditions.";
  } else if (staged >= 2 && executed < staged) {
    profileLabel = "Stages before committing";
    profileSummary = "You keep options open before you act. Escrow should stay prominent and legitimacy thresholds should stay explicit.";
  } else if (executed >= 2) {
    profileLabel = "Acts when evidence firms";
    profileSummary = "You move when the setup looks clean enough. The system should tighten disproof language so conviction stays honest.";
  } else if (cancelled >= 2) {
    profileLabel = "Reconsiders quickly";
    profileSummary = "You are willing to back away once the setup degrades. The product should keep invalidation cues highly visible.";
  }

  const overlays = [
    `Tone: keep guidance ${deferred > executed ? "measured and confirmation-heavy" : "clear and action-oriented"} under the current state.`,
    `Ranking: ${mandate?.rankingBias || "Bias the frontier toward cleaner recoverability."}`,
    `Escrow: ${mandate?.stagingBias || "Stage uncertain adds before they earn execution."}`,
  ];

  const warnings = [
    deferred > executed ? "Do not let waiting become drift when the frontier truly reopens." : null,
    executed > deferred && String(stressMode?.mode || "").toLowerCase().includes("protect")
      ? "You have been more active than the current protection mode wants. Keep size tighter."
      : null,
    cancelled >= 2 ? "Repeated staging then cancelling suggests your maturity conditions may still be vague." : null,
  ].filter(Boolean);

  return {
    headline: "Memory-driven guidance",
    profileLabel,
    profileSummary,
    overlays,
    warnings,
...
function buildRecoverabilityMap(snapshot, xray, frontier) {
  ...
  const holdingItems = (Array.isArray(xray?.weightedHoldings) ? xray.weightedHoldings : []).slice(0, 12).map((holding) => ({
    ...
    quadrant: holding.recoveryScore >= 0.6 && holding.fragilityScore < 0.5 ? "Earned" : holding.fragilityScore >= 0.65 ? "Fragile" : "Conditional",
  ...
  return {
    ...
    filters: [
      { id: "holdings", label: "Current holdings", count: holdingItems.length },
      { id: "watch", label: "Watch ideas", count: watchItems.length },
      { id: "blocked", label: "Blocked ideas", count: blockedItems.length },
    ],
...
function buildConfidencePanel(stressMode, modules, frontier, ledger) {
  ...
  return {
    ...
    contractStatus: stressMode?.contractStatusLabel || "Backup mode",
    trustState: modules?.command?.trustState || "Observe",
    decisionRights: modules?.command?.decisionRights || "Suggest only",
    evidenceTier: stressMode?.evidenceTier || "Live",
    analogCount,
    ...
    note:
      ledger?.items?.length
        ? "Confidence is displayed together with disproof conditions so the system can be challenged, not merely trusted."
        : "Confidence stays honest here: strong relative claims remain suppressed until history and evidence are sufficient.",
  };
}
...
function buildRecoverabilityBalanceSheet(snapshot, stressMode, frontier) {
  ...
  if (contract && raw && Object.keys(raw).length) {
    return {
      ...
      subhead: raw.summary || "Assets create future freedom. Liabilities consume it. Liquidity decides whether the book can afford new risk.",
      accountingState: humanizeBucket(raw.accounting_state || "balanced"),
      headlineState: raw.headline || "The book is earning and spending future freedom at the same time.",
      ...
      spendRule: raw.spend_rule || "Only spend optionality on moves that widen future freedom.",
      repairNote: raw.repair_note || "No repair note is available yet.",
  ...
  const assets = [
    { id: "recovery", label: "Recovery chance", value: recoverability, detail: "How much the current book can survive and still recover." },
    { id: "authority", label: "Evidence room", value: authority, detail: "How much the current evidence deserves to speak strongly." },
    { id: "room", label: "Room to act", value: roomToAct, detail: "How much gross action space the current posture still leaves open." },
  ];
  const liabilities = [
    { id: "phantom", label: "Phantom tax", value: phantom, detail: "Visible relief that still risks failing structurally." },
  ];

  return {
    headline: "Recoverability balance sheet",
    subhead: "A fallback accounting view built from the current stress and frontier snapshot.",
    accountingState: netFreedom >= 0.55 ? "Balanced" : "Stressed",
    headlineState: "Future freedom is being estimated from the current live posture.",
    ...
    reserves: [],
    spendRule: stressMode?.changeTrigger || "Espera una señal más clara antes de sumar riesgo.",
    repairNote: stressMode?.topMove?.summary || "No repair note is available yet.",
    notes: Array.isArray(stressMode?.fiberAtlas?.rows) && stressMode.fiberAtlas.rows.length
      ? [`Visible fiber ambiguity remains ${String(stressMode.fiberAtlas.ambiguityLabel || "unknown").toLowerCase()}.`]
      : [],
    source: "fallback",
  };
}
...
function buildCapitalTwin(portfolioModule, xray, frontier, stressMode, mandate, escrow) {
  ...
  const scenarioValues = [
    {
      ...
      explanation: "What the current book looks like if conditions stay roughly as they are.",
    },
    {
      id: "recovery",
      label: "Recovery broadens",
      delta: (recoverability * 0.14) + 0.03,
      explanation: "What happens if sponsorship and recoverability both improve from here.",
    },
    {
      id: "breakdown",
      label: "Breakdown resumes",
      delta: -((phantom * 0.1) + (protectMode ? 0.03 : 0.06)),
      explanation: "What the book gives back if the recent rebound proves too fragile.",
    },
    {
      id: "phantom_rebound",
      label: "Phantom rebound",
      delta: Math.max(-0.02, (recoverability * 0.02) - (phantom * 0.08)),
      explanation: "Prices bounce, but the structure does not repair enough to justify broad risk.",
    },
    {
      id: "sponsorship",
      label: "Sponsorship improves",
      delta: (recoverability * 0.09) + 0.01,
      explanation: "Selective actions become more credible if sponsorship visibly broadens.",
    },
  ];
  ...
  return {
    ...
    mandateLabel: mandate?.label || "Decision mandate",
    currentValueUsd: totalValue || null,
    baselineLabel: totalValue
      ? `Compared with your current live book at ${formatCurrency(totalValue)}.`
      : "Compared with the current connected book.",
    historyLabel:
      historySessions >= 5
        ? `The chart below is the stored portfolio path versus ${portfolioModule?.analytics?.benchmarkSymbol || "SPY"}.`
        : "The chart is still building from stored portfolio snapshots, so the scenario layer is more informative than the history line right now.",
    ...
    data_control: {
      ...
      notes: [
        ...
        modules.portfolio.holdingsSource?.connected
          ...
          : "Your private holdings are not connected yet, so the workspace is only showing the shared portfolio view.",
