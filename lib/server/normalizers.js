...
function buildActionsModule(snapshot, sharedAlpha) {
  ...
  return {
    ...
    subtitle: blocked
      ? "The market still does not support adding more risk. Stay patient and keep moves small."
      : liveAction
...
function makeAdviceTitle(action, snapshot, stressMode) {
  ...
  if ((action.ticker && ["SGOV", "SHY", "BIL", "TLT"].includes(action.ticker)) || action.ticker === defensiveTicker) {
    return "Preserve the reserve sleeve";
  }
...
function buildBackupAdvice(snapshot) {
  ...
  return {
    ...
    type: "hold",
    ticker: defensiveTicker,
    adviceTitle: "Preserve the reserve sleeve",
    size: "Keep current size",
    sizeValue: 0,
    funding: "No change",
    conviction: "No change is needed here unless a cleaner use for that capital appears.",
    whyNow: "The reserve sleeve should stay boring: it exists to support better portfolio decisions, not to become the headline.",
...
function buildWorkspaceStateSummary(snapshot, modules, justAdvice, stressMode) {
  ...
  return {
    ...
    stance: humanizeEngineLabel(snapshot?.overview?.recommended_action) || "Stay patient",
...
function inferHoldingRole(holding) {
  ...
  if ([
    ...
    return {
      id: "ballast",
      label: "Defense",
      description: "Preserves optionality while the frontier stays tight.",
      fragility: 0.12,
      recovery: 0.2,
      sponsorship: 0.3,
    };
...
function decorateFrontierAction(action, lane, stressMode, mandate) {
  ...
  const whyLane = lane === "unlocked"
    ...
    : lane === "staged"
      ? "You have preserved the option, but it still needs live state confirmation."
...
function buildActionFrontier(decisionWorkspace, stressMode, mandate) {
  ...
  const staged = (Array.isArray(decisionWorkspace?.escrow?.items) ? decisionWorkspace.escrow.items : [])
    ...
    .map((item) => ({
      ...
      evidenceBand: stressMode?.authorityLabel || "Usable",
      evidenceTier: stressMode?.evidenceTier || "Live",
      whyLane: "This action is preserved in escrow, but the system still wants more confirmation before it is clearly legitimate.",
      disproofCondition:
    ...
    const lesson =
      responseKey === "executed"
        ...
        : responseKey === "deferred"
          ? "Use this to learn whether waiting preserved capital or cost upside."
...
function buildMemoryGuidance(memory, stressMode, mandate) {
  ...
  if (deferred >= 2 && deferred > executed) {
    ...
    profileSummary = "You preserve options before you act. Escrow should stay prominent and legitimacy thresholds should stay explicit.";
  } else if (executed >= 2) {
  ...
  const warnings = [
    deferred > executed ? "Do not let patience become drift when the frontier truly reopens." : null,
...
function buildRecoverabilityBalanceSheet(snapshot, stressMode, frontier) {
  ...
  if (contract && raw && Object.keys(raw).length) {
    return {
      ...
      subhead: raw.summary || "Assets create future freedom. Liabilities consume it. Reserves decide whether the book can afford new risk.",
  ...
  const assets = [
    ...
    { id: "authority", label: "Authority reserve", value: authority, detail: "How much the current evidence deserves to speak strongly." },
