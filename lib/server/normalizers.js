...
function formatMetricLabel(value, formatter, fallback = "Not enough history") {
...
function buildPortfolioModule(snapshot, watchlistCount, sharedAlpha) {
  ...
  const maxDrawdownLabel = computedAnalytics.maxDrawdown !== null
    ? (hasPerformanceHistory ? fmtPct(computedAnalytics.maxDrawdown) : "Track more history")
    : holdingsSource.connected
      ? "Track more history"
      : "Not enough history";
  ...
  return {
    ...
    chartSource:
      liveGrowthComparison.length >= 5 || sectorExposure.length || valuationDistribution.length
        ? "Live portfolio history and holdings snapshot"
        : holdingsSource.connected
          ? "Private holdings are live. Performance history is still building."
