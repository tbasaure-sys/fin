...
function buildActionFramework(snapshot, sharedAlpha) {
  ...
  const compression = clamp01(numberOr(latest.compression_score, numberOr(snapshot?.overview?.compression_score, 0.41)));
  const freedom = clamp01(numberOr(latest.freedom_score, numberOr(snapshot?.overview?.freedom_score, 0.59)));
  const effectiveDimension = numberOr(latest.effective_dimension, 6.2);
