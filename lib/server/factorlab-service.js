...
function firstPayloadRow(payload) {
  return Array.isArray(payload) ? payload[0] || {} : payload || {};
}

function parseFmpMarketSnapshot(quotePayload, profilePayload, statementPayloads, ticker) {
  ...
  const income = firstPayloadRow(statementPayloads?.income);
  const balance = firstPayloadRow(statementPayloads?.balance);
  const cashFlow = firstPayloadRow(statementPayloads?.cashFlow);
  const price = finite(quote?.price);
  if (price === null || price <= 0 || String(quote?.symbol || "").toUpperCase() !== String(ticker || "").toUpperCase()) {
    throw new Error("FMP market price unavailable.");
  }
  const isEtf = Boolean(profile?.isEtf || quote?.isEtf);
  const isFund = Boolean(profile?.isFund || quote?.isFund);
  const isAdr = Boolean(profile?.isAdr || quote?.isAdr);
  const timestamp = finite(quote?.timestamp);
  const volume = finite(quote?.volume ?? quote?.avgVolume);
  return {
    ticker: String(ticker || "").toUpperCase(),
    name: String(profile?.companyName || quote?.name || ticker || "").trim(),
    price,
    marketCap: finite(quote?.marketCap ?? profile?.mktCap),
    marketDate: timestamp ? new Date(timestamp * 1000).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10),
    averageDailyValue: volume !== null && volume > 0 ? volume * price : null,
    residualVol: null,
    currency: quote?.currency || profile?.currency || "USD",
    exchange: quote?.exchange || profile?.exchangeShortName || profile?.exchange || null,
    sector: profile?.sector || null,
    industry: profile?.industry || null,
    instrumentType: isEtf ? "ETF" : isFund ? "FUND" : isAdr ? "ADR" : "EQUITY",
    source: "Financial Modeling Prep quote",
    fundamentals: {
      revenue: finite(income?.revenue),
      netIncome: finite(income?.netIncome),
      totalEquity: finite(balance?.totalStockholdersEquity ?? balance?.totalEquity),
      freeCashFlow: finite(cashFlow?.freeCashFlow),
      cash: finite(balance?.cashAndCashEquivalents ?? balance?.cashAndShortTermInvestments),
      debt: finite(balance?.totalDebt),
      dilutedShares: finite(income?.weightedAverageShsOutDil ?? income?.weightedAverageShsOut),
      fiscalYear: finite(income?.calendarYear ?? income?.fiscalYear),
    },
  };
}

async function fetchFmpMarketSnapshot(ticker, fetchImpl) {
  const apiKey = process.env.FMP_API_KEY || process.env.FINANCIAL_MODELING_PREP_API_KEY;
  if (!apiKey) throw new Error("FMP market provider is not configured.");
  const quoteUrl = new URL("https://financialmodelingprep.com/stable/quote");
  quoteUrl.searchParams.set("symbol", ticker);
  quoteUrl.searchParams.set("apikey", apiKey);
  const profileUrl = new URL("https://financialmodelingprep.com/stable/profile");
  profileUrl.searchParams.set("symbol", ticker);
  profileUrl.searchParams.set("apikey", apiKey);
  const statementUrl = (endpoint) => {
  ...
  const [quoteResponse, profileResponse, incomeResponse, balanceResponse, cashFlowResponse] = await Promise.all([
    fetchImpl(quoteUrl, { cache: "no-store", signal: AbortSignal.timeout(10_000) }),
    fetchImpl(profileUrl, { cache: "no-store", signal: AbortSignal.timeout(10_000) }),
    fetchImpl(statementUrl("income-statement"), { cache: "no-store", signal: AbortSignal.timeout(10_000) }),
    fetchImpl(statementUrl("balance-sheet-statement"), { cache: "no-store", signal: AbortSignal.timeout(10_000) }),
    fetchImpl(statementUrl("cash-flow-statement"), { cache: "no-store", signal: AbortSignal.timeout(10_000) }),
  ]);
  if (!quoteResponse.ok) throw new Error(`FMP market provider returned ${quoteResponse.status}.`);
  const quotePayload = await quoteResponse.json();
  const profilePayload = profileResponse.ok ? await profileResponse.json() : [];
  const statements = {
    income: incomeResponse.ok ? await incomeResponse.json() : [],
    balance: balanceResponse.ok ? await balanceResponse.json() : [],
    cashFlow: cashFlowResponse.ok ? await cashFlowResponse.json() : [],
  };
  return parseFmpMarketSnapshot(quotePayload, profilePayload, statements, ticker);
}

export async function fetchFactorLabMarketSnapshot(ticker, { fetchImpl = fetch } = {}) {
  const symbol = String(ticker || "").toUpperCase();
  let yahooMarket = null;
  let yahooError = null;
  try {
    ...
    yahooMarket = parseFactorLabMarketChart(await response.json(), ticker);
  } catch (error) {
    yahooError = error;
  }

  if (process.env.FMP_API_KEY || process.env.FINANCIAL_MODELING_PREP_API_KEY) {
    try {
      const fmpMarket = await fetchFmpMarketSnapshot(symbol, fetchImpl);
      if (!yahooMarket) return fmpMarket;
      return {
        ...fmpMarket,
        ...yahooMarket,
        name: fmpMarket.name,
        marketCap: yahooMarket.marketCap ?? fmpMarket.marketCap,
        averageDailyValue: yahooMarket.averageDailyValue ?? fmpMarket.averageDailyValue,
        sector: fmpMarket.sector,
        industry: fmpMarket.industry,
        fundamentals: fmpMarket.fundamentals,
        instrumentType: yahooMarket.instrumentType || fmpMarket.instrumentType,
      };
    } catch {
      if (!yahooMarket) throw yahooError || new Error("Market providers unavailable.");
    }
  }

  if (yahooMarket) return yahooMarket;
  throw yahooError || new Error("Market providers unavailable.");
}
