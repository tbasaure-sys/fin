...
const VERSION = "bls_stress_engine_v9_fhs_factor_bank";
...
const FALLBACK_ENDPOINT_CONTRACT = Object.freeze({
  ...
  status: "v9_7_fhs_factor_stress_engine_with_conditional_var",
  status_label: "V9.7 PIT FHS stress + conditional VaR ready",
  run_id: "fhs_v9_7_run_20260703_023947",
  served_engine: "fhs_v9_stress_factor_bank_projection",
  base_model: "fhs_v9_base",
  stress_model: "fhs_v9_stress",
  checkpoint_path: null,
  manifest_path: ".local_colab_drive/blsprime_ddpm_market_sim/artifacts/fhs_v9_7_run_20260703_023947/blsprime_market_simulation_manifest.json",
  ...
  stress_book_q01: -0.335289,
  stress_book_q01_note: "Pooled v9 adverse ladder q01; this is a stress scenario floor, not an unconditional forecast distribution.",
  ...
  stress_replay_status: "v9_walk_forward_stress_floor_covered",
  endpoint_gate: "Serve the PIT FHS factor stress bank for covered portfolios; validate daily VaR through rolling conditional Kupiec and Christoffersen.",
  ready_for_endpoint_requires: "Stress endpoint requires stress MMD leadership, Gaussian-level correlation, pre-cutoff walk-forward stress coverage, endpoint scenario count, sufficient portfolio coverage, and conditional rolling VaR5 Kupiec plus Christoffersen coverage.",
});
...
function runtimeDailyReturnClip() {
  const [min, max] = Array.isArray(MARKET_SIMULATION_CONTRACT.daily_return_clip)
    ? MARKET_SIMULATION_CONTRACT.daily_return_clip
    : Array.isArray(MARKET_SIMULATION_METRICS.dailyReturnClip)
      ? MARKET_SIMULATION_METRICS.dailyReturnClip
      : [-0.8, 1.5];
  return [Number.isFinite(Number(min)) ? Number(min) : -0.8, Number.isFinite(Number(max)) ? Number(max) : 1.5];
}

function runtimeTerminalReturnClip() {
  const [min, max] = Array.isArray(MARKET_SIMULATION_CONTRACT.terminal_return_clip)
    ? MARKET_SIMULATION_CONTRACT.terminal_return_clip
    : [-0.95, 2.5];
  return [Number.isFinite(Number(min)) ? Number(min) : -0.95, Number.isFinite(Number(max)) ? Number(max) : 2.5];
}
...
function readyForStressEndpoint(scorecard = {}) {
  if (scorecard.ready_for_stress_endpoint === true) return true;
  return Boolean(
    scorecard.product_candidate_is_stress_book &&
    scorecard.beats_gaussian_mmd_multi &&
    scorecard.beats_t_copula_mmd &&
    scorecard.beats_fhs_mmd &&
    scorecard.beats_same_stack_gaussian_mmd &&
    scorecard.corr_near_gaussian &&
    scorecard.walk_forward_stress_covers_all &&
    scorecard.endpoint_scenario_count_ok,
  );
}

function buildDeploymentContract(requestPolicy, guidanceInfo) {
  const scorecard = MARKET_SIMULATION_METRICS.scorecard || {};
  const stressReady = readyForStressEndpoint(scorecard);
  return {
    status: MARKET_SIMULATION_CONTRACT.status,
    statusLabel: MARKET_SIMULATION_CONTRACT.status_label || "V9 FHS factor stress champion",
    researchChampion: Boolean(scorecard.research_champion),
    ddpmResearchChampion: Boolean(scorecard.ddpm_research_champion),
    sameStackChampion: Boolean(scorecard.same_stack_champion),
    stressEngineChampion: Boolean(scorecard.stress_engine_champion || scorecard.product_candidate_is_stress_book),
    readyForEndpoint: Boolean(scorecard.ready_for_endpoint),
    readyForStressEndpoint: stressReady,
    endpointGate:
      MARKET_SIMULATION_CONTRACT.endpoint_gate ||
      "Endpoint serves the PIT FHS factor stress bank when portfolio coverage is sufficient; daily VaR is validated separately with rolling conditional volatility.",
    endpoint: MARKET_SIMULATION_CONTRACT.endpoint,
    runtime: {
      servedEngine: MARKET_SIMULATION_CONTRACT.served_engine || "fhs_v9_stress_factor_bank_projection",
...
const MAX_RUNTIME_UNIVERSE = 24;
const CASHLIKE_TICKERS = new Set(["SGOV", "SHY", "BIL", "SHV", "VGSH", "JPST", "DWBDS"]);
const FIXED_INCOME_TICKERS = new Set(["TLT", "IEF", "IEI", "AGG", "BND", "LQD", "TIP", "HYG", "JNK"]);

function holdingAssetClass(holding = {}) {
  const ticker = cleanTicker(holding?.ticker || holding?.symbol, "").toUpperCase();
  const assetType = String(holding?.assetType || holding?.asset_type || "").trim().toLowerCase();
  const sector = String(holding?.sector || holding?.theme || "").trim().toLowerCase();
  if (CASHLIKE_TICKERS.has(ticker) || assetType === "cash" || sector === "cash") return "cash_like";
  if (
    FIXED_INCOME_TICKERS.has(ticker) ||
    assetType.includes("bond") ||
    assetType.includes("fixed") ||
    sector.includes("bond") ||
    sector.includes("treasury") ||
    sector.includes("fixed income")
  ) {
    return "fixed_income";
  }
  if (assetType.includes("crypto")) return "crypto";
  return "equity_like";
}

function fallbackDailyVol({ holding, weight, riskScore }) {
  const assetClass = holdingAssetClass(holding);
  if (assetClass === "cash_like") return 0.0015;
  if (assetClass === "fixed_income") return clamp(0.006 + Math.sqrt(Math.max(weight, 0)) * 0.012, 0.006, 0.025);
  if (assetClass === "crypto") return clamp(0.035 + Math.sqrt(Math.max(weight, 0)) * 0.02, 0.03, 0.11);
  return clamp(0.009 + (riskScore || 2.5) * 0.0032 + Math.sqrt(Math.max(weight, 0)) * 0.012, 0.008, 0.075);
}
...
function buildUniverse(dashboard) {
  const holdings = safeList(dashboard?.modules?.portfolio?.holdings)
    ...
    .map((holding, index) => {
      ...
      const assetClass = holdingAssetClass(holding);
      const baseVol = fallbackDailyVol({ holding, weight, riskScore });
      return {
        ticker: cleanTicker(holding?.ticker || holding?.symbol, `ASSET${index + 1}`),
        sector: assetClass === "cash_like"
          ? "Cash"
          : assetClass === "fixed_income"
            ? "Fixed Income"
            : String(holding?.sector || holding?.theme || "Other"),
        assetClass,
        weight,
...
function choleskyDecomposition(matrix) {
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j <= i; j += 1) {
      ...
      if (i === j) {
        const residual = matrix[i][i] - sum;
        if (residual <= 1e-6) return null;
        lower[i][j] = Math.sqrt(residual);
      } else {
        if (lower[j][j] <= 1e-6) return null;
...
function shrinkCorrelationMatrix(matrix, shrink) {
  return matrix.map((row, i) => row.map((value, j) => {
    if (i === j) return 1;
    return clamp(value * shrink, -0.95, 0.95);
  }));
}

function cholesky(matrix) {
  const shrinkSchedule = [1, 0.85, 0.7, 0.55, 0.4, 0.25, 0.1, 0];
  for (const shrink of shrinkSchedule) {
    const lower = choleskyDecomposition(shrink === 1 ? matrix : shrinkCorrelationMatrix(matrix, shrink));
...
function buildScenarioBankOverlay({ universe, weights, regimeKey }) {
  ...
  if (!bank) {
    return {
      ...
      warnings: ["Deployable v9 factor scenario bank artifact is not available in this runtime."],
    };
  }
  ...
  if (!scenarioCount || !assetCount || matched.length < 1 || matchedWeightCoverage <= 0) {
    return {
      ...
      warnings: ["No current holdings matched the deployable v9 factor scenario bank universe."],
    };
  }
  ...
  const servedAsPrimary = Boolean(manifest.servedAsPrimary) && status === "available";
  return {
    status,
    available: true,
    servedAsPrimary,
    ...
    warnings: [
      servedAsPrimary
        ? "Risk numbers are served from the v9 PIT FHS factor scenario bank for matched portfolio weight."
        : "Scenario bank coverage is partial; the endpoint fell back to the visible historical-return runtime for primary risk.",
      matchedWeightCoverage < 0.7
        ? `Only ${pct(matchedWeightCoverage)} of portfolio weight matched the v9 factor bank universe; treat bank numbers as partial-book diagnostics.`
        : null,
...
function buildHistoricalReplay(championMetrics) {
  ...
  const refits = safeList(championMetrics?.walk_forward_refits);
  const rows = (refits.length
    ? refits.map((row) => ({
      id: `cutoff_${String(row.cutoff || "unknown").replace(/[^0-9]/g, "")}`,
      episode: `Pre-${row.cutoff || "cutoff"} refit`,
      actualMin: numberOrNull(row.actual_min_terminal),
      syntheticQ01: numberOrNull(row.stress_q01_terminal),
    }))
    : [
      {
        id: "covid_2020",
  ...
  const methodologyValidated = walkForward.stress_walk_forward_methodology_validated === true || (refits.length > 0 && coveredCount === valid.length);
  return {
    rows,
    coveredCount,
    ...
    methodologyStatus: methodologyValidated ? "stress_refit_validated" : "unconditional_stress_floor",
    methodologyNote:
      walkForward.stress_walk_forward_methodology_note ||
      "Stress coverage is judged from pre-cutoff refits. Unconditional Kupiec gates are reported separately as diagnostics.",
  };
}
...
function buildBaselineComparison(championMetrics) {
  ...
  const championRatio = numberOrNull(relativeToChampion.stress_mmd_multi_ratio_vs_same_stack ?? relativeToChampion.ddpm_mmd_multi_ratio_vs_same_stack);
  const corrDelta = numberOrNull(relative.corr_mae_delta_candidate_minus_gaussian);
  const stressReady = readyForStressEndpoint(scorecard);
  return {
    championModel: champion.model || "fhs_v9_stress",
    championStatus: champion.status || "served_stress_champion",
    ...
    stressVsSameStackMmdRatio: round(championRatio, 4),
...
function makeScenarioPath({ universe, lower, regime, rand, horizonDays, guidanceScale }) {
  const path = [];
  const [dailyMin, dailyMax] = runtimeDailyReturnClip();
  ...
  for (let day = 0; day < horizonDays; day += 1) {
    ...
    const returns = universe.map((asset, index) => {
      const idiosyncraticTail = rand() < guidedTailProbability * 0.16
        ...
        : 0;
      const rawReturn = asset.drift + regime.drift + correlated[index] * asset.vol * guidedVolMultiplier + marketTail + idiosyncraticTail;
      return clamp(rawReturn, dailyMin, dailyMax);
    });
    path.push(returns);
  }
  return path;
...
function cumulativeReturn(path, weights) {
  const [terminalMin, terminalMax] = runtimeTerminalReturnClip();
  let value = 1;
  for (const day of path) {
    const daily = day.reduce((sum, assetReturn, index) => sum + assetReturn * weights[index], 0);
    value *= (1 + daily);
  }
  return clamp(value - 1, terminalMin, terminalMax);
}
...
function cumulativePortfolioCurve(path, weights) {
  ...
  return path.map((day) => {
    const daily = day.reduce((sum, assetReturn, index) => sum + assetReturn * weights[index], 0);
    value *= (1 + daily);
    return round(clamp(value - 1, terminalMin, terminalMax), 4);
  });
}
...
export function buildDiffusionMarketSimulation(dashboard = {}, options = {}) {
  ...
  const servedScenarioBankPrimary = Boolean(scenarioBankOverlay.servedAsPrimary);
  const heuristicRisk = {
    expectedReturn: round(mean(portfolioReturns), 4),
    expectedReturnLabel: pct(mean(portfolioReturns)),
    medianReturn: round(quantile(portfolioReturns, 0.5), 4),
  ...
  const primaryRisk = servedScenarioBankPrimary
    ? {
      ...scenarioBankOverlay.risk,
      probabilityDrawdown10: null,
      probabilityDrawdown10Label: "-",
    }
    : heuristicRisk;
  const primaryTailContributors = servedScenarioBankPrimary ? scenarioBankOverlay.tailContributors : contributors;
  ...
  return {
    ...
    model: {
      family: "PIT FHS Factor Stress Engine",
      championModel: baselineComparison.championModel,
      trained: false,
      offlineChampionTrained: false,
      offlineChallengerTrained: false,
      trainedCheckpointServed: false,
      note: "V9 promotes the PIT FHS stress bank as the served stress engine when portfolio coverage is sufficient. Unconditional books remain diagnostic until train-only validation clears.",
      horizonDays,
      nScenarios,
    ...
    inputSources: {
      ...
      scenarioBankOverlay: {
        ...
        servedAsPrimary: servedScenarioBankPrimary,
      ...
      includedAssets: universe.length,
      maxRuntimeUniverse: MAX_RUNTIME_UNIVERSE,
      ...
      portfolioBasis: sourceHoldingsCount > 0 ? "user_holdings" : "demo_fallback",
      portfolioBasisLabel: sourceHoldingsCount > 0
        ? "Stress test sobre tu cartera real"
        : "Sin posiciones conectadas: este stress test usa una cartera demo (SPY/QQQ/SGOV). Agrega posiciones para ver tu riesgo real.",
    },
    universe: universe.map((row) => ({
      ticker: row.ticker,
      sector: row.sector,
      assetClass: row.assetClass || "equity_like",
    ...
    diagnostics: {
      ...
      baselineComparison,
      sampler: servedScenarioBankPrimary
        ? "v9 PIT FHS factor scenario bank projection"
        : guidanceInfo.stratified
    ...
    validation: {
      ...
      baselineComparison,
      endpointGate: {
        ready: baselineComparison.readyForStressEndpoint,
        statusLabel: baselineComparison.readyForStressEndpoint ? "V9 stress engine ready" : "Research gated",
    ...
    tailContributors: primaryTailContributors,
    ...
    warnings: [
      sourceHoldingsCount === 0 ? "No connected holdings: results describe a demo portfolio, not your book." : null,
      ...
      requestPolicy.policyApplied === "aggregated_to_minimum"
        ? `Requested scenario count was below the v9 minimum; aggregated to ${nScenarios}.`
        : null,
      ...
      servedScenarioBankPrimary
        ? "Sample paths are illustrative runtime paths; terminal risk metrics and tail contributors are served from the v9 factor scenario bank."
        : null,
      "DDPM research remains gated; the served stress engine is the CPU PIT FHS factor bank.",
      historicalModel
        ? "Fallback runtime covariance and volatility are estimated from historical return data when available."
        : "Fallback runtime did not have enough historical return data to estimate full real covariance.",
