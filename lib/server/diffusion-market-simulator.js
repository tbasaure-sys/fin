const VERSION = "bls_factor_ddpm_market_simulator_v7_contract";
const MAX_ENDPOINT_SCENARIOS = 6000;

const DDPM_V7_ENDPOINT_CONTRACT = Object.freeze({
...
const DDPM_V7_CHAMPION_METRICS = Object.freeze({
...
function buildTargetMatrix(universe, regime) {
  return universe.map((left) => universe.map((right) => targetCorrelation(left, right, regime)));
}
...
export function buildDiffusionMarketSimulation(dashboard = {}, options = {}) {
  ...
  const guidanceScale = clamp(Number(options.guidanceScale || 1.0), 0.5, 6);
  ...
  const targetMatrix = buildTargetMatrix(universe, regime);
  ...
  return {
    ...
    model: {
      family: "Factor-DDPM v7 deployment contract with correlation-aware runtime proxy",
      trained: false,
      offlineChampionTrained: true,
      trainedCheckpointServed: false,
      note: "The Colab Factor-DDPM v7 checkpoint is an offline research champion. This live endpoint serves a guarded proxy until the strict endpoint gates pass.",
      horizonDays,
      nScenarios,
    ...
    diagnostics: {
      ...
      sampler: guidanceInfo.stratified
        ? "seeded synthetic diffusion proxy with v7 stress-book guidance mix"
        : "seeded synthetic diffusion proxy",
    },
    ...
    warnings: [
      ...
      requestPolicy.policyApplied === "aggregated_to_minimum"
        ...
        : null,
      "Factor-DDPM v7 is a research champion, but this endpoint is not production-ready until strict MMD and correlation gates pass.",
      "Live endpoint currently serves a calibrated JavaScript proxy; the PyTorch checkpoint remains an offline Colab artifact.",
