import 'server-only';
import { newsPairs, reviewPriority, gateNewsAnswers } from '../research/jev.mjs';
...
export function createJevHttp({authenticate,service,market,store,verify,readPortfolio,companyDirectory=async()=>new Map(),consume=async()=>({allowed:true})}) {
  return async function handler(request) {
    ...
    try {
      ...
      if(body.kind==='news') {
        ...
        const names=await companyDirectory();
        items=items.map(item=>({...item,companyName:names.get(item.ticker.replaceAll('-','.'))||null}));
      ...
      return json({...result,kind:body.kind,context,total,selected:items.length,assessed:result.items.filter(r=>r.status==='available').length,items:result.items.map((r,i)=>{const answers=body.kind==='news'?gateNewsAnswers(items[i],r.answers):r.answers;return {...items[i],...r,id:items[i].id,answers,priority:body.kind==='news'?reviewPriority(items[i],answers):null}})});
