...
function normalizeAppName(value) {
  ...
  return /allocator workspace/i.test(raw) ? "BLS Prime" : raw;
}
...
export function getBrandConfig() {
  const appName = normalizeAppName(envString("NEXT_PUBLIC_BLS_APP_NAME", "BLS Prime"));
...
export function getPublicAppUrl() {
  // Preview account links must return to this deployment, never to a shared
  // production APP_URL. VERCEL_URL is a server-provided hostname, not user input.
  if (envString("VERCEL_ENV") === "preview") {
    const host = envString("VERCEL_URL");
    if (!/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.vercel\.app$/i.test(host)) {
