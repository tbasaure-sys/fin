...
function buildVerdict(presentation) {
  ...
  if (presentation.state === "market_implied") {
    return {
      kind: "market_implied",
      label: "Precio de mercado",
      headline: "La cotización es observable; el valor fundamental todavía no está publicado.",
      explanation: `${presentation.reason} No es una recomendación ni un valor razonable.`,
    };
  }
...
function buildExpectations(research, presentation, indicative) {
  if (presentation.state === "market_implied") {
    return list(indicative?.marketImplied?.expectations).map((row) => ({
      years: null,
      label: firstText(row?.label, row?.key) || "Expectativa implícita",
      value: finite(row?.value),
      unit: text(row?.unit) || "number",
      detail: firstText(row?.calculation, row?.interpretation),
      provenance: text(row?.provenance),
      source: text(row?.source),
      asOf: text(row?.asOf) || null,
...
function buildClosurePlan(research, presentation, missingEvidence) {
  const supplied = list(presentation.closureRequirements);
  const suppliedRows = supplied.map((item, index) => ({
    key: firstText(item?.key) || `closure-${index + 1}`,
    control: firstText(item?.control, item?.question) || "Control pendiente",
    why: firstText(item?.why, item?.whyItMatters) || "Este control puede cambiar la lectura de valuación.",
    estimatedImpact: firstText(item?.estimatedImpact) || "Puede cambiar el método o impedir publicar un valor razonable.",
    sourceNeeded: firstText(item?.sourceNeeded) || "Evidencia primaria fechada y trazable.",
    nextAction: firstText(item?.nextAction, item?.question) || "Resolver el control con evidencia trazable.",
    resolvable: true,
  }));
  if (presentation.state === "decision_ready") return [];
  const severityRank = { fatal: 0, critical: 0, high: 1, medium: 2, low: 3 };
  const auditKeys = list(research?.audit?.findings)
    .map((finding, index) => ({ key: firstText(finding?.code, finding?.control), severity: severityRank[text(finding?.severity).toLowerCase()] ?? 4, index }))
    .filter((item) => item.key)
    .sort((left, right) => left.severity - right.severity || left.index - right.index)
    .map((item) => item.key);
  const coverageKeys = missingEvidence.map((item) => item.key);
  const missingKeys = [...auditKeys, ...coverageKeys];
  if (!missingKeys.length) missingKeys.push("reliability_review");
  const derivedRows = [...new Set(missingKeys)].map((key) => {
  ...
  return [...derivedRows, ...suppliedRows]
...
export function buildCompanyDecisionView(research, { now = Date.now() } = {}) {
  const institutionalPresentation = buildEquityValuationPresentation(research, { now });
  const indicative = institutionalPresentation.state === "decision_ready" || institutionalPresentation.state === "research_grade"
    ? null
    : buildIndicativeValuation(research);
  const presentation = indicative ? {
    state: indicative.status,
    backed: indicative.status === "decision_ready",
    showValuationFigures: indicative.status === "decision_ready" || indicative.status === "research_grade",
    range: indicative.range,
    centralValue: indicative.range?.central ?? null,
    currentPrice: indicative.currentPrice,
    priceIsContextual: true,
    primaryMethod: indicative.method,
    confidence: indicative.confidence.score,
    currency: indicative.currency,
    marketDataAsOf: indicative.marketDataAsOf,
    priceSource: indicative.priceSource,
    reason: indicative.reason,
    reasons: [],
    limitations: indicative.limitations,
    showExecutiveJudgment: false,
    executiveJudgment: "",
    closureRequirements: indicative.closureRequirements,
  ...
  const drivers = normalizeTextRows(thesis.drivers || research?.valuation?.driver_summary?.requirements || research?.valuation?.driver_summary?.holds)
    || [];
  const effectiveDrivers = drivers.length ? drivers : list(indicative?.drivers).map((driver) => driver.label).filter(Boolean);
  ...
  const suppliedExplanation = research?.aurora?.explanation;
  const explanationWhy = list(suppliedExplanation?.why)
    .map((item) => ({
      title: firstText(item?.title, item?.label),
      explanation: firstText(item?.explanation, item?.detail),
    }))
    .filter((item) => item.title && item.explanation);
  const valuationExplanation = suppliedExplanation
    ? {
      provider: text(suppliedExplanation.provider) || "deterministic",
      model: text(suppliedExplanation.model) || null,
      summary: firstText(suppliedExplanation.summary, presentation.reason),
      why: explanationWhy.length
        ? explanationWhy
        : list(indicative?.drivers).map((driver) => ({ title: driver.label, explanation: driver.why })).filter((item) => item.title && item.explanation),
      risks: normalizeTextRows(suppliedExplanation.risks).length
        ? normalizeTextRows(suppliedExplanation.risks)
        : list(presentation.limitations),
      confidenceExplanation: firstText(
        suppliedExplanation.confidenceExplanation,
        suppliedExplanation.confidence_explanation,
        indicative?.confidence?.reason,
        presentation.reason,
      ),
    }
    : {
      ...
      model: null,
      summary: presentation.reason,
      why: list(indicative?.drivers).map((driver) => ({ title: driver.label, explanation: driver.why })).filter((item) => item.title && item.explanation),
      risks: list(presentation.limitations),
      confidenceExplanation: firstText(indicative?.confidence?.reason, presentation.reason),
    };
  ...
  return {
    ...
    analysis: {
      ...
      label: presentation.state === "decision_ready"
        ...
        : presentation.state === "research_grade"
          ...
          : presentation.state === "market_implied"
            ? "Precio de mercado"
            : "En revisión",
    ...
    valuation: {
      publishable: presentation.state === "decision_ready",
      kind: indicative?.kind || (presentation.backed ? "defendable" : "research"),
      range: presentation.range,
      method: presentation.primaryMethod,
      reason: presentation.reason,
      limitations: presentation.limitations,
      basis: indicative?.basis || "institutional_model",
      confidence: indicative?.confidence || {
        label: presentation.backed ? "Alta" : "Media",
        score: presentation.confidence,
        reason: presentation.reason,
      },
      drivers: indicative?.drivers || [],
      explanation: valuationExplanation,
    },
    expectations: buildExpectations(research, presentation, indicative),
    thesis: {
      summary: firstText(thesis?.summary, thesis?.headline, research?.memo?.summary, presentation.reason),
      drivers: effectiveDrivers,
      risks,
      mainDriver: effectiveDrivers[0] || "Impulsor principal aún no documentado.",
      mainRisk: risks[0] || presentation.limitations[0] || "Riesgo principal aún no documentado.",
    },
    scenarios: indicative?.scenarios || normalizeScenarios(research, presentation),
    evidence,
