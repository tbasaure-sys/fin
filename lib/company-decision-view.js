...
export function buildCompanyDecisionView(research, { now = Date.now() } = {}) {
  ...
  const indicative = institutionalPresentation.showValuationFigures ? null : buildIndicativeValuation(research);
  const presentation = indicative ? {
    state: "indicative",
    backed: false,
    showValuationFigures: true,
    range: indicative.range,
    centralValue: indicative.range.central,
  ...
  return {
    ...
    valuation: {
      ...
      kind: indicative ? "approximate" : presentation.backed ? "defendable" : "research",
    ...
    closurePlan: indicative ? [] : buildClosurePlan(research, presentation, evidence.missing),
  };
}
