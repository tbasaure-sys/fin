...
export function buildPortfolioDisplayRows(holdings) {
  const normalized = normalizePortfolioDraft(holdings);
  const sourceByTicker = new Map(
    (Array.isArray(holdings) ? holdings : [])
      .map((row) => [cleanTicker(row?.ticker), row])
      .filter(([ticker]) => ticker),
  );
  const totalValueUsd = normalized.reduce((sum, row) => sum + row.marketValueUsd, 0);

  return normalized.map((row) => {
    const source = sourceByTicker.get(row.ticker) || {};
    const totalReturn = finiteNumber(source.totalReturnInclDividends ?? source.total_return_incl_dividends ?? source.unrealizedReturn ?? source.unrealized_return);
    return {
      ...row,
      company: String(source.company || source.name || "").trim() || null,
      totalReturn,
      totalReturnLabel: source.totalReturnInclDividendsLabel || source.unrealizedReturnLabel || null,
      weight: totalValueUsd > 0 ? row.marketValueUsd / totalValueUsd : 0,
    };
  });
}

export function computePortfolioOverlapShare(holdingCount, effectiveBets) {
  const visible = Number(holdingCount);
  const effective = Number(effectiveBets);
  if (!(visible > 0) || !Number.isFinite(effective)) return 0;
  return Number(Math.max(0, Math.min(1, 1 - (effective / visible))).toFixed(4));
}
