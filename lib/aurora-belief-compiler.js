...
export function compileAuroraBeliefDrivers(snapshot = {}, options = {}) {
  ...
  const taxRate = clamp(firstFinite(driversOverride.taxRate, snapshot.taxRate) ?? 0.22, 0, 0.45);
  ...
  const baseFcf = firstFinite(driversOverride.baseFcf, driversOverride.fcf, snapshot.baseFcf, freeCashFlowFromStatement(cashFlow));
  const margin = firstFinite(driversOverride.margin, snapshot.margin, computeMargin(income));
  const roic = firstFinite(driversOverride.roic, snapshot.roic, computeRoic(income, balance, taxRate));
  ...
  const reinvestment = firstFinite(driversOverride.reinvestment, snapshot.reinvestment, computeReinvestment(income, cashFlow, taxRate));
  ...
  const rawLineage = {
    ...
    baseFcf: sourceLineageEntry("latest cash flow free cash flow", baseFcf, isFiniteNumber(baseFcf) ? 1 : 0),
    revenueCagr: sourceLineageEntry("income statement history CAGR", revenueCagr, isFiniteNumber(revenueCagr) ? 0.85 : 0),
    margin: sourceLineageEntry("latest EBIT / revenue", margin, isFiniteNumber(margin) ? 0.9 : 0),
    roic: sourceLineageEntry("NOPAT / invested capital", roic, isFiniteNumber(roic) ? 0.78 : 0),
    wacc: sourceLineageEntry("macro + beta + sector prior", wacc, 0.7),
    reinvestment: sourceLineageEntry("capex / NOPAT", reinvestment, isFiniteNumber(reinvestment) ? 0.72 : 0),
