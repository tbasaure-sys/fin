...
export function runFactorLab(input = {}) {
  ...
  if (refusal) {
    return {
      ...
      summary: { eligible: 0, returned: 0, coverage: 0, topScore: null, abstain: 0, researchable: 0, rankable: 0 },
      disclaimer: DISCLAIMER,
    };
  }

  const scoped = SAMPLE_UNIVERSE.filter((row) => isEligibleForUniverse(row, spec.universe));
  ...
  const ranked = surfaced
    ...
    .sort((a, b) => {
      ...
      if (TYPE_ORDER.indexOf(a.opportunityType) !== TYPE_ORDER.indexOf(b.opportunityType)) {
        return TYPE_ORDER.indexOf(a.opportunityType) - TYPE_ORDER.indexOf(b.opportunityType);
      }
  ...
  return {
    ...
    audit: [
      ...
      `${authorized.length} of ${SAMPLE_UNIVERSE.length} files cleared hard gates and evidence threshold.`,
    ...
    summary: {
      ...
      coverage: authorized.length / SAMPLE_UNIVERSE.length,
