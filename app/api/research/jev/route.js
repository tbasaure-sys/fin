...
import { jevCompanyDirectory } from '@/lib/server/jev-company-directory';
export const runtime='nodejs';
export const dynamic='force-dynamic';
export const maxDuration=90;
const local=process.env.NODE_ENV!=='production'&&process.env.BLS_PRIME_STORAGE_BACKEND==='memory';
const handler=createJevHttp({authenticate:requireApiAuthSession,service:jevService,market:researchMarketService,store:thesisRepository,verify:verifyDossier,companyDirectory:jevCompanyDirectory,consume:consumePublicRateLimit,readPortfolio:local?async()=>({status:'available',holdings:[]}):readOwnedPortfolio});
