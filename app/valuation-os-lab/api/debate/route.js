...
function agent(id, label, stance, vote, summary, evidence = [], concerns = []) {
  return {
    id,
    label,
    status: "complete",
...
function buildAgents(ctx) {
  ...
  return [
    agent(
      ...
      voteFromScore(clamp(quality - missing * 0.08, 0, 1)),
      missing
      ...
      [
        ...
        coverage.secCompanyFacts ? "SEC companyfacts loaded" : "SEC companyfacts unavailable",
    ...
    agent(
      ...
      voteFromScore(clamp(0.5 + upside, 0, 1)),
...
function deterministicVerdict(ctx, agents) {
  ...
  const feasibility = isFiniteNumber(ctx.feasibility) ? ctx.feasibility : 0.5;
  let decision = "Watch";
  if (missing) decision = "Not decision-ready";
  else if (votes.constructive >= 3 && isFiniteNumber(upside) && upside > 0.12 && feasibility >= 0.52) decision = "Constructive";
  ...
  return {
    ...
    executive_judgment:
      decision === "Not decision-ready"
        ? `Do not treat ${ctx.ticker} as decision-ready until missing live drivers are filled: ${ctx.missingDrivers.join(", ")}.`
    ...
    strongest_points: agents
      .filter((item) => item.vote === "constructive")
      .flatMap((item) => item.evidence.slice(0, 1))
      .slice(0, 4),
    red_team: agents.flatMap((item) => item.concerns.slice(0, 1)).filter(Boolean).slice(0, 5),
    open_questions: [
        ...
        messages: [
          {
            role: "system",
            content:
              "You are the final orchestrator for Valuation OS. Specialists already produced deterministic, source-aware views. Use only the supplied JSON; do not invent data. Return only valid JSON with keys: decision, executive_judgment, strongest_points, red_team, open_questions, memo_patch.",
          },
          {
            role: "user",
...
async function buildDebate(ctx) {
  ...
  const deterministic = deterministicVerdict(ctx, agents);
  ...
  return {
    version: "valuation_os_debate_v1",
    runtime: "deterministic_agents_plus_single_optional_orchestrator",
