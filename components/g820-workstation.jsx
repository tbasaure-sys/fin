    ...
    <aside className={styles.detail} aria-label={`${copy.passport}: ${detail.identity.name}`}>
      ...
      {detail.researchPlan ? <section className={styles.evidenceSection}>
        ...
        <div className={styles.redTeam}>
          ...
          {(detail.evidence.provenance?.ownerEvidence?.annual || []).slice(0, 2).map((annual, i) => annual.sbc?.source ? <p key={i}><a href={annual.sbc.source} target="_blank" rel="noreferrer">{language === 'en' ? 'Original document' : 'Documento original'} · {annual.end} · SBC {money(annual.sbc.value)}</a> · {annual.sbc.filed ? dateLabel(annual.sbc.filed, language) : language === 'en' ? 'availability unverified' : 'disponibilidad no verificada'}</p> : null)}
        </div>
      </section> : null}
