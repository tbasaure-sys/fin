...
export function ValuationRevision({record,language='es'}){
 ...
 return <section className={styles.results} data-valuation-revision aria-label={title}>
  ...
  {b.position?<p>{number(b.position.quantity)} {en?'shares recorded at both dates: ':'acciones registradas en ambas fechas: '}<strong>{signed(b.position.gapChangeUsd)} USD</strong> {en?'change in the model gap for that quantity. Not realized P&L or portfolio performance.':'de cambio en la brecha del modelo para esa cantidad. No es ganancia realizada ni rendimiento de la cartera.'}</p>:null}
  <details className={styles.details}><summary>{en?'Inspect the changed assumptions and sources':'Revisar los supuestos y fuentes que cambiaron'}</summary>
   ...
   {b.changedFacts.map(k=><div key={k}><h5>{names[k]}</h5>{[before?.financial?.facts?.[k],record.financial?.facts?.[k]].map((f,i)=><p key={i}>{i?(en?'Current: ':'Actual: '):(en?'Previous: ':'Anterior: ')}{f?<><strong>{number(f.value)} {f.unit==='shares'?(en?'shares':'acciones'):f.unit||'—'}</strong><br/>{f.start?`${f.start} → `:''}{f.end}{' · '}<a href={f.url} target="_blank" rel="noreferrer">{en?'Source':'Fuente'} ↗</a></>:<span>—</span>}</p>)}</div>)}
   ...
   {b.position?<p>{en?'Matching quantities at two dates do not establish that the holding was maintained between them. This comparison does not reconstruct intervening trades or transfers.':'La misma cantidad en dos fechas no demuestra que se mantuviera la posición entre ellas. Esta comparación no reconstruye compras, ventas ni transferencias intermedias.'}</p>:null}
