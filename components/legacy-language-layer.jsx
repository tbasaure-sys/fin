...
const TEXT_TRANSLATIONS = {
  ...
  "Open AURORA": "Abrir AURORA",
  "What would make you flip today rather than merely reduce confidence?": "¿Qué te haría cambiar de postura hoy, y no solo reducir tu confianza?",
};

const PATTERN_TRANSLATIONS = [
  [/^([\d,]+) data points$/, (count) => `${count.replace(/,/g, ".")} datos`],
];

function translatePattern(key) {
  for (const [pattern, render] of PATTERN_TRANSLATIONS) {
...
function translateNodeText(node, language, originalTextByNode) {
  ...
  const translated = language === "es" ? TEXT_TRANSLATIONS[key] || translatePattern(key) : null;
  if (translated) {
