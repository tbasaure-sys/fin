...
export function briefInput(){
 ...
 const fact=(value,i,id)=>({value,...dates[i],factId:id,contextId:'context-'+i});
 const values={revenue:[100,120],cfo:[30,40],capex:[10,15],buybacks:[9,16],dividends:[3,5],cashChange:[2,1],cfi:[-12,-17],cff:[-16,-22]};
 const metrics=Object.fromEntries(Object.entries(values).map(([metric,pair])=>[metric,Object.fromEntries(pair.map((value,i)=>{
  const raw={value,unit:'USD',...dates[i],availableAt:source.acceptedAt,accession:source.accession,concepts:[metric],sourceHash:'b'.repeat(64),filingHash:source.sha256,url:source.url};
